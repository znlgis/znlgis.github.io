...
指定 Pi **默认使用的模型供应商**。Pi 支持的供应商列表见[第四章：模型供应商与认证体系](https://znlgis.github.io/ai/pi/04-模型供应商与认证体系/)，常见的值包括：
...
Pi 在启动时会自动解析 `packages` 数组，下载并加载每个包中注册的扩展、技能、提示模板和主题。关于包的编写和分发细节，见[第十章：Pi 包管理与分发](https://znlgis.github.io/ai/pi/10-Pi包管理与分发/)。
...
一个 TypeScript / JavaScript 文件路径数组，指定 Pi 启动时需要加载的**扩展文件**。扩展是 Pi 最强大的定制手段——你可以在扩展中定义新工具、注册生命周期钩子、注入自定义系统提示等。详细开发方法见[第九章：Extensions 扩展开发](https://znlgis.github.io/ai/pi/09-Extensions扩展开发/)。
...
一个路径数组，指向要加载的**技能文件**（`SKILL.md`）。技能是 Pi 的核心能力之一——每个技能文件包含一段结构化的提示词，告诉模型"如何完成某类特定任务"。详见[第八章：Skills 技能系统](https://znlgis.github.io/ai/pi/08-Skills技能系统/)。
...
主题系统的完整定制指南（包括完整的自定义主题 JSON 结构、颜色字段详解、热重载机制）请见 [第八章：Skills 技能系统](https://znlgis.github.io/ai/pi/08-Skills技能系统/) 的 8.8 Themes 主题系统小节。
...
**临时覆盖 API Key**。这个参数会覆盖 `~/.pi/agent/credentials.json` 中存储的 API Key（凭证文件的详细说明见[第四章](https://znlgis.github.io/ai/pi/04-模型供应商与认证体系/)）。
...
下一章：[第六章：交互式TUI深入](https://znlgis.github.io/ai/pi/06-交互式TUI深入/) 将深入 Pi 的**交互式 TUI**——斜杠命令体系、键盘快捷键、会话管理、checkpoint/rollback、以及 profile 切换等日常使用中最高频的操作。
