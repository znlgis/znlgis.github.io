...
|----------|----------|-------------|----------|
| **Extensions（扩展）** | `.ts` / `.js` | 注册新工具、命令、生命周期钩子、TUI 组件 | [第九章](09-Extensions扩展开发) |
| **Skills（技能）** | `SKILL.md` | 告诉模型"如何完成某类特定任务"的渐进式文档 | [第八章](08-Skills技能系统) |
| **Prompt Templates（提示模板）** | `.md` | 追加到系统提示末尾的行为约束或风格指南 | [5.6.4](05-配置体系全解#564-prompts) |
| **Themes（主题）** | `.json` | 定义 TUI 各组件的颜色方案 | [5.6.5](05-配置体系全解#565-themes) |
...
安装的包会被记录在 `settings.json` 的 `packages` 数组中。第五章（[5.6.1](05-配置体系全解#561-packages)）已简要介绍过这个字段，这里复习并深化：
...
路径是相对于包根目录的。每个 `.ts` / `.js` 文件必须导出一个默认函数，接收 `ExtensionAPI` 对象并在其上注册工具、命令或事件监听器。具体 API 参见[第九章：Extensions 扩展开发](09-Extensions扩展开发)。
...
技能文件遵循 [agentskills.io](https://agentskills.io) 标准。Pi 在启动时只加载技能名称和一句话摘要，完整内容在 Agent 首次使用时才按需加载（渐进式披露）。详细技能编写规范见[第八章：Skills 技能系统](08-Skills技能系统)。
...
`packages` 字段已在第五章（[5.6.1](05-配置体系全解#561-packages)）中介绍过。这里从"包管理"的角度补充一些细节：
...
4. **最小权限原则**：
   ...
   - 对于包含 Extensions 的包，这些 Extensions 可以执行任意 shell 命令、读写任意文件——在"完全可信"的项目之外使用它们时，请通过 Docker 容器或 Gondolin 微 VM 运行 Pi（见[第十二章：容器化与安全沙箱](12-容器化与安全沙箱)）。
...
下一章：[第十一章：SDK 嵌入与 RPC 模式](11-SDK嵌入与RPC模式) 将讲解如何通过 SDK 将 Pi 的 Agent 能力嵌入到自己的 Node.js 应用中，以及通过 RPC 协议与非 Node.js 程序集成。
