...
- **不是简单的"包一层 API"。** Pi 底层有 **[pi-ai](https://github.com/earendil-works/pi/blob/main/packages/ai)**（统一多供应商 LLM API），内部处理了跨供应商上下文传递、结构化工具结果分片、增量 JSON 解析、token/成本追踪、缓存命中率统计等复杂工程问题；有 **[pi-agent-core](https://github.com/earendil-works/pi/blob/main/packages/agent/)** 处理 Agent 循环、状态管理、消息队列、传输抽象；有 **[pi-tui](https://github.com/earendil-works/pi/blob/main/packages/tui/)** 实现差异化渲染（differential rendering）的终端 UI 框架。这是一套从底层 LLM API 到顶层 UI 都自己做的"全栈"Agent 系统。
...
| Forks | 13K+ |
| Commits | 4,789+ |
| Releases | 250+（最新：v0.85.1，截至 2026 年 9 月） |
| 许可证 | MIT |
...
| npm 包生态 | 4600+ Pi Packages（截至 2026 年 9 月） |
...
多种安装方式详见 [第二章：安装与环境配置](https://znlgis.github.io/ai/pi/02-安装与环境配置/)。
...
下一章：[第二章：安装与环境配置](https://znlgis.github.io/ai/pi/02-安装与环境配置/) 将从零开始安装 Pi，完成环境配置，并跑通你的第一个 Pi 会话。
