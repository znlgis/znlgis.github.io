...
OpenGIS-Skills 的生命力来自社区共建。一个人的知识边界有限，但当一个 GIS 老手贡献 GDAL 技能、一个 CAD 工程师贡献 FreeCAD 技能、一个 C# 后端贡献 SqlSugar 技能时，整套技能集的价值就会指数级增长。75 个技能只是一个起点——你的贡献才是这个项目真正的未来。
...
- **[gdal](https://znlgis.github.io/gis/gdal/SKILL.md)** — GDAL/OGR 地理空间数据转换和处理 — 详见 `gis/gdal/SKILL.md`
- **[pyqgis](https://znlgis.github.io/gis/pyqgis/SKILL.md)** — QGIS Python API (PyQGIS) 和 Processing 框架 — 详见 `gis/pyqgis/SKILL.md`
...
+ - **[mapserver](https://znlgis.github.io/gis/mapserver/SKILL.md)** — MapServer WMS/WFS 地图服务配置 — 详见 `gis/mapserver/SKILL.md`
```
...
OpenGIS-Skills 的 GIS 分类当前有 24 个技能，覆盖了最核心的数据处理（GDAL、PostGIS、GeoPandas）、Web 服务（GeoServer、QGIS Server基础的但提升不足）、前端可视化（CesiumJS、OpenLayers、MapsUI）等方向，但仍有大量重要的开源 GIS 工具未被覆盖。
...
当前 CAD 分类覆盖了 20 个技能，集中在 FreeCAD 生态和核心几何库，但 3D 建模和工程 CAD 方向仍有明显的空白带。
...
AI 分类当前有 10 个技能，覆盖了 MCP 协议、检索增强生成和一些 Agent 框架的基础能力。随着 2025-2026 年 AI Agent 和大语言模型应用生态的爆发式增长，AI 领域是当前覆盖缺口最大的分类之一。
...
C# 分类当前有 9 个技能，集中在后端 ORM 和 AOP 框架。.NET 生态中仍有许多常用库缺少技能覆盖。
...
| **第五章：GIS 技能详解** | 逐一了解 24 个 GIS 技能 | 覆盖了从 GDAL 数据处理到 CesiumJS 3D 可视化的全栈 GIS 知识 |
| **第六章：CAD 技能详解** | 逐一了解 20 个 CAD 技能 | 覆盖了 FreeCAD Python 脚本到 OpenCASCADE 几何内核的 CAD 开发工具链 |
...
| **第九章：IoT / 3D / Others 详解** | 逐一了解剩余 12 个技能 | 覆盖了物联网、3D 开发、通用工具三大补充领域 |
...
理论需要实践的检验。只有亲手把技能加载进工具、亲眼看到 AI 的回答质量发生变化，你才能真正理解这 75 个 SKILL.md 的价值。哪怕只是加载了一个 GDAL 技能和一个 QGIS 技能，也是一种全新的体验。
...
它的价值不在于技术本身有多先进，而在于"把领域知识结构化、标准化、可复用"这个想法被执行到了极致。当 75 个技能变成 100 个、200 个，当 GIS 的覆盖面从桌面工具扩展到云原生地图服务、从 2D 制图延伸到 3D 点云和数字孪生时，这套技能集将不再是"一个有用的辅助工具"，而是 AI 在 GIS/CAD 领域辅助编程的**基础设施**。
...
> **教程到此结束。** 共 12 章，覆盖 OpenGIS-Skills 的核心理念、三层索引架构、SKILL.md 编写规范、全部 75 个技能详解、AI 工具集成方式、实战工作流组合、最佳实践、以及贡献指南与社区。欢迎提出反馈和改进建议——这本教程本身也是一个开源项目，它的完善同样需要你的参与。
