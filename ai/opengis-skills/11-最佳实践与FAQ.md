...
|------|----------|-----------|----------|----------|
| **小型** | 100-300 行 | 约 1.5K-4K tokens | cad/tongwen、ai/pi、gis/cesiumjs | 几乎无负担，可放心加载 |
| **中型** | 300-800 行 | 约 4K-12K tokens | gis/postgis、cad/clipper2、ai/dify、csharp/sqlsugar | 最常见的技能规模，1-2 个同时加载安全 |
...
第 4 步：必要时横向扩展 → 加载相关 L3 技能
         ├─ 例如使用 gdal 处理矢量时发现几何拓扑问题 → 加载 gis/nettopologysuite
         ├─ 例如使用 pyqgis 时发现需要无头批处理 → 加载 gis/qgis-process
...
举例：你需要把一批栅格影像做投影变换后再在前端三维地球上展示。在根索引中搜索 `#raster #3d` 即可分别定位到 gis/gdal 和 gis/cesiumjs 两个技能——没有这两个标签，你可能需要翻找 75 个技能文件。
...
| 分类 | 功能重叠示例 | 如何选择 |
|------|-------------|----------|
| gis/ | gdal vs pyqgis vs qgis-process 都能做数据处理 | gdal 适合批量脚本，pyqgis 适合 QGIS 生态内自动化，qgis-process 适合无头命令行批处理 |
| cad/ | ifoxcad vs lightningcad vs tongwen 都面向 AutoCAD 二次开发 | 取决于你要做的能力：通用二开框架、围护深化插件、工程图纸翻译 |
...
这一步遵循"最小化原则"：只在技能内部明确引用另一个技能时才加载。例如，gdal-api 技能在"矢量批处理"部分提到"如需了解 QGIS 处理工具箱的算法清单，参见 gis/qgis-process"，此时才有必要加载 qgis-process 技能。

> **为什么不要预先加载所有可能相关的技能？** 因为"可能相关"的范围太大——一个 GDAL 任务可能在几何运算阶段需要 shapely（gis/shapely），在可视化阶段需要 openlayers（gis/openlayers），在证书部署阶段需要 acme.sh（others/acme.sh）。如果预先全部加载，上下文将被无关信息淹没。正确做法是：遇到问题时再按需加载。
...
1. **同一会话可以多次加载不同技能**——当你先加载 gdal-api 完成文件转换，然后加载 cesiumjs 做可视化时，两个技能的内容都会保留在上下文中（直到超出上下文窗口限制）。
...
1. [已完成的关键步骤和代码文件路径]
2. [当前卡在哪个问题上]
3. [之前加载过的技能：gis/gdal-api, gis/geopandas]
请先加载 @gis/gdal-api/SKILL.md 和 @gis/geopandas/SKILL.md，然后帮助我解决 [具体问题]。"
```
...
| 全栈开发 | 加载 gis/cesiumjs + gis/openlayers，负责前端可视化 | 加载 gis/gdal-api + gis/postgis，负责后端服务 |
...
- @.skills/opengis-skills/gis/shapely/SKILL.md
...
- 前端可视化任务：优先加载 gis/cesiumjs 或 gis/openlayers
...
- custom-003: 新增了公司内部坐标系定义 (gis/geopandas/SKILL.md, 第 80 行后)
...
| # | 问题 | 答案 |
|---|------|------|
| 1 | **如何知道该加载哪个技能？** | 最可靠的方式是两步定位法。第一步：加载根索引 `@SKILL.md`，使用标签搜索功能定位大类。例如你的问题是"把 GeoJSON 转成 Shapefile"，在根索引中搜索 `#conversion #vector` 即可定位到 `gis/gdal` 和 `gis/geopandas`。第二步：加载 L2 索引确认具体技能，然后加载 L3 技能执行任务。如果标签搜索不生效（部分工具不支持），直接在提示词中描述你的需求，让 AI 推荐应该加载哪个技能。 |
| 2 | **多个技能同时加载会冲突吗？** | 不会冲突。opengis-skills 的每个技能文件是独立的知识单元，设计上遵循"无副作用"原则——技能之间不存在互相依赖或覆盖关系。你可以安全地同时加载 2-3 个 L3 技能（例如同时加载 `gis/gdal-api` 和 `gis/geopandas`，一个做格式转换一个做矢量分析）。唯一的限制是 Token 预算（见 11.1.1），建议同时加载的技能不超过 3 个。 |
...
| 19 | **对于大型项目，如何组织技能加载策略？** | 大型 GIS 项目通常涉及多个技术栈（数据处理 + 后端服务 + 前端可视化 + 部署运维），一次性加载所有相关技能必然超出 Token 预算。推荐**分阶段加载策略**：按项目的开发阶段，逐个加载对应技能。示例如下——**阶段 1（架构设计）**：加载 `ai/superpowers-zh`（头脑风暴方法论），设计系统架构和模块划分，此时不需要加载具体的工具技能。**阶段 2（数据处理）**：加载 `gis/gdal-api` + `gis/postgis`，完成 ETL 脚本开发和数据库设计。**阶段 3（后端服务）**：加载 `gis/geoserver` 或 `csharp/furion`（取决于技术选型），实现 API 服务。**阶段 4（前端可视化）**：加载 `gis/cesiumjs` 或 `gis/openlayers`，实现地图可视化。**阶段 5（部署）**：加载 `gis/geoserver-cloud`，完成容器化与云端部署。每个阶段使用独立的 AI 会话，上下文保持专注，Token 消耗可控。 |
...
| 22 | **AI 生成的代码不准确怎么办？** | 代码不准确有三种常见原因。（1）**技能没加载对**——你可能加载的是 pyqgis 技能但在写 GDAL 代码，检查当前会话中加载的技能是否匹配你的任务。（2）**技能版本与工具版本不匹配**——技能基于 GDAL 3.8 编写，但你的环境是 GDAL 3.4，部分 API 不兼容。解决方案：在提示词中明确指定版本"请生成兼容 GDAL 3.4 的代码"。（3）**技能内容与官方文档有偏差**——技能可能未及时更新到最新 API。解决方案：在 AI 回答后手动对照官方文档验证关键 API 调用。如果发现持续不准确，向仓库提交 Issue。 |
...
2. **加载了错误的技能**（例如加载了 pyqgis 但在写 GDAL 代码）
...
第 1 层：确认技能与任务匹配
  ...
  □ AI 生成的是 pyqgis 代码 → 你是否加载了 gis/pyqgis 技能？
  □ 如果不匹配 → 重新加载正确技能
...
**判断"是否需要辅助技能"的简单测试**：
- 当前技能文件中明确提到了另一个技能名（如"详细参考 gis/geopandas"）→ 加载
...
**适用此技巧的大技能**：`gis/gdal-api`（~1500 行）、`ai/opencode`（~1200 行）、`gis/opengis-all`（~900 行）、`csharp/sqlsugar`（~800 行）。
...
| 修复一个 Bug，涉及 gdal-api 和 geopandas 两个技能 | 同会话 | Bug 是连接两个技能的纽带 |
