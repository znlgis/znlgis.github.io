...
在 opengis-skills 仓库中，三层索引架构是整个系统设计的灵魂。它决定了 AI 工具如何发现、加载和利用 75 个技能文件，是所有上层功能的基础。本章将从设计动机出发，逐层深入解析 L1（全局入口）、L2（分类索引）和 L3（项目技能）的结构与协作方式，并通过完整的 AI 加载决策流程展示这套架构在真实场景中的运作机制。
...
所有主流 AI 编程助手——Claude、Cursor、Cline、GitHub Copilot Chat、DeepSeek Chat——都有一个共同约束：**上下文窗口（context window）是有限的且按 Token 计费**。opengis-skills 仓库包含 75 个技能文件，每个 SKILL.md 文件从 200 行到 1500 行不等。如果 AI 工具在每次对话中都全量加载 75 个文件，会产生以下问题：

| 问题 | 影响 |
|------|------|
| Token 消耗爆炸 | 75 个文件 × 平均 800 行 × 约 40 Token/行 ≈ 210 万 Token，远超大多数模型的窗口上限 |
...
- **递增精度**：从 L1（75 个技能的全貌）→ L2（一个分类下的 8-24 个技能）→ L3（1 个技能的深度知识），每一步都缩小范围
...
|------|------|---------|------|---------|----------|
| L1 | 全局入口 | 根目录 `SKILL.md` | 1 个 | 75 个技能的全量索引 + 标签搜索系统 | 不确定具体工具时，先加载此文件获取全貌 |
| L2 | 分类索引 | `gis/SKILL.md`、`cad/SKILL.md`、`csharp/SKILL.md`、`ai/SKILL.md`、`iot/SKILL.md`、`3d/SKILL.md`、`others/SKILL.md` | 7 个 | 该分类下 1-24 个技能的概要 + 领域概述 | 明确大类但不确定具体工具 |
| L3 | 项目技能 | `<category>/<project>/SKILL.md` | 75 个 | 单个项目的深度知识（API、工作流、FAQ） | 已确定使用哪个工具 |
...
opengis-skills/
│
├── SKILL.md                 ← L1：全局入口（75 个技能全量索引 + 标签搜索）
│
├── gis/                     ← 分类目录
│   ├── SKILL.md             ← L2：GIS 分类索引（24 个技能概要）
│   ├── gdal/                ← 子项目
...
│
├── cad/
│   ├── SKILL.md             ← L2：CAD 分类索引（20 个技能概要）
│   ├── freecad/
│   │   └── SKILL.md         ← L3：FreeCAD 技能
│   └── ...（共 20 个子项目）
│
├── csharp/
│   ├── SKILL.md             ← L2：C# 分类索引（9 个技能概要）
│   └── ...（共 9 个子项目）
│
├── ai/
│   ├── SKILL.md             ← L2：AI 分类索引（10 个技能概要）
│   └── ...（共 10 个子项目）
│
├── iot/
...
**总 Token 消耗**：L1（12K）+ L2（8K）+ L3（18K）= 约 38K Token。相比于全量加载 75 个文件（约 210 万 Token），节省了 **98.2% 的上下文窗口**。
...
```
提问："我想做空间数据处理，用什么好？"
  → L1：加载根 SKILL.md，浏览 75 个技能概览
  → L2：发现 GIS 分类有 24 个技能，加载 gis/SKILL.md
...
expertise for 75 open-source projects. One-stop skill index with tag-based search
...
由于根 SKILL.md 是全局入口，它必须用**最紧凑的信息密度**呈现所有 75 个技能的概况。为此采用了一张按分类组织的导航表：
...
2. **按分类分组**：GIS 24 个、CAD 20 个、C# 9 个、AI 10 个、IoT 1 个、3D 5 个、Others 6 个——共七个分类，用 emoji 和中文标题区分
...
L2 分类索引是 L1 和 L3 之间的**减震层**——当用户问题过于模糊无法直接定位到 L3 时，L2 提供了一个中等粒度的视图，用 200 行的文件覆盖一个领域内 1-24 个技能的关键信息。

仓库共有 7 个 L2 索引文件，对应 7 个分类目录：

| 分类索引 | 技能数量 | 领域覆盖 |
|---------|---------|---------|
| `gis/SKILL.md` | 24 | 空间数据处理、地图服务、Web GIS |
| `cad/SKILL.md` | 20 | 参数化建模、几何内核、BIM/PCB |
| `csharp/SKILL.md` | 9 | .NET 框架、ORM、Office 操作 |
| `ai/SKILL.md` | 10 | LLM 应用、Agent 框架、AI 编程方法论 |
| `iot/SKILL.md` | 1 | 物联网与嵌入式 |
| `3d/SKILL.md` | 5 | 3D 高斯泼溅、AEC/BIM 三维数据处理 |
| `others/SKILL.md` | 6 | Go 语言、RPA 自动化、邮件平台、Java 脚手架、SSL 证书 |

### 2.4.1 以 `gis/SKILL.md` 为例深度解析

`gis/SKILL.md` 是最大的 L2 索引文件，涵盖 24 个 GIS 技能。它的结构代表了所有 L2 索引的标准模式。
...
20 个技能按职能分为 7 个子类：几何内核与算法（3 个）、参数化 3D CAD（5 个）、2D CAD 与制图（3 个）、PCB/EDA 设计（1 个）、BIM 与 IFC（2 个）、.NET AutoCAD 开发（4 个）、数据交换与可视化（2 个）。

**`csharp/SKILL.md`**（9 个技能）、**`ai/SKILL.md`**（10 个技能）：结构与上述一致，规模较小。每个都有独立的快速导航表和相关分类引用。

**`iot/SKILL.md`**（1 个技能）、**`3d/SKILL.md`**（5 个技能）、**`others/SKILL.md`**（6 个）：虽然技能数量较少，但仍遵循完全一致的 L2 模板。模板的一致性确保了 AI 工具在跨分类切换时不需要切换解析策略。
...
- 主文件和 reference 之间用相对路径链接：`[reference/vector-tools.md](https://znlgis.github.io/ai/opengis-skills/02-三层索引架构/reference/vector-tools/)`
...
所有 75 个 L3 技能遵循统一的编写规范（详见第三章），核心约束为：
...
三层索引和 RAG 并非互斥关系。在实际使用中，可以结合两者：**RAG 用于初步搜索（找到语义相关的技能），三层索引用于精确定位和深度加载**。opengis-skills 当前选择了纯文件系统的标签搜索方案，是因为 GIS/CAD/3D 等领域的工具数量有限（75 个），结构化标签已经足够精准，不需要借助 Embedding 的模糊匹配能力。
...
提示词模板适用于**3-5 个高频场景的速查**，三层索引适用于**50+ 个工具的系统化管理**。opengis-skills 的 75 个技能如果以扁平列表的方式组织，AI 工具需要线性扫描整个列表才能定位目标——这正是三层架构要解决的"搜索成本随技能数量线性增长"的问题。
...
2. **粒度匹配**：L1 提供 75 个技能的全局概览，L2 提供某个领域的 1-24 个技能概要，L3 提供单个工具的深度知识——每一层刚好匹配一种典型的知识需求
