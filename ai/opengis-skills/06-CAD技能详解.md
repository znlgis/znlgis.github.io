...
CAD（计算机辅助设计）是 OpenGIS-Skills 中第二大技能分类，共收录 **20 个技能**。与 GIS 领域侧重空间数据处理不同，CAD 类技能覆盖了从底层几何内核到上层的建筑设计应用、从 2D 制图到 BIM 信息模型、从多平台桌面应用到 Web 端轻量方案的完整链条。无论你是在开发一个几何算法库，还是在做 AutoCAD 插件二次开发，或是构建一套在线 3D 查看器，本章都能帮你快速定位到最匹配的技能。

本章按工具的技术层次和应用场景，将 20 个技能划分为七个子领域：几何内核与算法、参数化 3D CAD、2D 制图、PCB/EDA、BIM 与 IFC、.NET AutoCAD 二次开发、数据交换与可视化。每个技能都会给出核心定位、适用场景和关键 API 或代码示例，让你在面对具体任务时能够快速决策"我应该加载哪个技能"。

---

## 6.1 CAD 技能全景

下表给出了全部 20 个 CAD 技能的快速索引。你可以按"分类"列快速定位到感兴趣的子领域，也可以按"等级"列判断技能的技术深度——"内核"级最深，"框架/应用"级居中，"插件"级最贴业务。
...
面对 20 个技能，选择哪个取决于你的任务类型。以下决策树帮你快速定位：
...
本章详细解析了 opengis-skills 中全部 20 个 CAD 技能。如果只能记住三个关键点，请记住：
