...
> **版本说明**：本章对应仓库当前主线（v4.0.0 及后续重构）。相比早期版本（v23）：`/search`、`/consult` 已移除，新增了 `/vision`、`/learn`；`/simplify` 的路由从 oracle 改为 light-orchestrator（两阶段：spawn oracle 只读分析 → 自身应用编辑）；2026-09 审查体系重构——`/review` 改为 flash 初检两级模型（默认 light-orchestrator 跑 Tier 1，命中升级触发条件才委派 pro 的 reviewer），独立的 `/review-pr` 移除（PR 回帖并入 `/review` 与 `/deep-review`），新增 `/deep-review`（强制 pro 全量审查）；命令总数为 **18 个**。
...
5. **模型自动匹配**：命令通过 Agent 间接绑定模型——Pro 处理深度推理（deep-worker、oracle、reviewer；主 Agent solo 跟随会话模型，默认 pro），Flash（原生多模态）处理路由、规划、检索、轻量执行与多模态。路由时自动选择最佳模型
...
| 分类 | 命令数 | 典型场景 |
|------|--------|----------|
| Agent 路由命令 | 8 | 直接指定 Agent 处理特定类型任务 |
| 操作命令 | 4 | 提交、发版、配置反思、会话交接 |
| 内联命令 | 4 | 结构图、经验沉淀、简化、清理 |
| 规约命令 | 2 | spec-workflow 提案与实施 |

> **历史沿革**：早期版本（v23）将命令分为 9 大类共 18 个，包含 `/search`（外部搜索）、`/consult`（方案咨询）、`/review-pr`（PR 审查回帖）。v38 精简为 4 大类 17 个：`/search` 的能力由 explore/librarian 的只读探索覆盖，`/consult` 由 consultant Agent 直接路由覆盖。2026-09 审查体系再调整：`/review` 改为 flash 初检两级模型、`/review-pr` 移除（PR 回帖并入 `/review`）、新增 `/deep-review` 强制 pro 全量审查，当前共 18 个。
...
| Agent 模型 | `deepseek/deepseek-flash` |
| 触发技能 | 无 |
| 权限 | 读写 |
...
|------|-----|
| 路由 Agent | `vision` |
| Agent 模型 | `deepseek/deepseek-flash`（原生多模态） |
| 触发技能 | `vision-prep` |
| 权限 | 读写（仅单处轻量视觉修正） |
...
**用途**：读取并理解图像、截图、图表、UI 设计稿等多模态内容。vision 运行在 `deepseek-flash` 上（原生多模态），是配置中专职"看图"的 Agent；视觉输入是 opt-in 的——只有用户明确提供图像时才走此命令。
...
**注意事项**：
- vision 以描述看到的内容为主，绝不臆造图片中不存在的信息
- 单处轻量视觉修正（如按设计稿改一个颜色）在其范围内；需要深度推理或多文件改动时升级到 deep-worker
- 图像输入有成本上限控制（`attachment.image` 配置：超过 1600px / 2MB 会自动缩放），超大图片建议先用 `vision-prep` 技能预处理

---

#### `/review` —— 代码审查（flash 初检，按需升级 pro）

| 属性 | 值 |
|------|-----|
| 路由 Agent | `light-orchestrator`（Tier 1）→ 命中触发条件时委派 `reviewer`（Tier 2） |
| Agent 模型 | Tier 1 `deepseek/deepseek-flash`；Tier 2 `deepseek/deepseek-v4-pro` |
| 触发技能 | `code-review`（PR 模式额外加载 `gh-cli`） |
| 权限 | 只报告不改代码（template 明确 `Do not modify code`） |
| template | `Load the code-review skill and run the tier-1 (flash) pass. Scope the change set, compute effective size, and pick the path per the skill. ... If any escalation trigger fires (Full path, high-stakes override, trust boundary, or a critical/major finding you cannot confirm from the diff alone), delegate to the reviewer subagent (pro) and pass your findings as unverified leads. If a PR ref or URL is present in the arguments, also load the gh-cli skill and post findings as a pending GitHub review ... (event=COMMENT, never auto-approve). Do not modify code.` |

**用途**：对当前改动进行多维度、分严重程度的代码审查，采用 **flash 初检 → 按需升级**的两级模型：Tier 1 走 Abbreviated 路径（≤8 个逻辑文件且 ≤300 有效行、无高风险触发）时直接出**完整报告**（干净结果不因"再确认一下"而升级）；命中升级触发条件（Full 路径 / 高风险 / 信任边界 / 需跨文件确认的 critical-major）才委派 pro 的 `reviewer`，并把 Tier 1 发现作为**未验证线索**传递、避免重复推导。小 diff 走 flash 初检约省 **67%** 成本。支持两种模式：
...
> **历史沿革**：早期版本（v23）曾将 PR 审查拆为独立的 `/review-pr` 命令，后合并进 `/review`（通过参数是否含 PR ref 自动切换模式）；2026-09 起 `/review` 的默认执行层从 reviewer（pro）降为 light-orchestrator（flash）初检，强制 pro 全量审查由独立的 `/deep-review` 承担。
...
**注意事项**：
- 两级审查都不改代码。如果审查发现的问题需要修复，请手动使用 `/deep` 或 `/quick` 处理
- **scope-first 门控**：如果 diff 很大（>500 有效行）或过于琐碎，会先报告一个审查计划并停止，而不是盲目深审——避免浪费 token；Full 路径本身即是升级触发条件之一
...
#### `/deep-review` —— 强制 pro 全量审查

| 属性 | 值 |
|------|-----|
| 路由 Agent | `reviewer` |
| Agent 模型 | `deepseek/deepseek-v4-pro` |
| 触发技能 | `code-review` + `gh-cli`（带 PR ref 时） |
| 权限 | **只读**（oracle/reviewer 等只读 Agent 之一） |
| template | `Load the code-review skill and run the Full path regardless of size. If a PR ref or URL is present in the arguments, also load the gh-cli skill and post findings as a pending GitHub review per its 'Reviewing PRs' section (event=COMMENT, never auto-approve). Report findings by severity. Do not modify code.` |

**用途**：跳过 flash 初检，强制 `reviewer` 按 Full 路径全量审查。适用场景：大 diff、触及信任边界（认证/授权、迁移、并发、公共 API）、或用户明确要求深度审查。带 PR ref/URL 时同样支持 GitHub 回帖（event=COMMENT）。

**使用示例**：
```text
/deep-review 全量审查 PR #42
```
...
/review       → flash 初检，输出分级报告（命中触发条件自动升级 pro）
```
...
/review #42 → 带 PR ref：flash 初检 + 按需升级，回帖 GitHub（event=COMMENT）

# 场景B：本地审查（默认最省）
/review     → flash 初检本地改动，小 diff 直接出完整分级报告
/deep-review→ 跳过初检，强制 pro 全量审查（大 diff / 高风险 / 信任边界时用）
