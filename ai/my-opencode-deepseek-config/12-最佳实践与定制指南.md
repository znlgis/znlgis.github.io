...
将 `/review` 设为团队的代码审查前置步骤。`/review` 在 v38 起合并了 PR 审查与本地 diff 审查两种模式：
...
| 只读（Read-Only） | 探索/分析类 Agent（oracle, reviewer, explore, librarian, vision） | `permission.task: "*": deny` + bash allow-list 仅放行 `git status/diff/log/show`、`rg`、`Get-ChildItem`、`Get-Content`，其余 `*: deny` |
