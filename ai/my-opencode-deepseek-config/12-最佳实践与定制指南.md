...
而使用命令别名（如 `/review`、`/commit`）时，路由是**确定性的**——直接跳转到目标 Agent，跳过 Orchestrator 的推理步骤。
...
而预先规划验证路径后，实现是一次到位的。AGENTS.md 的 **Self-Verification** 与 **Quality Bar** 规则要求实现前先规划最窄的验证路径——在动手前明确：
- 修改后将影响哪些调用者
- 需要运行哪些测试
- 需要检查哪些边界条件
- 验证通过的客观标准是什么
...
将 `/review` 设为团队的代码审查前置步骤。`/review` 采用 flash 初检两级模型，并合并了 PR 审查与本地 diff 审查两种模式：

1. 开发者完成功能后，先本地运行 `/review`（本地 diff 模式），修复发现的所有 Critical/Major 级别问题
2. 创建 PR 后，运行 `/review <PR 引用>`（PR 模式），审查层加载 `gh-cli` 技能并把审查结果作为 GitHub Review 回帖（`event=COMMENT`，绝不自动 approve）；大 PR 需要 pro 全量时用 `/deep-review`
3. 人工 Reviewer 聚焦于架构和业务逻辑——语法和反模式问题已被 AI 处理

> **版本说明**：早期版本用独立的 `/review-pr` 命令承载 PR 审查，该命令已被移除——传入 PR 引用即进入 `/review` 的 PR 模式自动回帖，否则审查本地 diff；2026-09 起 `/review` 默认由 light-orchestrator（Flash）做 Tier 1 初检，命中升级触发条件才委派 reviewer（Pro），另有 `/deep-review` 直接强制 pro 全量审查。
...
- Use deepseek/deepseek-flash for simple query formatting
...
Agent 通过 `.md` 文件的 YAML frontmatter 声明元数据，无需在 `opencode.jsonc` 中注册。frontmatter 支持 `mode`（primary/subagent）、`model`、`steps`、`color`、`permission` 等字段：
...
> **版本说明**：早期版本在 `opencode.jsonc` 的 `agents` 字段中用 `name/model/prompt/permissions.files` 结构注册 Agent，v20 起改为 `agents/<name>.md` 文件 + frontmatter 声明，v38 的权限模型也从 `files.allow/ask/deny` 数组演进为 `permission.read/bash/skill` 对象形式（bash 采用 allow-list + ask-list + 兜底 `*: ask` 的 last-match-wins 结构）。
...
| 只读（Read-Only） | 探索/分析类 Agent（oracle, reviewer, explore, librarian） | `permission.edit: deny` + `permission.task: deny` + bash 兜底 `*: deny` 置前、仅放行只读子命令（`git status/diff/log/show/blame/grep`、`rg`；oracle/reviewer 另放行 `gh pr view/diff`、`gh issue view`、`gh api`；librarian 无任何 bash 白名单） |
...
|------|---------------|---------|---------|
| `deepseek/deepseek-v4-pro` | deep-worker、oracle、reviewer（主 Agent solo 无 model 字段，跟随会话所选模型，默认即 pro） | 默认开启（深度推理） | 较高（输入 $0.66/1M） |
| `deepseek/deepseek-flash`（原生多模态） | orchestrator（主）、planner、light-orchestrator、consultant、ui-builder、explore、librarian、vision | 关闭（temperature 0）；planner/light-orchestrator 经 `reasoningEffort: low` 重开轻量思考 | 较低（输入 $0.22/1M，约 1/3） |
...
| subagent_depth | 保持默认 2 | 恰好覆盖最深链路 orchestrator → light-orchestrator → oracle；更深嵌套只会放大 token |
...
| DeepSeek V4 Flash（原生多模态，原 Flash-Vision-Exp 已并入） | $0.22 | $0.66 | $0.007 | 约 0.33x |

**关键洞察**：Flash 的输入价格约为 Pro 的 1/3，而**缓存读取**（cache_read）比普通输入便宜约 30 倍（Pro $0.022 vs $0.66）——这正是 AGENTS.md 强调"字节稳定前缀"以最大化 prompt-cache 命中的原因。如果 70% 的任务能用 Flash 完成，总体成本可以降低约 50%。仓库提供 `scripts/estimate-cost.js` 脚本可按输入/缓存/输出 Token 量精确估算成本。
...
本配置当前使用 `subagent_depth: 2`：Orchestrator → Subagent → 孙 Subagent 最多两层——恰好覆盖实际最深链路（orchestrator → light-orchestrator → oracle/reviewer），其余 subagent 一律 `task: deny`。第 3 层曾存在但无人使用，只会白送意外嵌套的 token 放大面，因此被砍掉。

如果你确实需要更深的委派链，可以调回 `subagent_depth: 3`——但每加一层，每轮上下文都会携带完整父链，Token 消耗随之上升。另注意：桌面 v2 端需同时设置同值的 `experimental.subagent_depth`（classic 键对 v2 无效）。
...
- 路由策略基于 DeepSeek 的双模型特性设计（Pro / 原生多模态 Flash），混用会打乱路由逻辑
- Token 成本估算失去参考意义
...
**A**: 原 DCP 插件已移除，压缩由 OpenCode **内置 compaction 独立承担**（唯一压缩层），其设计目标是保留关键信息、去除冗余：
- 每轮 `prune` 只清除过时工具输出（保留最新约 40K tokens 工具输出、保护 `skill` 输出，且只在能释放 >20K tokens 时才动作）
- 触发压缩时，`compaction` Agent（Flash）摘要会话头部，同时按 `tail_turns: 8` / `preserve_recent_tokens: 12000` 保留最近对话的**逐字尾部**
- 重复的工具结果被 prune 消化；关键决策、当前进度、待办事项落在受保护的尾部区

一般丢失关键上下文的概率很低。若你发现特定场景被压损，优先调大该模型的 `limit.input` 工作窗口（推迟触发点），而非引入第三方压缩插件。
...
### Q: `subagent_depth=2`（当前配置）的实际意义？

**A**: `subagent_depth` 控制任务委托的最大嵌套层级：

- `depth=1`：只有 Orchestrator 可以直接分派任务。Worker 不能再委托子任务。
- `depth=2`（本配置取值）：允许两层委托——恰好覆盖实际最深链路 `orchestrator → light-orchestrator → oracle/reviewer`。
- `depth=3`：三层委托。曾有，但除最深链路外无 Agent 使用（其余 subagent 一律 `task: deny`），只会放大意外嵌套的 token 面，v4.0.0 后降为 2。

**实际影响**：本配置里大多数委派在 depth=1 完成；`/simplify` 与 `/review` 升级是仅有的 depth=2 场景。超过 2 层的嵌套通常意味着任务分解有问题。另注意：桌面 v2 只认同值的 `experimental.subagent_depth`，两处需并存。
...
- name: AI Code Review
  run: |
    opencode run --non-interactive --command "/review" \
      ...
      --max-tokens 30000
  env:
    DEEPSEEK_API_KEY: ${% raw %}{{ secrets.DEEPSEEK_API_KEY }}{% endraw %}
```
...
| solo | 跟随会话（默认 Pro） | 读写 | 单模型内联执行、零委派——需要全程单一模型时的主入口（无 model 字段） |
...
| vision | Flash（原生多模态） | 读写（限轻量视觉修正） | 多模态图像识别、截图/图片解读（深度改动转交 deep-worker） |
...
| `/review` | 代码审查（flash 初检，命中触发条件升级 pro；PR 模式自动回帖 / 本地 diff 模式） | light-orchestrator + code-review →（按需）reviewer + gh-cli |
| `/deep-review` | 强制 pro 全量审查（跳过 flash 初检） | reviewer + code-review（+ gh-cli） |
| `/plan` | 制定技术方案 | planner |
...
| grilling | 需求澄清（一次一问、多选优先；已并入原 wait-what 复述确认与 grill-with-docs 术语锐化分支） | 需求模糊、消息令人困惑时 |
