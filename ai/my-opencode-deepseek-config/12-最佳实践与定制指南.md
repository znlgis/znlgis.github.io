...
而预先规划验证路径后，实现是一次到位的。在动手前先通过 `verification-planning` 技能明确：
- 修改后将影响哪些调用者
- 需要运行哪些测试
- 需要检查哪些边界条件
- 验证通过的客观标准是什么
...
```bash
# 审查 PR 并自动回帖到 GitHub（逐行评论）
/review-pr  # 使用 gh-cli + code-review 技能，将审查结果发布为 GitHub Review
...
| Agent 名称 | 模型 | 权限 | 核心用途 |
|-----------|------|------|---------|
| orchestrator | Pro | 读写 | 意图门控、任务路由、调度决策——整个系统的"大脑"（主入口） |
| planner | Pro | 读写 | 战略规划、架构设计、项目拆解——"设计然后建造" |
| deep-worker | Pro | 读写 | 重型多文件实现、复杂重构——系统的"主力工程师" |
| oracle | Pro | 只读 | 深度代码分析、根因诊断、代码简化分析——"透视眼" |
| reviewer | Pro | 只读 | 多维度代码审查、安全审查——"质量守门人" |
| consultant | Pro | 读写 | 技术顾问、方案对比、最佳实践建议——"智囊" |
| ui-builder | Pro | 读写 | 前端 UI 组件、样式、布局、交互——"界面专家" |
| explore | Flash | 只读 | 代码库搜索、文件探索、信息检索（内部隐藏） |
| librarian | Flash | 只读 | 外部文档检索、Web 搜索、API 研究（内部隐藏） |
| light-orchestrator | Flash | 读写 | 轻量单文件编辑、简单修改——日常琐事的"万能工" |
...
| `/review` | 双轴并行代码审查 | reviewer + code-review |
| `/review-pr` | 审查 PR 并回帖 GitHub | reviewer + code-review + gh-cli |
| `/plan` | 制定技术方案 | planner |
| `/search` | 外部搜索与文档检索 | librarian |
| `/oracle` | 深度分析与根因调试 | oracle |
| `/consult` | 方案讨论与决策支持 | consultant |
...
| `/simplify` | 行为保持的代码简化 | oracle + simplify |
...
| verification-planning | 实现前规划验证路径 | 实现复杂变更前手动加载 |
...
| shared-language | 构建共享术语表 | 反复解释相同概念时 |
| writing-great-skills | 编写高质量技能文件 | 创建/编辑技能时 |
