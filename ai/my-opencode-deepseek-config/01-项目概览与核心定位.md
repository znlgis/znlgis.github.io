...
展开来说：这是一个**纯配置驱动、零额外依赖**的 OpenCode 多 Agent 配置仓库。它把 `opencode.jsonc`、`agents/*.md`、`skills/*/SKILL.md`、`AGENTS.md` 这四类纯文本文件组织成一个有机整体，将 DeepSeek V4 双模型（Pro + 原生多模态 Flash）在 OpenCode 多 Agent 框架下的能力发挥到极致。

核心理念只有四个字：**Token 效率优先**——用最小的上下文成本达到最好的开发效果。这不是一个花哨的"AI 生产力工具"，而是一套经过 38+ 个迭代阶段（v1–v38+）打磨出来的务实方案：路径引用替代文件粘贴、技能按需加载而非全量注入、内置 compaction 显式工作窗口替代隐式截断、AGENTS.md 全局规则持续精简（当前 311 行）——每一步都在追问"能不能少花一点 token"。
...
**本方案的答案**：通过 `provider.deepseek` 配置将模型矩阵锁定在 DeepSeek V4 家族（pro / flash，其中 flash 为原生多模态），并在 AGENTS.md 中明确"不引入新模型"的约束。同时保留扩展性——换模型只需修改 provider 配置，Agent 和 Skills 体系零耦合。
...
**本方案的答案**：贯彻 Token 效率优先——Skill 通过原生 `skill` 工具按需加载（只在触发时才注入上下文）；上下文压缩仅用内置 compaction，由 `limit.input` 显式声明工作窗口（flash 115K / pro 148K tokens）设定触发点，每轮再经 prune 清理旧工具输出；AGENTS.md 明确要求"引用路径，不粘贴文件"；甚至在 Agent 系统提示的设计上也做了去重（v20 版本将 AGENTS.md 精简 22%）。
...
| 文件类型 | 作用 | 示例 |
|---------|------|------|
| `opencode.jsonc` | 核心配置：模型绑定、Agent 定义、权限基线、实验功能开关 | DeepSeek 双模型绑定、subagent_depth: 2、权限 deny/ask 策略 |
| `agents/*.md` | 12 个 Agent 的系统提示词，定义角色、能力、约束 | orchestrator.md 包含意图门控逻辑和完整路由表 |
| `skills/*/SKILL.md` | 23 个可复用技能的详细指令，按需加载 | code-review/SKILL.md 包含双轴并行审查维度、严重度分级、熵扫描和收敛检查 |
...
| 模型 | 角色 | 典型用途 |
|------|------|---------|
| `deepseek/deepseek-v4-pro` | 推理与决策引擎 | 根因分析、代码审查、重型多文件实现 |
| `deepseek/deepseek-flash` | 查询与执行引擎（原生多模态） | 编排路由、规划、代码库搜索、文档检索、轻量单文件编辑、交接文档生成；读取图片/截图/图表（原 flash-vision-exp 已并入此模型） |

路由策略确保"杀鸡不用牛刀"：
- **Flash 优先**：搜索、查找、简单编辑等明确定义的任务优先走 Flash Agent
- **Pro 专注推理**：规划、分析、审查、复杂实现只用 Pro
- **Vision 专项**：涉及图像/截图/多模态输入的任务路由到 vision Agent（`deepseek-flash`，原生多模态）
...
| `solo` | 跟随会话所选模型（默认 pro） | 读写 | 单模型内联执行器：零委派（不派子 Agent、不用后台助手），直接完成分析/实现/验证 |
...
| `vision` | flash（原生多模态） | 多模态：读取图片、截图、图表；可执行单处轻量视觉修正 |
...
技能（Skill）是本方案的能力模块。OpenCode 通过原生 `skill` 工具按需暴露——Agent 只在需要时才加载，不会常驻上下文。23 个 Skill 覆盖了从代码审查到版本发布的全流程：
...
- `grilling` — 需求澄清（一次一问，多选优先；已合并原 `wait-what` 复述确认与 `grill-with-docs` 术语锐化两条分支）
...
> 注：技能清单随仓库持续演进，完整且最新的 23 个技能列表请以仓库 `skills/` 目录与 README 为准。
...
2. **技能按需加载**：23 个 Skill 通过 `skill` 工具按需加载，不会在每次对话时全部注入上下文。一个代码审查任务不会拖着一整套 Git Release 流程的指令。

3. **内置 compaction 显式窗口**：`opencode.jsonc` 用 `limit.input` 声明各模型工作窗口，触发点 = `limit.input − compaction.reserved`（flash 115,072 / pro 147,840 tokens），另有每轮 `prune` 清理旧工具输出——唯一的压缩层就是内置 compaction，不引入第三方压缩插件。

4. **去重与精简**：v20 版本中，AGENTS.md 从 292 行精简到 229 行（22% 的减幅），Agent 提示词去重 20%。因为多个 Agent 的提示词共享同一套 AGENTS.md 全局规则，在各自提示词中重复声明只会浪费 Token。后续迭代在精简与补强之间演进，当前为 311 行。
...
| 技能 | 无预置技能 | 23 个可复用技能，按需加载 |
| 全局规则 | 无 | 311 行 AGENTS.md，包含反模式、证据纪律、拒绝契约 |
| 命令别名 | 无 | 18 条快捷命令 |
| 上下文管理 | 原生 compaction | 内置 compaction 显式窗口 + 每轮 prune + 路径引用策略 |
| 模型分工 | 无策略 | Pro/Flash 双模型 + 思考档位严格分工，Flash 优先 |
...
| Skill 数量 | 较少 | 23 个，覆盖更全面 |
| Agent 数量 | 相近 | 12 个，增加 solo、vision、ui-builder、consultant 等 |
| 上下文管理 | 基础 | 内置 compaction 显式窗口 + 路径引用 + 去重 |
...
你使用 DeepSeek API 按量付费，希望每一分钱都花在刀刃上。本方案的 Token 效率策略（Flash 优先路由、技能按需加载、内置 compaction 显式窗口、路径引用）能让本来 50K Token 的一次对话控制在 20K 以内——不是夸张，是一系列设计决策的叠加效果。
...
本方案目前锁定 DeepSeek Provider（`provider.deepseek` 双模型矩阵）。虽然改 Provider 配置不复杂，但 Agent 提示词中对模型行为的一些假设（如对 Token 效率的强调程度）是在 DeepSeek 上验证过的。换模型不是不行，但效果需要重新调优。
...
3. **模型感知路由**：根据任务类型决定用 Pro 还是 Flash——探索类走 Flash（explore/librarian），推理类走 Pro（oracle/reviewer），重型实现走 Pro（deep-worker），轻量编辑走 Flash（light-orchestrator）。
...
Orchestrator 自身运行在 `deepseek/deepseek-flash` 上——因为路由决策虽需要一定推理，但绝大多数是模式匹配与分类，Flash 足够胜任且成本更低；遇到复杂任务时通过后备链升级到 Pro 子 Agent。

### 第三层：执行层

执行层分为两个子层，按模型能力划分：

**Pro Agent 群（推理与决策）**——运行在 `deepseek/deepseek-v4-pro`（`solo` 例外：无显式 `model` 字段，跟随会话所选模型，默认即 pro）：
- `solo`：主 Agent，单模型内联执行（零委派）
- `deep-worker`：多文件重型实现，复杂调试
- `oracle`：只读，深度分析代码逻辑、追溯问题根因
- `reviewer`：只读，多维度代码审查

**Flash Agent 群（查询与轻量执行）**——运行在 `deepseek/deepseek-flash`：
...
**Vision Agent**——同样运行在 `deepseek/deepseek-flash`（原生多模态）：
- `vision`：读取图片、截图、图表等多模态输入；可执行单处轻量视觉修正，深度改动升级 deep-worker

**关键约束**：
- 只读 Agent（oracle、reviewer、explore、librarian）**绝不修改文件**
- 执行 Agent（deep-worker、light-orchestrator）**禁止自行研究和委托**
- Agent 嵌套深度限制为 2 层（`subagent_depth: 2`，恰好覆盖最深链路 orchestrator → light-orchestrator → oracle；桌面 v2 端只认同值的 `experimental.subagent_depth`，两处并存以兼容），防止无限递归

### 第四层：能力层

执行层的 Agent 通过加载 Skill 获取专业能力。23 个 Skill 按功能域组织在 `skills/` 目录下，通过 OpenCode 原生的 `skill` 工具按需加载。

插件只有一个：

- **superpowers（obra/superpowers，固定 #v6.4.1）**：提供过程型技能（brainstorming、systematic-debugging、TDD、writing-plans 等），确保 Agent 在面临特定类型任务时"先想清楚再动手"。v6.4.1 起子会话（task subagent）不再注入 bootstrap，固定版本（pin）以保证字节稳定前缀。上下文压缩 100% 交给 OpenCode 内置 compaction（`limit.input` 显式工作窗口 + 每轮 `prune`），原 DCP 第三方压缩插件已移除。
...
| 文件 | 作用 | 关键内容 |
|------|------|---------|
| `opencode.jsonc` | 核心配置 | 模型绑定、Agent 定义与权限、subagent_depth: 2、内置 compaction 参数、权限基线 |
...
│  ├──────────────────────────────────────────────────────┤   │
│  │  1 Plugin  (增效但不喧宾夺主)                         │   │
│  │  superpowers#v6.4.1 (无 DCP，压缩=内置 compaction)   │   │
│  └──────────────────────┬───────────────────────────────┘   │
├─────────────────────────┼───────────────────────────────────┤
│  配置层                  ▼                                    │
│  ┌──────────────────────────────────────────────────────┐   │
│  │  opencode.jsonc (核心配置 · 模型 · Agent · 权限)      │   │
│  │  compaction     (内置压缩 · 显式工作窗口)             │   │
│  │  AGENTS.md      (全局规则 · 311行 · 所有Agent共享)    │   │
│  └──────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
```
...
**纵向维度**：按任务难度分配模型。简单的搜索和查询用 Flash，复杂的推理和决策用 Pro，图像理解交给原生多模态的 Flash（vision Agent）。这不是粗暴的二分法，而是通过 Orchestrator 的意图门控 + 后备链实现的动态路由——如果一个任务在路由时被判断为"简单"但执行中发现需要更深的推理，后备链会将它自动升级到 Pro。
...
- **按需加载**：Skill 在触发时才注入，不在每次对话中都携带 23 个 Skill 的全部内容——节省 80% 的 Skill Token。
- **显式压缩窗口**：内置 compaction 在 `limit.input − reserved`（flash 115K / pro 148K tokens）触发，避免上下文无限膨胀——单次请求可携带的上下文上限从隐式 ~968K 降到显式的约 1/8。
...
- **superpowers**：不引入新的 Agent，只提供过程纪律技能——"遇到 Bug→先系统诊断→再修复"。它增强的是 Agent 的**过程纪律**，而不是替代 Agent 的能力。上下文压缩不再外包给插件：原 DCP 插件已移除，压缩 100% 由内置 compaction 承担。

它的约束：不使用额外的模型、不修改 OpenCode 的核心行为、不引入新的依赖。它是"增效器"而非"替代品"。
...
2. **迭代记录**：README 中的"迭代里程碑"表格记录了从 v1 到 v38+ 的每一次关键变更，形成了可追溯的演进历史。这不是文档的装饰，而是后续决策的依据——"我们为什么反复精简 AGENTS.md？因为分析发现大量内容在 Agent 系统提示中已经重复声明了。"
...
│   │   ├── solo.md                       ← 会话模型（默认pro）主入口：单模型内联执行（零委派）
...
│   │   └── vision.md                     ← flash（原生多模态）：多模态读取
│   │
│   ├── skills/                            ← 23 个可复用技能 (SKILL.md)
...
│   │                                       模型绑定（Pro + Flash，Flash 原生多模态）、
│   │                                       Agent 定义与权限（subagent_depth: 2）、
│   │                                       Provider 模型矩阵、内置 compaction、权限基线
│   │
│   └── AGENTS.md                         ← 全局规则（311 行），所有 Agent 共享
│                                           核心原则、语言策略、反模式、质量基准、
...
| Skill 技能 | 23 个 | `skills/` 目录下每个子目录包含一个 `SKILL.md` |
| 核心配置 | 2 个 | `opencode.jsonc`（压缩参数已并入其 compaction 块）+ `AGENTS.md` |
...
- **`scripts/`** 存放辅助脚本：`sync-config.ps1` 将 `opencode/` 同步到 `~/.config/opencode`，`estimate-cost.js` 估算不同模型的调用成本。
...
它是一个为 DeepSeek V4 双模型（Pro + 原生多模态 Flash）精心调优的 OpenCode 配置仓库。它有 12 个各司其职的 Agent、23 个按需加载的 Skill、18 条快捷命令、311 行全局规则，以及贯穿始终的 Token 效率优先哲学。
...
[← 返回目录](https://znlgis.github.io/ai/my-opencode-deepseek-config/01-项目概览与核心定位/) | [下一章：安装部署与环境配置 →](https://znlgis.github.io/ai/my-opencode-deepseek-config/02-安装部署与环境配置/)
