...
> 技能是 OpenCode 框架中可复用的能力模块。Agent 是角色（"谁来做"），技能是工具箱（"怎么做"）。本章完整覆盖本配置中全部 23 个自定义技能和 1 个插件（superpowers；上下文压缩由 OpenCode 内置 compaction 独立成章），逐一详解每个技能的作用、触发条件、核心工作流和关键设计点。
...
- `{skill,skills}/**/SKILL.md` — 配置目录下的本地技能（本项目中的 23 个自定义技能）
- `.agents/skills/**/SKILL.md` — 项目级或全局级技能
- `superpowers` 插件路径 — 插件提供的 15 个过程型技能
...
| **分析与领域类** | `codebase-design`、`domain-modeling`、`grilling`（含原 `grill-with-docs`/`wait-what` 分支） | 架构词汇、领域术语、需求澄清 |
| **办公与多模态类** | `office-docs`、`vision-prep` | Word/Excel 读写、大图/PDF 预处理 |
| **协作与流程类** | `to-tickets`、`triage` | 拆解工单、Issue 分流 |

> **版本说明**：本配置的技能体系经历了持续精简与扩充。v21 移除 `deepwork`/`conventional-commits`/`diagnose`，v23 移除 `gh-skill`，v38 将技能从 24 个精简到 20 个（移除 `/search`、`/consult` 相关技能），随后并入 `shared-language`（并入 `domain-modeling`）、`writing-great-skills`（并入 `writing-for-agents`）等并扩充 mattpocock/办公文档系列，一度达到 25 个；2026-09 又把 `wait-what`、`grill-with-docs` 合并回 `grilling`（一个行为不占两个名册位），当前为 23 个。技能数量本身不是目标——每个技能都必须通过"无操作裁剪"测试；并且名册的 name+description 是**每轮常驻** token 成本，「用不上或与现有技能重复」的应当合并删除，而非留着备用。
...
- **两级模型（本仓库接线）**：`/review` 默认由 flash（light-orchestrator）跑 Tier 1 初检、干净结果直接出完整报告；命中升级触发条件才委派 pro 的 `reviewer` 确认未验证线索。大 diff（>~500 有效行）拆 Standards/Spec 两轴并行审查后合并为一份报告
...
- **权限配置**：read 默认允许 + 拒绝危险；bash 是**默认 ask** 基线：`permission.read` 拒绝 `.env*`（除 `.env.example`）、`permission.bash` 兜底 `"*": "ask"` 放**最前**，后面依次列 allow 与 ask 规则（匹配为 last-match-wins，后面的具体规则覆盖兜底）

**关键设计点**：

- 必须验证 `$schema`（`https://opencode.ai/config.json`）——不要猜测键名
- 双模型约束是绝对的：只允许 `deepseek/deepseek-v4-pro` 与原生多模态的 `deepseek/deepseek-flash`
...
**作用**：当需求模糊、在规划或实现前需要澄清时使用——一次只问一个问题，优先多选，直到意图清晰。该技能还内置两条分支：**消息完全没听懂 → 一句话重述确认**（原 `wait-what`）、**领域术语模糊 → 协同 `domain-modeling` 锐化**（原 `grill-with-docs`）。
...
- 当需求模糊且领域语言也模糊时，走 grilling 内置的 `domain-modeling` 协同分支（原 `grill-with-docs` 已并入本技能）

### 6.2.20 grill-with-docs —— 组合澄清（已并入 grilling）

> **状态**：该技能已从 `skills/` 目录**移除**（2026-09），组合逻辑并入 `grilling` 的「领域术语模糊 → 协同 `domain-modeling` 锐化」分支。移除理由：它只是 `grilling` + `domain-modeling` 的组合说明，一个行为不该占两个名册位——名册条目是每轮常驻的 token 成本。

其原工作流仍然有效，现由 grilling 直接承载：

1. 用 grilling 的方法一次问一个问题澄清需求
2. 同时用 domain-modeling 的方法记录和锐化领域术语
3. 两者交替进行，直到需求和术语都清晰

### 6.2.21 wait-what —— 意图确认（已并入 grilling）

> **状态**：该技能已从 `skills/` 目录**移除**（2026-09），行为并入 `grilling` 的「消息完全没听懂 → 一句话重述确认」分支。移除理由：与 grilling 的「先确认再动手」规则重复。
...
OpenCode 支持通过 `opencode.jsonc` 中的 `plugin` 字段引入外部插件，扩展框架能力。本配置只使用**一个**插件，且**固定了版本**（pin）以保证字节稳定前缀、避免自动更新导致的前缀漂移：
...
**来源**：`obra/superpowers`（https://github.com/obra/superpowers），固定 tag `#v6.4.1`

**定位**：提供过程型技能（v6.4.1 名册含 15 个，比 v6.3.0 多 1 个、名册 +484 字节），覆盖从需求探索到代码交付的完整软件开发流程。这些技能侧重于**方法论和流程规范**——如何思考、如何规划、如何协作——与本地 23 个自定义技能的"具体工具操作"定位互补。**常用的 14 项如下**：
...
**注入机制（v6.4.1 的关键变化）**：`using-superpowers` 的 bootstrap 注入**主会话**的首条用户消息，核心指令是：在做出任何响应（包括澄清问题）之前，必须先检查并调用适用技能。**v6.4.1 起子会话（task subagent）不再注入 bootstrap**（源码以 `firstUser.info.sessionID + parentID` 判定）——每委派一个子智能体省 3,110 字节 ≈ 780 tokens。

**无模型映射能力**：插件（`.opencode/plugins/superpowers.js`）只做 skills 路径注册 + bootstrap 注入，**不提供任何模型配置项**——superpowers 工作流的模型分档只能在 Agent 层完成。本配置的接线映射写在 `AGENTS.md` 的 Plugins 小节，且只接**没有本地等价物**的 4 项：`writing-plans` → `planner`；`test-driven-development`、`verification-before-completion` → `deep-worker`；`verification-before-completion` → `light-orchestrator`。`systematic-debugging`、`brainstorming`、`requesting-`/`receiving-code-review` 等在本地都有等价物（`diagnosing-bugs`、`spec-workflow`/`grilling`、`code-review`），一律**不接线**以免两套流程打架。

**与本地技能的关系**：superpowers 的技能侧重于"如何管理流程"，本地 23 个技能侧重于"如何执行具体操作"。例如，`systematic-debugging` 定义调试的思考框架（先提出假设还是先构建反馈环），而本地技能提供具体工具操作层面的执行规范。两者互补而非重复——但在路由 Agent 上优先用本地蒸馏版。

### 6.3.2 DCP 插件

> **状态：已移除（2026-09）**。`@tarquinen/opencode-dcp` 连同其配置文件 `opencode/dcp.jsonc`、插件缓存、`compress` 权限均已从仓库清理。它的两块价值——**绝对值阈值提前压缩**与**工具调用去重**——现在由内置 compaction 覆盖：`provider.deepseek.models.*.limit.input` 显式声明工作窗口（flash 131,072 / pro 163,840），触发点 = `limit.input − compaction.reserved`（flash 115,072 / pro 147,840 tokens），每轮 `prune: true` 清理旧工具输出。**内置 compaction 是唯一的压缩层**；少一个插件就少一条前缀漂移与启动开销路径。单次请求可携带的上下文上限因此从隐式的 ~968K 降到显式的 115K/148K（约 1/8）。

以下为**历史设计记录**（该插件已不在当前配置中，保留供理解「主动压缩优于被动溢出」的动机）：

**来源**：`@tarquinen/opencode-dcp`（OpenCode Dynamic Context Pruning），曾固定版本 `@3.1.15`

**定位**：自主上下文裁剪和去重插件。

**配置位置**：曾为 `~/.config/opencode/dcp.jsonc`（已删除）
...
**设计哲学**：主动压缩优于被动溢出——这一动机如今由内置 compaction 的**显式工作窗口**实现：与其继承 968K 的隐式默认触发点，不如用 `limit.input` 把压缩点定死在 115K/148K，配合每轮 `prune`，在窗口满了之前持续瘦身。
...
技能没有任何"归属"Agent——它们是**共享的**。但每个 Agent 的 `permission.skill` 白名单（默认 deny + 按职责放行）裁剪出各自的名册——**被 deny 的 skill 不会出现在该 Agent 的 skill 名册里**，所以这份白名单同时就是常驻上下文预算：

| Agent | 最常搭配的技能 | 典型场景 |
|-------|---------------|----------|
| `orchestrator` | `codemap`、`grilling` | 结构概览、需求澄清（白名单仅此两项） |
| `planner` | `spec-workflow`、`codebase-design`、`writing-plans` | 需求分析、规划分解 |
| `deep-worker` | `remove-deadcode`、`spec-workflow`、`git-release`、`git-master`、`to-tickets`、`triage`、`resolving-merge-conflicts`、`diagnosing-bugs`、`codebase-design`、`domain-modeling`、`opencode-config`、`writing-for-agents`、`test-driven-development`、`verification-before-completion` | 重度实现与自我验证 |
| `reviewer` | `code-review`、`security-review`、`gh-cli` | 代码审查、安全审计 |
| `oracle` | `reflect`、`simplify`、`diagnosing-bugs` | 持续改进、简化分析、根因定位 |
| `light-orchestrator` | `handoff`、`simplify`、`spec-workflow`、`code-review`、`gh-cli`、`verification-before-completion` | 轻量任务、flash 初检、简化执行 |
| `explore` | `codemap` | 仓库探索 |
| `librarian` | `verify-with-docs` | 外部文档检索 |
| `consultant` | `domain-modeling` | 决策分析、方案评估 |
| `ui-builder` | `codebase-design` | 前端与 UI |
...
**需求澄清流程**：
```
grilling（内置复述确认分支）→ grilling 术语锐化分支 → domain-modeling
```
...
```jsonc
// 示例：/review 命令（flash 初检 + 按需升级两级模型）
"command": {
  "review": {
    "description": "Code review — flash first pass, pro escalation on complex findings",
    "agent": "light-orchestrator",
    "template": "Load the code-review skill and run the tier-1 (flash) pass. ... If any escalation trigger fires (Full path, high-stakes override, trust boundary, or a critical/major finding you cannot confirm from the diff alone), delegate to the `reviewer` subagent (pro) and pass your findings as unverified leads. If a PR ref or URL is present ... post findings as a pending GitHub review ... (event=COMMENT, never auto-approve). Do not modify code."
  }
}
```

当用户输入 `/review` 时：
1. OpenCode 路由到 `light-orchestrator`（flash）Agent
2. 它按 template 加载 `code-review` 技能跑 Tier 1 初检（PR 场景还加载 `gh-cli` 回帖）
3. 命中升级触发条件（Full 路径 / 高风险 / 信任边界 / 需跨文件确认的 critical-major）时才委派 `reviewer`（pro），并传递未验证线索；`/deep-review` 则跳过初检直接强制 pro 全量审查
...
1. **插件技能**（superpowers 的 15 个技能）——最先注册
...
- **23 个自定义技能** 提供从代码审查到发布管理、从安全审计到领域建模、从办公文档到多模态预处理的全面工具覆盖，每项技能都是经过实战打磨的精确指令集
- **15 个 superpowers 技能（v6.4.1）** 提供软件开发的方法论基础设施——强制设计先行、根因分析先行、测试先行、证据先行；但只接线无本地等价物的 4 项，其余走本地蒸馏版
- **1 个插件**（superpowers）扩展流程纪律——原 DCP 压缩插件已移除，上下文窗口由内置 compaction 显式管理

这套技能体系的设计哲学是 **"不加载就不花钱"**：39 个可发现技能（23 自定义 + 15 superpowers + 1 内置）中，一个典型会话通常只加载 2-5 个，其余的在文件系统中静默等待——这正是 OpenCode 相比传统将所有能力塞进 system prompt 的方案的核心优势。同时要记住**名册即预算**：每个技能 name+description 的常驻成本对未设白名单的 Agent 全量生效（本仓库名册约 10.6KB ≈ 2.6K tokens），所以各 Agent 的 `permission.skill` 白名单裁剪是每轮都生效的节省。
