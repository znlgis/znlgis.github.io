...
> 技能是 OpenCode 框架中可复用的能力模块。Agent 是角色（"谁来做"），技能是工具箱（"怎么做"）。本章完整覆盖本配置中全部 16 个自定义技能和 2 个插件生态，逐一详解每个技能的作用、触发条件、核心工作流和关键设计点。
...
这种设计与传统 AI 编码助手中"把所有能力塞进 system prompt"的做法截然不同：技能是**上下文惰性加载**的，一个典型的编码会话可能只加载 2-3 个技能，而不是让所有 30 个技能的全部指令始终占据上下文窗口。
...
- `{skill,skills}/**/SKILL.md` — 配置目录下的本地技能（本项目中的 16 个自定义技能）
...
本配置中的 16 个自定义技能按其核心用途分为五大类：

| 分类 | 技能 | 定位 |
|------|------|------|
| **开发流程类** | `code-review`、`spec-workflow`、`verification-planning` | 规范编码→审查→交付的完整链路 |
| **代码质量类** | `security-review`、`remove-deadcode`、`simplify` | 安全审计、清理和简化 |
| **工具操作类** | `gh-cli`、`git-master`、`git-release` | GitHub CLI 和 Git 高级操作 |
| **配置管理类** | `opencode-config`、`reflect`、`writing-great-skills` | 配置编写、持续改进和技能编写规范 |
| **效率工具类** | `codemap`、`handoff`、`verify-with-docs`、`shared-language` | 提效辅助：结构图、交接、文档验证、领域术语 |
...
### 6.2.9 opencode-config —— OpenCode 配置管理
...
### 6.2.13 verification-planning —— 验证规划

**作用**：在实现代码前先回答"这个变更如何被证明正确？"选择最小成本的验证路径，定义单一可观测的"已验证"条件。

**触发条件**：实现任何非平凡变更前（2+ 文件、逻辑变更、新特性）、AGENTS.md 要求验证计划时、在 `/deep` 或 `/spec-apply` 之前。
...
### 6.2.15 shared-language —— 领域术语表

**作用**：构建并维护项目级共享语言文档（`.opencode/CONTEXT.md`），用一句话定义每个项目特有术语，大幅减少跨会话的重复解释 token 消耗。每个术语定义可替代 5-20 句重复解释。

**触发条件**：任务提到 "domain language"、"CONTEXT.md"、"terminology"、"shared vocabulary"，或同一概念在多个会话中被反复解释（>2 句），或在设计讨论后需要固化决策术语。
...
- 删除不再适用的条目（已删除的功能、已重命名的概念）
- 按字母顺序排序

**关键设计点**：

- 共享语言文档是 **token 倍增器**：一个术语定义在后续会话中被引用时，每次节省数十句重复解释的 token
- 不仅是效率工具——它也是领域知识的**持久化编码**，防止团队/Agent 对同一概念产生歧义

### 6.2.16 writing-great-skills —— 技能编写规范

**作用**：指导编写和维护高质量、token 高效技能的规范和检查清单。覆盖无操作裁剪、正向措辞、完成标准三核心原则，同时内置 Conventional Commits 格式参考（替代已移除的 `conventional-commits` 技能）。

**触发条件**：创建新技能、编辑已有技能，或任务提到 "skill"、"write-skill"、"SKILL.md"、"writing-skills"。

**核心规范**：

1. **无操作裁剪（最关键）**：对技能中每句话执行测试——"如果删除这句话，Agent 的行为会改变吗？"如果答案是否定的，立即删除。Agent 已经知道默认行为，每句保留的话必须通过此测试
2. **正向措辞**：只说 Agent **应该**做什么，从不说它**不应该**做什么。否定表述（"不要跳过"、"避免忘记"）反而使禁止行为更易被模型触及
3. **完成标准**：技能中每一步必须有可观测、可检查的完成条件。没有完成标准的步骤会导致 Agent 过早声明"已完成"——完成条件是任何人都能检查的条件："文件存在于路径 X"、"grep 返回 0 匹配"、"输出包含字符串 Y"
4. **信息层级**：步骤内内容（Agent 执行该步必须知道的）→ 技能内参考（Agent 可能需要知道的背景）→ 外部参考（Agent 按需获取的文档链接）。尽可能推到外部参考，步骤内内容保持最少

**裁剪检查清单**：最终确认前扫描——无操作语句 → 重复指令（出现两次则保留一处）→ 冗余描述（20 字可缩减到 5 字）→ 否定措辞 → 模糊步骤（无完成标准则添加）

**内置 Conventional Commits 参考**：类型 `feat/fix/docs/chore/refactor/test/style/perf/ci`，格式 `type(scope): imperative-mood subject`，主题祈使语气（"add" 而非 "added"）、小写、无尾随句号

**关键设计点**：

- 写技能就是应用于过程文档的 TDD：让子代理在没有技能时失败（RED）→ 写技能 → 证明已规范（GREEN）→ 闭合漏洞（REFACTOR）
- 该技能融合了 Conventional Commits 标准作为内嵌参考，因此在技能目录中移除了独立的 `conventional-commits` 技能——遵循"减少表面积"原则
...
**定位**：提供 14 个过程型技能，覆盖从需求探索到代码交付的完整软件开发流程。这些技能侧重于**方法论和流程规范**——如何思考、如何规划、如何协作——与本地 16 个自定义技能的"具体工具操作"定位互补。
...
**与本地技能的关系**：superpowers 的技能侧重于"如何管理流程"，本地 16 个技能侧重于"如何执行具体操作"。例如，`systematic-debugging` 定义调试的思考框架（先提出假设还是先构建反馈环），而本地技能提供具体工具操作层面的执行规范。两者互补而非重复。
...
| `deep-worker` | `verification-planning`、`code-review`（loop 模式） | 重度实现、自我审查 |
| `reviewer` | `code-review`、`security-review` | 代码审查、安全审计 |
| `oracle` | `simplify`、`systematic-debugging` | 深度分析、简化 |
| `light-orchestrator` | `codemap`、`shared-language` | 轻量任务、快速操作 |
...
**特性开发全流程**：
```
brainstorming → spec-workflow (propose) → writing-plans → verification-planning
→ verification-before-completion → code-review → git-release
```

**Bug 修复流程**：
```
systematic-debugging → test-driven-development → verification-planning 
...
当用户输入 `/review` 并触发审查修复循环时：
1. OpenCode 路由到 `deep-worker` Agent
2. `deep-worker` 根据 template 中的指令识别出需要加载 `code-review` 技能
3. 通过 `skill` 工具加载技能内容，开始执行循环

类似的链路贯穿整个命令体系：`/spec-propose` → `spec-workflow`、`/spec-apply` → `spec-workflow`、`/release` → `git-release` + `gh-cli`。
...
- **16 个自定义技能** 提供从代码审查到发布管理、从安全审计到领域建模的全面工具覆盖，每项技能都是经过实战打磨的精确指令集
- **14 个 superpowers 技能** 提供软件开发的方法论基础设施——强制设计先行、根因分析先行、测试先行、证据先行
- **2 个插件**（superpowers + DCP）扩展框架边界——superpowers 注入流程规范，DCP 自主管理上下文窗口

这套技能体系的设计哲学是 **"不加载就不花钱"**：30 个技能中，一个典型会话通常只加载 2-5 个，其余的在文件系统中静默等待——这正是 OpenCode 相比传统将所有能力塞进 system prompt 的方案的核心优势。
