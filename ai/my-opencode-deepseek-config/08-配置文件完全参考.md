...
> 如需进一步了解双模型分工和 routing 策略，请参考 [第三章：模型配置详解](https://znlgis.github.io/ai/my-opencode-deepseek-config/08-配置文件完全参考/03-模型配置详解/)。
...
> AGENTS.md 的完整解读见 [第九章：全局规则 AGENTS.md 深度解读](https://znlgis.github.io/ai/my-opencode-deepseek-config/08-配置文件完全参考/09-全局规则AGENTS.md深度解读/)。
...
- **作用**：开启 Git 快照功能。每次 Agent 修改文件后，OpenCode 自动创建一个 Git 提交快照。这带来的好处：
  ...
  > **前提**：项目必须在 Git 仓库中（`git init` 初始化后），快照功能才能生效。这也是为什么 [第二章](https://znlgis.github.io/ai/my-opencode-deepseek-config/08-配置文件完全参考/02-安装部署与环境配置/) 强调 Git 是前置依赖。
...
命令别名的完整结构和详解见 [第七章：命令别名完整指南](https://znlgis.github.io/ai/my-opencode-deepseek-config/08-配置文件完全参考/07-命令别名完整指南/)。此处仅展示结构示例：
...
> **下一章**：[第九章：全局规则 AGENTS.md 深度解读](https://znlgis.github.io/ai/my-opencode-deepseek-config/08-配置文件完全参考/09-全局规则AGENTS.md深度解读/) —— 逐段解析管理所有 Agent 行为规范的"宪法文件"AGENTS.md，包括核心原则、代码风格要求、Token 效率策略、反模式禁令等。
