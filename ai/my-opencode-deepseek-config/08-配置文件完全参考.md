...
DCP 负责**主动管理**——在上下文还没满之前就进行分析和优化。OpenCode 原生压缩负责**被动兜底**——当 DCP 未能完全阻止溢出时，原生压缩作为最后的保障。详见 [8.2.8 节](#828-dcp-与-opencode-原生压缩的协同)。
...
### 8.2.8 DCP 与 OpenCode 原生压缩的协同 {#828-dcp-与-opencode-原生压缩的协同}
