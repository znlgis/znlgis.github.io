---
layout: default
title: my-opencode-deepseek-config 教程目录
---

# my-opencode-deepseek-config 教程目录

> 本教程基于 [znlgis/my-opencode-deepseek-config](https://github.com/znlgis/my-opencode-deepseek-config) 仓库 (v38+) 的配置文件与设计文档整理，面向中文读者系统讲解这套 **OpenCode × DeepSeek 最优配置方案**——一个纯配置驱动的多 Agent 协作框架。教程覆盖项目定位、安装部署、模型配置、多 Agent 体系、Orchestrator 路由机制、技能与插件生态（23个技能+1个插件）、18条命令别名、配置参考、全局规则解读、实战工作流、设计决策与迭代历程（v1-v38+），以及最佳实践与定制指南。OpenCode 迭代很快，实际命令与字段请以当前版本的 `opencode --help` 与官方文档为准。
...
如果你的核心诉求是「理解这套配置的设计思路并高效定制自己的方案」，建议沿主线阅读：

**第一章（概览）→ 第三章（模型配置）→ 第四章（多Agent体系）→ 第五章（路由机制）→ 第八章（配置参考）→ 第十二章（定制指南）**

其中第五章是理解整个框架调度逻辑的核心，第十二章给出可直接套用的定制方法与常见场景模板。

---

## 📖 推荐学习路线

- **新手路线**：按章节顺序 1 → 12 通读，每章独立成篇，从概览到实战循序渐进。
- **进阶路线**：如果你已经熟悉 OpenCode 基础，可直接跳至第四章（多Agent体系）和第五章（路由机制），再回溯第三章（模型配置）和第八章（配置参考）查漏补缺。
