...
2. 自动路由到 `planner` Agent（Flash 模型 + `reasoningEffort: low`，规划类任务属于"中档"思考层）
...
2. 路由到 `light-orchestrator`（Flash）加载 `code-review` 技能跑 Tier 1 初检
3. ≤8 个逻辑文件且 ≤300 有效行、无高风险触发 → 初检直接出完整报告；命中升级触发条件（Full 路径 / 信任边界 / 需跨文件确认的 critical-major）→ 委派 `reviewer`（Pro），初检发现作为未验证线索传递
4. 根据 diff 大小自适应审查深度
5. 产出多维度审查报告
...
（Orchestrator 路由到 light-orchestrator，flash Tier 1 初检）
light-orchestrator：正在分析 diff（7 个文件，+312 行，-4 行）。触及参数校验/注入面，命中升级触发条件 → 委派 reviewer（Pro）确认。
...
（/review flash 初检 + 按需升级审查，发现 2 个问题）
...
代码审查是本配置方案的核心能力之一。`/review` 采用 **flash 初检 → 按需升级 pro** 的两级模型（2026-09 起），合并了原 `/review-pr` 的 PR 回帖能力，根据参数自动切换两种模式；另有 `/deep-review` 跳过初检、强制 `reviewer` 全量审查。

### 10.4.1 /review（审查 PR 并回帖）

**适用场景**：GitHub 上已经有一个 PR，需要全面审查并自动将审查意见发到 PR 评论区。

**路由过程**：

1. Orchestrator 路由到 `light-orchestrator`（Flash）做 Tier 1 初检；命中升级触发条件时委派 `reviewer`（Pro，只读）确认（大 PR 也可直接用 `/deep-review` 强制 pro 全量）
2. 初检/升级层加载 `code-review` 技能；若参数含 PR 引用/URL，额外加载 `gh-cli` 技能
3. 通过 `gh` 拉取 PR diff
4. 多维度审查
5. 通过 `gh` 将审查报告以 `event=COMMENT` 的 pending review 形式回帖到 GitHub PR 页面（绝不自动 approve）

**模拟对话**：

```
用户： /review https://github.com/my-team/my-repo/pull/152

（light-orchestrator 加载 code-review + gh-cli 初检，按需升级 reviewer 确认）
reviewer：
...
🟢 一致性（通过）
  ...
  正在将审查报告以 pending review 回帖到 GitHub PR #152...
  ✅ 审查报告已发布到 PR 评论区。
...
```
/review https://github.com/xxx/pull/N   ──→ PR 模式（自动回帖到 GitHub）
/review                                  ──→ 本地 diff 模式（仅审查，不自动修复）
```

**范围优先门控**：无论哪种模式，若 diff 过大（>500 有效行）或过于琐碎，审查层会先报告一个范围化审查计划并停下，而不是盲目深审——这是 scope-first 门控，避免在超大 diff 上浪费 token；Full 路径本身也是升级触发条件之一。

```
/review                                  ──→ 默认：flash 初检，命中触发条件才升级 pro
/deep-review                             ──→ 强制 reviewer（pro）Full 路径全量审查
```
...
> **版本说明**：v38 移除了 `/search` 与 `/consult` 两个命令别名（它们与自然语言路由高度重叠，属于冗余）。底层 `librarian`（外部检索）与 `consultant`（方案评估）Agent 仍然存在，通过 Orchestrator 自然语言路由即可触达。`/plan` 命令保留。

### 10.5.1 工作流全景图

```
自然语言描述调研需求 → Orchestrator 路由 librarian（外部信息收集）
  ↓
自然语言追问方案 → Orchestrator 路由 consultant（多方案对比评估，给出推荐）
  ↓
...
#### 阶段一：外部调研（自然语言 → librarian）

**路由**：Orchestrator → `librarian` Agent（Flash 模型，快速检索）  
**技能**：`verify-with-docs`（核对官方文档）

```
用户： 帮我调研 Next.js 14 的 Server Actions 怎么处理文件上传

（Orchestrator 识别为"调研/查资料"意图 → 路由 librarian）
librarian：
  正在搜索 Next.js 官方文档...
  正在搜索社区最佳实践...
...
建议下一步：让 consultant 讨论哪种方案适合你的场景。
```

#### 阶段二：方案讨论（自然语言 → consultant）

**路由**：Orchestrator → `consultant` Agent（Flash 模型，方案评估与建议）

```
用户： 我有这些限制：文件不超过10MB，需要显示上传进度，帮我评估下方案

（Orchestrator 识别为"方案评估/给建议"意图 → 路由 consultant）
consultant：
...
**路由**：Orchestrator → `planner` Agent（Flash 模型 + `reasoningEffort: low`）
...
```
/simplify         → light-orchestrator 两阶段：先派 oracle 只读分析出方案，
                    再由 light-orchestrator 亲自应用编辑（行为保持）
  ↓
/rmslop           → 清理重构中留下的死代码
  ↓
/commit           → 规范提交
...
> **版本说明**：v38 起 `/simplify` 路由到 `light-orchestrator`（而非 oracle），采用两阶段流程 —— 派生的 oracle 只做只读分析、绝不编辑；light-orchestrator 只应用编辑、绝不分析。职责严格分离。

### 10.7.2 各阶段详解

#### 阶段一：分析简化（/simplify）

**路由**：Orchestrator → `light-orchestrator` Agent + 加载 `simplify` 技能，内部派生 `oracle` 子 Agent 做只读分析
...
（light-orchestrator 派生 oracle 子 Agent 做只读分析）
oracle：
...
（oracle 只读分析完成，将精确的 oldString→newString 方案交回 light-orchestrator）
light-orchestrator：
  已收到 oracle 的简化方案。现在亲自应用这些编辑（oracle 从不改代码）。
```

#### 阶段二：应用编辑

```
（light-orchestrator 按 oracle 方案逐项应用编辑，每步验证行为保持）
...
2. 技能利用率低（利用率为 4/23）
   23 个技能中只有 4 个被实际使用过。常规开发中 code-review、
   spec-workflow、codemap 高频使用；其余 19 个从未触发。
...
├─ 「帮我审查代码」
│   ├─ 审查已有的 PR
│   │   └─ /review https://github.com/xxx/pull/N（PR 模式，自动回帖）
│   │
│   └─ 审查当前改动（本地未提交或刚提交）
│       └─ /review（本地 diff 模式）
│
├─ 「我不确定怎么实现」
│   └─ 自然语言调研（librarian）→ 自然语言方案讨论（consultant）→ /plan → /deep
│
├─ 「代码太复杂，想简化」
│   └─ /simplify → /rmslop → /commit
│
├─ 「想优化我的开发配置」
│   └─ /reflect → 手动更新配置
│
...
- **不确定**：自然语言调研（librarian）→ 自然语言方案讨论（consultant）→ /plan
- **改配置**：/reflect
- **提代码**：/commit
...
| 代码审查 | `/review`（PR 或本地 diff） | reviewer 审查 | 审查报告 |
| 方案调研 | 自然语言 | librarian → consultant → plan → deep | 可执行的实施计划 |
| 日常轻量 | 自然语言 | light-orchestrator / explore / librarian | 即时响应 |
| 代码简化 | `/simplify` | light-orchestrator（派 oracle 分析）→ rmslop → commit | 简化后的干净代码 |
