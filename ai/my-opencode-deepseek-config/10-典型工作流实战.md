...
代码审查是本配置方案的核心能力之一。`/review` 命令在 v38 起合并了原先独立的 `/review-pr`，根据参数自动切换两种模式。
...
1. Orchestrator 路由到 `reviewer` Agent（Pro 模型，只读）
2. reviewer 加载 `code-review` 技能；若参数含 PR 引用/URL，额外加载 `gh-cli` 技能
3. 通过 `gh` 拉取 PR diff
4. 多维度审查
...
**范围优先门控**：无论哪种模式，若 diff 过大（>500 有效行）或过于琐碎，reviewer 会先报告一个范围化审查计划并停下，而不是盲目深审 —— 这是 v38 引入的 scope-first 门控，避免在超大 diff 上浪费 Pro token。
...
2. 技能利用率低（利用率为 4/25）
   25 个技能中只有 4 个被实际使用过。常规开发中 code-review、
   spec-workflow、codemap 高频使用；其余 21 个从未触发。
