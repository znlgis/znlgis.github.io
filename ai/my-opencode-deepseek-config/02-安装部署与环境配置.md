...
本配置仓库在 `opencode/` 子目录下包含 10 个 Agent 定义、16 个技能文件、全局规则（AGENTS.md）、插件配置（dcp.jsonc）和主配置文件（opencode.jsonc）。推荐克隆到任意位置后通过环境变量或符号链接指向 `opencode/` 子目录即可使用。
...
│   │   ├── deep-worker.md      ← 重型实现
│   │   ├── oracle.md           ← 深度代码分析（只读）
│   │   ├── reviewer.md         ← 双轴代码审查（只读）
│   │   ├── consultant.md       ← 方案讨论与建议
│   │   ├── ui-builder.md       ← 前端与 UI
│   │   ├── explore.md          ← 代码库搜索（只读）
│   │   ├── librarian.md        ← 外部检索（只读）
│   │   └── light-orchestrator.md ← 简单编辑
│   ├── skills/                 ← 16 个可复用技能
...
| `/ui` | 前端/UI 工作 | `/review` | 双轴代码审查 |
| `/review-pr` | 审查 PR 并回帖 | `/plan` | 制定方案 / 技术设计 |
| `/search` | 外部搜索 / 查文档 | `/oracle` | 深度分析 / 问题溯源 |
| `/consult` | 方案咨询 / 对比建议 | `/commit` | 生成 Conventional Commits 提交信息 |
| `/release` | 准备 Tag 发布 | `/reflect` | 发现摩擦 → 配置优化 |
| `/handoff` | 会话交接文档 | `/rmslop` | 清理死代码和 AI slop |
| `/codemap` | 生成仓库结构图 | `/simplify` | 行为保持的代码简化 |
| `/spec-propose` | 起草变更提案 | `/spec-apply` | 按 tasks.md 逐一实现 |
