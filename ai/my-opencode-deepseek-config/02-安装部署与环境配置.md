...
本配置方案基于 DeepSeek V4 双模型（Pro + Flash，其中 Flash 为原生多模态），需要有效的 API Key。详见 [2.2 节](#22-deepseek-api-key-获取)。
...
- 进入「充值」页面，支持支付宝 / 微信支付
- 最低充值金额为 1 元人民币
- DeepSeek V4 采用按量付费，2026-08-16 起区分高峰 / 低谷时段定价（低谷约为高峰的一半）。以低谷价计（USD/百万 tokens）：Pro 输入 0.66 / 输出 1.98；Flash（原生多模态，原 Flash-Vision-Exp 已并入）输入 0.22 / 输出 0.66。缓存命中读取价远低于输入价（Pro 0.022、Flash 0.007），是成本优化的关键杠杆
...
在模型列表中选择 `deepseek-v4-pro` 作为主模型。系统会自动将 `deepseek-flash` 设为小模型。
...
本配置方案的 `opencode.jsonc` 中通过 `provider.deepseek.models` 预设了 DeepSeek 双模型矩阵，并声明了各自的工作窗口（`limit.input`）、成本与思考开关：
...
> **思考开关的默认值**：Pro 的 thinking 默认开启（无需显式配置 `thinking: { "type": "enabled" }`），Flash（原生多模态）则在 provider 层显式关闭 thinking 并设 `temperature: 0`，这是 DeepSeek 官方（deepseek-harness）推荐的成本优化做法。`options.thinking` 是 provider 透传字段，OpenCode 会原样转发给 DeepSeek API。

> **模型 ID 命名规则**：OpenCode 中模型的完整标识为 `provider_id/model_id`，即 `deepseek/deepseek-v4-pro` 和 `deepseek/deepseek-flash`——原 `deepseek-v4-flash`（文本）与 `deepseek-v4-flash-vision-exp`（视觉）已合并为一个原生多模态模型。`modalities` 声明了 deepseek-flash 支持图片输入，否则 OpenCode 会将其视为纯文本模型而拒绝 `read_image` 调用。

---

## 2.5 克隆配置仓库

本配置仓库在 `opencode/` 子目录下包含 12 个 Agent 定义、23 个技能文件、全局规则（AGENTS.md）和主配置文件（opencode.jsonc——含插件 pin 与内置 compaction 参数，无独立压缩配置文件）。推荐克隆到任意位置后通过环境变量或符号链接指向 `opencode/` 子目录即可使用。
...
│   │   ├── solo.md             ← 单模型内联执行器（跟随会话模型，默认 Pro，主 Agent）
...
│   │   ├── grilling/           ← 需求澄清提问（已合并原 wait-what / grill-with-docs）
...
│   ├── AGENTS.md               ← 全局规则（所有 Agent 共享，311 行）
│   └── opencode.jsonc          ← 主配置文件（18 条命令）
├── scripts/                    ← 辅助脚本
...
输入 `/models`，应显示当前模型为 `deepseek/deepseek-v4-pro`，小模型为 `deepseek/deepseek-flash`。你可以手动切换模型来测试连接是否正常。
...
- `solo` — 单模型内联执行器（跟随会话模型，默认 Pro，主 Agent）
...
输入 `/help` 查看已加载的自定义命令。本配置仓库提供了 18 条命令别名，分为 Agent 路由命令、操作命令、内联命令和规约命令四类：

| 命令 | 用途 | 命令 | 用途 |
|------|------|------|------|
| `/deep` | 重型实现 / 多文件改动 | `/quick` | 轻量任务 / 单文件编辑 |
| `/ui` | 前端/UI 工作 | `/vision` | 多模态识别（图片/截图） |
| `/review` | 代码审查（flash 初检，按需升级 pro） | `/deep-review` | 强制 pro 全量审查 |
...
| `/spec-apply` | 按 tasks.md 逐一实现 | `/plan` | 制定方案 / 技术设计 |
...
# 应看到：agents/  skills/  AGENTS.md  opencode.jsonc  ...
```
...
全部通过后，恭喜 —— 你的 OpenCode × DeepSeek 多 Agent 协作环境已就绪！现在可以进入 [第三章：模型配置详解](https://znlgis.github.io/ai/my-opencode-deepseek-config/03-模型配置详解/)，深入了解双模型分工与路由策略。

---

> **下一章**：[第三章：模型配置详解](https://znlgis.github.io/ai/my-opencode-deepseek-config/03-模型配置详解/) —— 深入讲解 DeepSeek V4 Pro 与原生多模态 Flash 的模型分工原则、Provider 配置细节、thinking mode 启用方式以及 Subagent 的模型分配逻辑。
