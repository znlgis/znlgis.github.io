...
全部通过后，恭喜 —— 你的 OpenCode × DeepSeek 多 Agent 协作环境已就绪！现在可以进入 [第三章：模型配置详解](03-模型配置详解)，深入了解双模型分工与路由策略。

---

> **下一章**：[第三章：模型配置详解](03-模型配置详解) —— 深入讲解 DeepSeek V4 Pro 与 Flash 的模型分工原则、Provider 配置细节、thinking mode 启用方式以及 Subagent 的模型分配逻辑。
