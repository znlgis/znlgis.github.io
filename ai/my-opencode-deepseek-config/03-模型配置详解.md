...
这套方案只使用两个模型（原视觉变体已并入 Flash），通过角色分工与思考档位实现精度、速度与多模态能力的平衡。
...
### 3.1.2 deepseek-flash：快速轻量引擎

`deepseek-flash` 是 DeepSeek 的快速推理模型，强调低延迟和高吞吐。在本方案中，它处理所有「不需要深度推理」的工作：
...
### 3.1.3 原生多模态：vision Agent 的专属入口

`deepseek-flash` 原生支持图像输入——原先的文本版 `deepseek-v4-flash` 与视觉版 `deepseek-v4-flash-vision-exp` 已合并为一个模型，成本不变。在本方案中，视觉能力主要服务于一个角色：

| 角色 | 场景 | 核心需求 |
|------|------|----------|
| **Vision** | 读取图片、截图、图表，多模态识别 | 视觉理解、图像描述 |

Vision 子代理负责「看」并报告所见，可做单处轻量视觉修正；当任务需要基于图像做深度推理或多文件改动时，会升级到 Deep Worker（Pro）。
...
其中 `deepseek` 是 Provider ID，`deepseek-v4-pro` 和 `deepseek-flash` 是模型 ID（后者为合并后的原生多模态模型，通过 `modalities` 声明图像输入）。这种命名规则贯穿整个配置体系——无论是在 `opencode.jsonc` 的顶层 `model` 字段、Agent 定义文件中的 `model` 字段，还是命令路由中，都以同样的格式引用模型。

> **重要**：由于模型 ID 本身包含斜杠 `/`，在 `agent` 配置中建议始终使用 `model` 字段明确指定模型，而非依赖默认继承。这避免了 Provider 枚举时模型 ID 被错误解析为路径。

### 3.1.5 成本对比

两个模型的定价差异显著，按 DeepSeek 官方定价（2026-08-16 生效的峰谷定价，以下为**非高峰**价，单位 USD / 1M tokens；高峰时段翻倍）：

| 模型 | 输入 | 输出 | 缓存读取 | 缓存写入 |
|------|------|------|---------|---------|
| deepseek-v4-pro | 0.66 | 1.98 | 0.022 | 0.66 |
| deepseek-flash（原生多模态） | 0.22 | 0.66 | 0.007 | 0.22 |
...
- **多模态不额外收费**——原 vision-exp 已并入 Flash，图像输入与文本同价。
...
### 3.2.1 模型隔离设计：双模型矩阵 + 规则约束

早期版本曾用 `enabled_providers` / `disabled_providers` 双重锁来强制只加载 DeepSeek。自 v31 起，这两个字段被移除——OpenCode 演进后不再需要它们来保证隔离。当前的做法是**双管齐下**：

1. **配置层**：`provider.deepseek.models` 只声明两个 DeepSeek 模型（pro / flash），并写入各自的工作窗口（`limit`）、成本与 options。未声明的模型不会出现在 `/models` 列表中。
2. **规则层**：AGENTS.md 的 Constraints 明确写入「**No new models.** Only `deepseek/deepseek-v4-pro` and the natively multimodal `deepseek/deepseek-flash` may be used. Do not introduce others.」——即使有人手动添加了其他 Provider 或模型，这条规则也会约束 Agent 不去使用。
...
简言之：**用同一个模型家族的 Pro + Flash（原生多模态）组合，比「跨 Provider 混搭」更能保证一致性和可预测性。**
...
### 3.3.3 双模型矩阵的 options 设计

两个模型的 `options` 差异体现了「思考分层」的核心设计：

| 模型 | temperature | thinking | 说明 |
|------|-------------|----------|------|
| deepseek-flash（原生多模态） | 0 | `{type:"disabled"}` | 确定性 + 关闭思考，最快最省；`modalities` 声明图像输入 |
...
- **`modalities` 声明了 deepseek-flash 的图像输入能力**。没有它，OpenCode 会把该模型当作纯文本模型，拒绝 `read_image` 调用。

### 3.3.4 思考强度分层：reasoning_effort

除了模型级的 thinking 开关，还有**请求级**的思考强度控制 `reasoning_effort`（`low`/`high`/`max`）。它**不是模型 ID**，而是通过 Agent 前言的 `options`（camelCase `reasoningEffort`，深合并覆盖 `model.options`）逐 Agent 设置。把它放在 Agent 层而非模型层，是为了保持双模型矩阵不被破坏。

AGENTS.md 定义了三个思考层级：

| 层级 | 模型 + 思考 | 适用 Agent |
|------|------------|-----------|
| **trivial** | Flash + 思考关闭（最省） | explore / librarian / consultant / ui-builder / orchestrator |
| **mid** | Flash + 思考开启 + `reasoningEffort: low` | planner / light-orchestrator |
| **deep** | Pro + 默认高思考 | deep-worker / oracle / reviewer |
...
Flash（原生多模态）则显式关闭思考（`thinking: {type:"disabled"}`），这是官方成本节省手段。
...
3. **内部降级链**：`deepseek-flash → deepseek-v4-pro`，不引入第三方模型，由 OpenCode 内部自动处理。
...
跟随会话模型（默认 deepseek-v4-pro）:
  └── solo（主）        — 单模型内联执行（无显式 model 字段）

Flash（deepseek-flash，原生多模态）:
  ├── orchestrator（主） — 主控调度
  ├── planner           — 战略规划
...
- **Flash（原生多模态）**：`temperature: 0`（确定性输出，配合思考关闭）。
...
| 模型 | 上下文窗口（context） | 输出上限（output） | 工作窗口（limit.input） | 压缩触发点（input − reserved） |
|------|-----------|---------|---------|---------|
| deepseek-v4-pro | 1,000,000 | 384,000 | 163,840 | 147,840 |
| deepseek-flash（原生多模态） | 1,000,000 | 384,000 | 131,072 | 115,072 |
...
- `prune: true`：每轮请求自动裁剪旧工具输出（仅 classic 路径 1.18.x 生效，桌面 v2 忽略）
- `tail_turns: 8`：保留最近 8 轮对话的完整历史
- `preserve_recent_tokens: 12000`：在压缩后额外保留最近 ~12K tokens 的对话
- `reserved: 16000`：溢出缓冲，仅在模型声明了 `limit.input` 时参与触发点计算（flash 131,072−16,000=115,072；pro 163,840−16,000=147,840）

> **注意**：`prune` 在早期版本为 `false`，v38 起改为 `true`。内置 compaction 现在是**唯一的压缩层**——原 DCP 第三方插件连同 `dcp.jsonc` 已于 2026-09 移除，其价值（绝对值阈值提前压缩、工具调用去重）由 `limit.input` 显式工作窗口与每轮 `prune` 覆盖。兼容性：桌面 v2 忽略 `prune`/`tail_turns`，`reserved` 与 `preserve_recent_tokens` 在 v2 端分别以 `buffer`、`keep.tokens` 改名继续生效。
...
|-------|-------|------|-------|---------|
| orchestrator（主） | flash | 关闭 | 100 | 完整（task 白名单） |
| solo（主） | 跟随会话（默认 v4-pro） | 随该模型 | 100 | 完整（task 全禁） |
| planner | flash（thinking 开） | low | 60 | 完整 |
| oracle | v4-pro | 默认开 | 40 | 只读 |
| reviewer | v4-pro | 默认开 | 40 | 只读 |
| consultant | flash | 关闭 | 30 | 读写 |
| deep-worker | v4-pro | 默认开 | 100 | 完整 |
| ui-builder | flash | 关闭 | 60 | 完整 |
| explore | flash | 关闭 | 40 | 只读 |
| librarian | flash | 关闭 | 30 | 只读 |
| light-orchestrator | flash（thinking 开） | low | 30 | 完整 |
| vision | flash（原生多模态） | 关闭 | 25 | 读写（轻量视觉修正） |
...
如果把所有 150+ 次调用都交给 Pro，总成本大约会翻 3-4 倍。这就是双模型协作的价值：**高频调用的成本被 Flash 压到了最低，Pro 只在高价值的推理场景下出场，「看」的任务由原生多模态的 Flash（vision Agent）在用户明确提供图像时才出场。**
...
1. **双模型体系**：`deepseek-v4-pro` 负责深度推理（实现、分析、审查），`deepseek-flash`（原生多模态）负责检索、规划、简单执行（搜索、文档查询、配置修改、后台任务）与多模态识别（Vision）。

2. **单一 Provider 设计**：通过「双模型矩阵声明 + AGENTS.md 规则约束」只用 DeepSeek（早期版本用白名单 + 黑名单双重锁，v31 起移除）。避免多 Provider 混搭导致的行为不一致、成本不可控和配置复杂化。

3. **Thinking 模式**：Pro 思考默认开启，Flash（原生多模态）显式关闭。思考强度通过 `reasoningEffort` 逐 Agent 分层（trivial / mid / deep）。
...
6. **多模型协作**：Pro 与 Flash 不是替代关系，而是互补关系。Flash 做探索、检索和简单执行（原生多模态，vision Agent 在需要图像理解时介入），产出精炼结论注入主流程；Pro 做深度推理和复杂实现，利用 Flash 的检索成果快速起步。
