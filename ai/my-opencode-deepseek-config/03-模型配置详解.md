...
```jsonc
"agent": {
    "build": { "model": "deepseek/deepseek-v4-pro", "mode": "subagent" },
...
OpenCode 内部已自动处理 `small_model` → `model` 的回退：当 Flash 模型不可用时，系统内部自动降级到 Pro 模型，无需在配置中显式指定 `fallback` 字段。
...
> **下一章**：[第四章：多 Agent 体系完全解析](https://znlgis.github.io/ai/my-opencode-deepseek-config/04-多Agent体系完全解析/) —— 深入剖析 10 个 Agent 的设计哲学、职责边界和协作关系。
