...
基于 vitest，覆盖 `packages/*/*/tests/**`、`apps/*/tests/**`（含原 `examples/` 迁来的 `apps/cli/tests/profiles/**`）以及 `scripts/**/*.spec.ts`。测试文件跟着被测代码走，不集中堆放。testing.md 明确要求优先覆盖边界情况、错误路径、事件顺序、并发竞态，以及针对契约回归的"永久测试"（permanent tests），典型如 `packages/core/agent-loop/tests/contract-regressions.spec.ts`。
...
`test:snapshot` 是无 key 的快照回放：ACP 通过随附的 `dsh --profile acp` 启动真实 profile，重放一段录制好的会话，比对规范化后的 JSON-RPC 与重新持久化后的日志；headless 后端场景通过 `dsh --profile headless` 加各自的场景 patch 启动语料组合（`snapshots/AGENTS.md` 明确要求所有被测进程都经 `dsh` CLI + 随附 profile 拉起）。可以用 `-t <name>` 过滤单个场景。
...
- `pnpm dsh --profile acp`——从源码跑 ACP 自动化服务器（JSON-RPC stdio，需要 `DEEPSEEK_API_KEY`）
- `pnpm run demo:ptc`——以 PTC 工具模式跑一次 headless 任务（需要 key）
- `pnpm run demo:inspector`——带 inspector 叠加层起 `web` profile
- `pnpm run check:windows-wine`——只在诊断已知 Windows 故障时用（需要 wine），CI 拥有该信号

从源码跑这些 demo 之前要先 `pnpm run build`。`headless` 一次性 agent 需要环境或仓库根 `.env` 里的 `DEEPSEEK_API_KEY`；过去的 `demo:acp`/`demo:cordis` 脚本已随顶层 `examples/` 退役——ACP 面直接 `dsh --profile acp`，自指 Cordis 工具集改由 web profile 的 `cordis` agent preset 承载（见第 7 章 7.7）。
...
| 表面 | 归属 | 机制 |
|---|---|---|
| ACP 自动化 | `snapshots/acp/`（`acp.snapshot.ts`，通过随附的 `dsh --profile acp` 驱动） | 启动真实 ACP profile，回放录制会话，比对 JSON-RPC + 重持久化日志 |
| headless 后端 | `snapshots/session/`（`headless.snapshot.ts`，走随附 headless profile） | 内部 canonical-event JSONL 快照 + 回放 fixture |
...
- TypeScript 客户端：`snapshots/sdk/`（`sdk.snapshot.ts`，spawn 随附的 `dsh --profile sdk` runtime）
...
以 `snapshots/acp/`（`acp.snapshot.ts`）的主场景为例，`test:snapshot:record` 走的是：

1. 通过随附的 `dsh --profile acp` 启动真实 ACP profile，发一段真实转录
...
"真实入口路径"还意味着发布的产物——一个包的 `bin` 在纯 `node` 下跑构建后的 `lib/bin.js`，暴露 tsx 掩盖的故障（settle 竞态、模块解析、被吞的加载失败）。这也适用于非 index 运行时入口（worker-thread 兄弟模块 `lib/worker.cjs`）和跨 bundle 共享的单例模块（`packages/sdk/server/tests/built-scope-carrier.e2e.ts`）。保持构建产物冒烟绿（`apps/cli/tests/built-bin.e2e.ts`，以及各包的 `tests/built-lib.e2e.ts` 一类），并断言"确实缺失的配置以非零退出"。
