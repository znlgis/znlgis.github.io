...
> **写作时点声明**：本文写作于 2026 年 8 月，对应仓库版本 `0.1.6-alpha.2`。安装命令与版本号均以 [docs/development.md](https://github.com/deepseek-ai/deepseek-harness/blob/master/docs/development.md) 与根 `package.json` 的 `master` 原文核对；项目处于 developer preview，命令可能随上游迭代变化。
...
| Python SDK（`deepseek-harness-sdk`） | x64 / arm64 | 14+ arm64 / x64 | x64（持久 shell 为 `pwsh`；无 Windows arm64 wheel） |

两点提醒：

- **`dsh` 不需要 WSL2**，Windows 原生即可跑（这点与 Pi 等终端 agent 不同）。差异在工具面：基础 bundle 在 win32 上禁用 `bash`、启用 `pwsh`。
- **Python SDK 有明确的平台白名单**（Linux x64/arm64、macOS 14+ arm64/x64、Windows x64）。Windows 上持久 shell 用 `pwsh` 而非 `bash`；Windows arm64 没有 wheel，该形态需要程序化接入时走 CLI 或 ACP/JSON-RPC（第七、第六章有更细说明）。
...
- **可以钉死版本**。developer preview 期破坏性变更频繁，脚本或 CI 里建议固定版本号，避免 `npx` 每次拉到不同的最新版：
  ...
  ```sh
  npx @deepseek-ai/dsh@0.1.6-alpha.2 web
  ```
