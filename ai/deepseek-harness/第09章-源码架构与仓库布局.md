...
| `native/` | 原生辅助程序：`native/system` 工作区（`@deepseek-ai/node-addon-system`），landlock-run 启动器与 flock 等条目的平台预编译二进制 |
| `examples/` | 已移除：原顶层可运行组合示例不再存在，官方可运行形态现由五个随附 profile 模板（`web` / `headless` / `sdk` / `sdk-minimal` / `acp`）承载 |
...
如果你第一次打开这个仓库，推荐的阅读顺序是：`README.md` → `docs/architecture.md` → `docs/development.md` → `packages/README.md`，然后挑一个内置 profile 模板（比如 `headless`）看它的 bundle patch 叠加与 [composition.md](https://github.com/deepseek-ai/deepseek-harness/blob/master/apps/cli/composition.md)，最后钻进 `packages/core/` 的五个脊柱包。architecture.md 也建议"用 agent 探索代码库"——因为 Cordis 的插件树很难靠线性读文件理解。
...
| `ssh/` | POSIX SSH 远程执行 provider 族：共享连接 + 配套的 fs / subprocess / sandbox 远程 provider |
...
| `workflow/` | Workflow seam、PTC 进程引擎、模型可见 `workflow`/`ralph` 工具 |
...
| `runtime-diagnostics/` | 运行时诊断：包内 invariant 检查与报告（发布预期 Support） |
...
`core/` 组还有一层更细的分工值得记住：`agent` 拥有公开契约，`agent-loop` 是它的默认实现；扩展插件依赖 `dsh-agent` 这个 seam，所以 driver 始终可替换。而"可运行的组合"不属于 `core/`，属于 `bundle/` 的 profile 层——`core/` 只提供可替换的脊柱零件，不提供整机。
...
- **扩展插件依赖 Service Definition，从不依赖具体 provider。** `dsh-agent-loop` 可替换；UI、hook、工具插件都依赖 `dsh-agent`。组合 bundle（`packages/bundle/*`）可以依赖主干（spine）插件。
...
pnpm dsh --profile acp                                    # ACP 自动化 server（JSON-RPC stdio）
pnpm run demo:ptc                                         # 以 PTC 工具模式跑一次 headless 任务
```
...
3. 看一个 `apps/cli/config/examples/` 里的 `cordis.yml`，理解"产品 = 有序插件树"。
...
## 9.15 入口与示例：apps 与 profile 模板

`apps/` 与内置 profile 模板是理解"包如何变成可运行程序"的两块拼图。
...
**可运行组合已 profile 化。** 原顶层 `examples/`（headless-agent / acp-agent / jsonrpc-agent 等）与 `packages/examples/` 演示 bundle 都已从仓库移除：官方可运行形态现在由五个内置 profile 模板承载——`web`、`headless`、`sdk`、`sdk-minimal`、`acp`，首次使用自动从模板初始化，launcher 按序叠加它们的 bundle patch。此外 `apps/cli/config/examples/` 保留了少量可直接阅读的 `cordis.yml` 组合示例（`github-review`、`mcp-memory`、`schedule`），是理解"产品 = 有序插件树"的现成教材；每个随附 profile 的确切组成见生成文档 [apps/cli/composition.md](https://github.com/deepseek-ai/deepseek-harness/blob/master/apps/cli/composition.md)。

## 9.16 本章小结

- 仓库是 pnpm monorepo，`packages/` 按 `<group>/<pkg>` 组织，包名统一 `@deepseek-ai/dsh-<pkg>`，分组不进入 npm scope。
- 顶层目录职责清晰：`vendor/` 是内嵌 Cordis，`apps/` 是入口，`python/` 与 `native/` 是跨语言部分，可运行组合由内置 profile 模板与 `apps/cli/config/examples/` 承载，`.agents/` 是设计决策日志。
- `packages/` 绝大多数分组是"能力族"，按 Service Definition / Service Provider / Consumer 三角色拆分，依赖方向单向。
- profile 与 bundle 通过 `package.json` 的 `dsh` 字段自描述；`dsh-base` 是除 `sdk-minimal`（自带完整树）之外各内置 profile 的第一层。
