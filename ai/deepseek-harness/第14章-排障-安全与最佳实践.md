...
Landlock 的底层是 [native/system](https://github.com/deepseek-ai/deepseek-harness/blob/master/native/README.md)——一个"自我限制然后 exec"的启动器（landlock-run），以原生 addon 包 `@deepseek-ai/node-addon-system` 的 `/landlock-run` 子路径发布，平台二进制（Linux x64/arm64、macOS arm64/x64）随子包提供。sandbox-local 只负责"模式到授予的映射 + runner 选择"，路径解析与 probe 解析留给带版本的二进制，防止契约漂移。

### 14.3.4 SSH 远程执行 provider 族

[packages/ssh/](https://github.com/deepseek-ai/deepseek-harness/blob/master/packages/ssh/README.md) 把整个文件系统/进程执行世界放进一台 POSIX SSH 远端主机：harness 与模型、会话存储留在本地，只有文件与进程在远端（早期的 E2B 远程 POC 已从仓库移除，这条路线现在由 SSH provider 族承载）：

| 包 | 角色 |
|---|---|
| `@deepseek-ai/dsh-ssh` | 一条共享 OpenSSH 连接：由部署方提供的 alias 承载认证与主机身份，远端安装 helper；就绪前校验安装产物摘要，连接关闭或租约到期时由 helper 负责远端清理 |
| `@deepseek-ai/dsh-fs-ssh` | 用同一条连接实现文件系统 seam |
| `@deepseek-ai/dsh-subprocess-ssh` | 用同一条连接实现可执行查找、进程组、stdio、终端会话 |
| `@deepseek-ai/dsh-sandbox-ssh` | 文件效果沙箱的远端实现 |

四个包在同一 host 上落在同一台远端 Linux runtime 里，高层能力仍由 provider 中立的消费者构建。关键点在于**边界没动**：harness 进程、Cordis 对象、模型调用、agent/会话状态、持久化、skills、高层协议状态都留在本地——只有"执行世界"（文件与进程）被搬到远端。

### 14.3.5 远程沙箱为什么能迁移 Bash/PTY/LSP

dsh-bash-local、dsh-terminal-bash、dsh-lsp-stdio 不需要 SSH 专用 fork：它们把每个"执行世界"操作委托给 `ctx.fs` 和 `ctx.subprocess`。所以挂上 `fs-ssh` / `subprocess-ssh` 这一族适配器，Bash、PTY 终端、LSP 的可变工作就自动落进同一个远程主机。这正是第 11 章"capability seam = Service Definition / Provider / Consumer"设计带来的红利：**换 provider，不换消费者。**
...
### 14.6.2 Python SDK 的平台组合

Python SDK 教程明确列出前置条件：Linux x64 / Linux arm64 / macOS 14 或更新的 arm64 / macOS x64，或 Windows x64：

> Linux 与 macOS wheel 带 target-native 的 `-rg` sidecar，Windows 带 `-rg.exe`，macOS 还额外带 `node-pty` 的 `-spawn-helper`；Windows 的 ConPTY 原生 addon 随包提供。

如果你的目标是 **Windows arm64** 上跑 Python SDK，当前版本没有该平台的 wheel——发布目标只到 Windows x64。
...
- 沙箱后端：本地三平台 runner（bwrap/Landlock、Seatbelt、Windows ACL），SSH 远程 provider 族（fs-ssh/subprocess-ssh/sandbox-ssh），native/system 的 landlock-run addon；换 provider 不换消费者，Bash/PTY/LSP 自动迁入远程执行世界。
