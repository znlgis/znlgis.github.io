...
> **写作时点声明**：本文写作于 2026 年 8 月，对应仓库版本 `0.1.6-alpha.2`。项目目前处于 **developer preview（开发者预览）** 阶段，官方在 [README](https://github.com/deepseek-ai/deepseek-harness/blob/master/README.md) 中明确警告 **THERE WILL BE COMPATIBILITY-BREAKING CHANGES（将出现破坏兼容性的变更）**。文中所有命令、配置键、包名均以 `master` 分支原文核对，但仍可能随上游迭代而失效，请以仓库当前代码为准。
...
- **pnpm workspace monorepo**。`workspaces` 字段涵盖 `vendor/*`、`packages/*/*`、`native/system`（及其 `packages/*`）、`apps/*`、`website`。所有内部包以 `@deepseek-ai/dsh-<name>` 命名，跨包用包名导入。
...
- 内置 bundle 大多骑在同一个底层 bundle `dsh-base` 之上——它提供模型适配器、工具、持久化、沙箱与审批策略、settings/credentials、telemetry 等公共能力（见 [dsh-base README](https://github.com/deepseek-ai/deepseek-harness/blob/master/packages/bundle/base/README.md)）。
- `dsh-web-app` 在 base 上加浏览器应用；`dsh-headless` 在 base 上加一个「无服务器的一次性 runner」；`dsh-acp-app` / `dsh-sdk-app` 同理挂载 ACP 与 SDK 自动化面，而 `dsh-sdk-minimal` 不带 base、自带完整插件树。
- `web` profile = base + web-app，`headless` profile = base + headless，`acp` profile = base + acp-app，`sdk` profile = base + sdk-app，`sdk-minimal` 独立成型。

因此你理解一次「插件树怎么拼」，就同时理解了三个入口——这也是第五章 CLI / Profile / Bundle 体系会展开的主题。

### 1.5.5 源码 checkout 下的演示命令

如果你从源码克隆（第二章路线 B），[开发指南](https://github.com/deepseek-ai/deepseek-harness/blob/master/docs/development.md)的 Profile runs 一节给出了源码形态的演示命令。先构建一次，然后：
...
# 2. PTC 模式演示：同一 headless profile，启用代码呈现（PTC）模式（需 key）
pnpm run demo:ptc -- "summarize this workspace"
```

第二条 `demo:ptc` 值得记住：它在同一个一次性 runner 上打开 PTC 模式（模型编写的 TypeScript 在全新 Node 进程中、与 Bash 同一沙箱策略下执行），直观展示了「agent 循环的工具面可以从配置层整体换档」——这也是第五章与第十一章会展开的 seam 思想的一个缩影。
...
| **bundle** | Cordis 配置行 + 其所挂载代码的分发格式；`dsh-base` / `dsh-web-app` / `dsh-headless` / `dsh-acp-app` / `dsh-sdk-app` / `dsh-sdk-minimal` 是六个内置 bundle | 第五章 |
...
| Stars | 约 220k（截至 2026 年 9 月） |
| 许可证 | MIT |
| 当前版本 | `0.1.6-alpha.2`（根 `package.json`） |
...
- 生态事实：MIT、约 220k stars、`0.1.6-alpha.2`、developer preview（破坏性变更警告）、社区走 GitHub Discussions / Discord / 企微群、插件加 `dsh-plugin` topic。
