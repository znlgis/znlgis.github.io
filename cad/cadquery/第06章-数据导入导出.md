...
从 v2.8 开始，CadQuery 的 STEP 导出支持 `unit` 参数，允许指定导出文件的长度单位：
...
# 默认单位为毫米
cq.exporters.export(box, "box_mm.step", unit="MM")

# 厘米
cq.exporters.export(box, "box_cm.step", unit="CM")

# 米
cq.exporters.export(box, "box_m.step", unit="M")

# 英寸
cq.exporters.export(box, "box_inch.step", unit="INCH")
```

**支持的单位：** `"MM"`（毫米，默认）、`"CM"`（厘米）、`"M"`（米）、`"INCH"`（英寸）。

**为什么单位很重要？** 在精密制造和跨国协作中，单位不匹配可能导致零件尺寸完全错误。例如，一个 50mm 的零件如果以英寸单位打开，会被读作 50 英寸（1270mm），造成严重尺寸偏差。明确指定单位可以避免这类兼容性问题。

STEP 导入同样支持单位参数（v2.8+）：

```python
import cadquery as cq

# 导入时指定源文件单位
part = cq.importers.importStep("part_inch.step", unit="INCH")

# 导入后自动转换为内部单位（毫米）
print(part.solids().val().BoundingBox())  # 尺寸已经是毫米
```
...
### 6.2.6 带材料的STEP导入导出（v2.7+）

从 v2.7 开始，CadQuery 支持在 STEP 文件中导入导出材料信息，实现与主流 CAD 软件的材料互操作性。

**导出时保留材料信息：**

```python
import cadquery as cq
from cadquery import Material

box = cq.Workplane("XY").box(50, 30, 20)

# 设置材料属性
box.material = Material(name="Aluminum", density=2.7)
...
# 导入包含材料的 STEP 文件
part = cq.importers.importStep("box.step")

# 读取材料属性
print(part.material)  # 输出: Material(name="Aluminum", density=2.7)
print(part.material.name)    # "Aluminum"
print(part.material.density) # 2.7
```

**与其他 CAD 软件的互操作性：**

- **SolidWorks**：导出 STEP 时选择"包含材料"选项，CadQuery 可读取材料名称和密度
- **FreeCAD**：材料信息通过 STEP AP242 标准交换，双侧均可读写
- **CATIA/NX**：支持 STEP AP242 材料扩展，材料名称可互通
- **注意事项**：并非所有 CAD 软件都完整支持 STEP 材料信息。密度值在不同软件间可能有微小精度差异，建议导入后校验关键参数。
...
- **`.gltf`**：基于JSON的文本格式，附带外部资源文件（.bin、纹理等），适合调试和版本管理
- **`.glb`**：二进制格式，所有资源打包在单一文件中，文件更紧凑、加载更快，推荐用于生产环境和Web发布

**装配体glTF导出：** glTF导出会保留装配体的层级结构，使每个子部件在Web查看器中可作为独立节点操作。这与第05章介绍的装配体导出机制一致，详见[第05章-装配体与约束系统](https://znlgis.github.io/cad/cadquery/第06章-数据导入导出/第05章-装配体与约束系统/)。
...
<div style="display: flex; justify-content: space-between; margin: 20px 0;">
  <a href="https://znlgis.github.io/cad/cadquery/第06章-数据导入导出/第05章-装配体与约束系统/" style="text-decoration: none;">← 上一章</a>
  <a href="https://znlgis.github.io/cad/cadquery/第06章-数据导入导出/" style="text-decoration: none;">目录</a>
  <a href="https://znlgis.github.io/cad/cadquery/第06章-数据导入导出/第07章-高级建模技巧/" style="text-decoration: none;">下一章 →</a>
</div>
