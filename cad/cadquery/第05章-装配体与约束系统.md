...
v2.6+ 版本增强了STEP文件导入功能，支持直接导入含名称、图层和颜色信息的装配体文件：

```python
import cadquery as cq

# 直接导入STEP装配体
assembly = cq.import_step("assembly.step")

# 子装配体和零件保留原始名称和颜色
# 可以遍历查看装配结构
for name, sub in assembly.traverse():
    shape = sub.obj
    if shape is not None:
        print(f"零件: {name}, 颜色: {sub.color}")

# 进一步编辑或导出
assembly.save("modified_assembly.step")
```
...
> **提示**：关于STEP、STL等格式的详细导入导出处理，请参阅[第06章：数据导入导出](https://znlgis.github.io/cad/cadquery/第05章-装配体与约束系统/第06章-数据导入导出/)。

### 5.6.5 XBF/XML格式导入导出

XBF（eXtensible Binary Format）是CadQuery的**原生装配体格式**，以XML为基础完整保留装配体结构、约束关系和元数据：
...
# 导出为XBF格式
cq.export(assy, "assembly.xbf")

# 从XBF格式导入
assy_loaded = cq.importers.importXBF("assembly.xbf")

# 验证导入后的结构
print(f"零件数量: {len(list(assy_loaded.traverse()))}")
```
...
与第06章导入导出章节配合使用，可以实现完整的装配体数据交换工作流。

### 5.6.6 装配体材料支持

v2.7+ 版本引入了装配体材料属性，可以为装配体或子零件设置材料信息，支持密度、颜色等属性：

```python
import cadquery as cq
from cadquery import Material, Assembly

# 定义材料
steel = Material(name="Steel", density=7.85, color=(128, 128, 128))
aluminum = Material(name="Aluminum", density=2.70, color=(200, 200, 220))
brass = Material(name="Brass", density=8.50, color=(180, 150, 50))

# 创建零件并设置材料
base = cq.Workplane("XY").box(50, 50, 10)
shaft = cq.Workplane("XY").cylinder(30, 10)
bushing = cq.Workplane("XY").circle(10).circle(8).extrude(10)

# 创建装配体
assy = cq.Assembly()
assy.add(base, name="base", material=steel)
assy.add(shaft, name="shaft", material=steel,
         loc=cq.Location(cq.Vector(0, 0, 10)))
assy.add(bushing, name="bushing", material=brass,
         loc=cq.Location(cq.Vector(0, 0, 20)))

# 材料随STEP导出/导入
assy.save("material_assembly.step")

# 导入后材料信息会保留
loaded = cq.import_step("material_assembly.step")
```

**材料属性说明：**
- `name`：材料名称标识
- `density`：密度（g/cm³），用于质量计算
- `color`：RGB颜色元组，用于可视化显示

**应用场景：**
- **质量计算**：根据密度和体积自动计算零件质量
- **BOM生成**：按材料类型统计零件清单
- **渲染显示**：不同材料使用不同颜色呈现
- **工程分析**：为CAE工具提供材料信息

> 材料信息随STEP文件完整导入导出，确保在不同CAD系统间传递材料属性。
...
<div style="display: flex; justify-content: space-between; margin: 20px 0;">
  <a href="https://znlgis.github.io/cad/cadquery/第05章-装配体与约束系统/第04章-选择器系统详解/" style="text-decoration: none;">← 上一章</a>
  <a href="https://znlgis.github.io/cad/cadquery/第05章-装配体与约束系统/" style="text-decoration: none;">目录</a>
  <a href="https://znlgis.github.io/cad/cadquery/第05章-装配体与约束系统/第06章-数据导入导出/" style="text-decoration: none;">下一章 →</a>
</div>
