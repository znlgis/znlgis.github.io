...
形态学（Mathematical Morphology）思想源自图像处理：用一个"结构元素"在目标形状上滑动，通过膨胀与腐蚀两种基本操作及其组合，实现去噪、平滑、边界提取等变换。这套思想同样适用于矢量多边形——结构元素就是一把"刷子"，沿多边形边界向外扫过一圈得到膨胀，向内扫过一圈得到腐蚀。

Clipper1 的 `ClipperOffset` 恰好提供了形态学所需的全部原料：正 delta 是向外扫过的膨胀，负 delta 是向内扫过的腐蚀。而连接类型决定了结构元素的形状：`jtRound` 对应圆盘结构元素（各向同性），`jtSquare` 对应方形结构元素。

### 4.8.1 膨胀与腐蚀

**膨胀（Dilation）**：多边形向外扩张。细小突起被放大，小于结构元素直径的孔洞被填满，相邻图斑间的距离缩小。

**腐蚀（Erosion）**：多边形向内收缩。细小突起被削平，窄于结构元素直径的"脖颈"被切断，尺寸不足的小图斑可能整体消失。

使用 `ClipperOffset` 实现时需要注意两点：

1. **方向约定**：外轮廓需要正面积方向（CCW），孔洞取相反方向，Clipper1 据此判断偏移朝向
2. **Execute 签名**：`Execute(ref Paths solution, double delta)`，solution 参数必须加 `ref`
...
class MorphologyBasic
{
    private const double SCALE = 1e6;

    /// <summary>
    /// 膨胀：正 delta 向外偏移（jtRound = 圆盘结构元素）
    /// </summary>
    public static Paths Dilate(Paths input, double radius)
    {
        ClipperOffset co = new ClipperOffset();
        co.AddPaths(input, JoinType.jtRound, EndType.etClosedPolygon);

        Paths dilated = new Paths();
        co.Execute(ref dilated, radius * SCALE);
        return dilated;
    }
    ...
    /// 腐蚀：负 delta 向内偏移
    /// </summary>
    public static Paths Erode(Paths input, double radius)
    {
        ClipperOffset co = new ClipperOffset();
        co.AddPaths(input, JoinType.jtRound, EndType.etClosedPolygon);

        Paths eroded = new Paths();
        co.Execute(ref eroded, -radius * SCALE);
        return eroded;
    }
}
```
...
- `jtRound` 对应**圆盘结构元素**，膨胀结果各向同性，与图像处理中的圆形卷积核等价
- `jtSquare` 对应**方形结构元素**，角点处保持直角，适合需要正交效果的场景
- 腐蚀距离过大时多边形整体消失，`Execute` 返回空 `Paths`，调用方需处理 `solution.Count == 0` 的情况
- delta 与 radius 都要先乘以缩放系数，与输入坐标保持同一量纲

### 4.8.2 开运算与闭运算

单独膨胀或腐蚀都会改变图斑的总体尺寸。实践中更常用的是两者的组合：

**开运算（Opening）**：先腐蚀后膨胀。几何效果是**剔除细小突起、断开窄桥**，主体形状与面积基本保持不变。

**闭运算（Closing）**：先膨胀后腐蚀。几何效果是**填充凹陷、弥合窄缝**，把相邻近的图斑"拉拢"在一起。

```csharp
class MorphologyOpenClose
{
    private const double SCALE = 1e6;
    ...
    /// 开运算：先腐蚀后膨胀
    /// </summary>
    public static Paths Opening(Paths input, double radius)
    {
        Paths eroded = Offset(input, -radius * SCALE);
        if (eroded.Count == 0)
            return new Paths();  // 腐蚀后无剩余，主体已被完全剔除

        return Offset(eroded, radius * SCALE);
    }

    /// <summary>
    /// 闭运算：先膨胀后腐蚀
    /// </summary>
    public static Paths Closing(Paths input, double radius)
    {
        Paths dilated = Offset(input, radius * SCALE);
        return Offset(dilated, -radius * SCALE);
    }

    private static Paths Offset(Paths input, double delta)
    {
        ClipperOffset co = new ClipperOffset();
        co.AddPaths(input, JoinType.jtRound, EndType.etClosedPolygon);
...
- 开运算的腐蚀阶段会"掐断"宽度小于结构元素直径的窄桥，随后的膨胀无法复原，窄桥被永久移除
- 闭运算的膨胀阶段会"填平"宽度小于结构元素直径的窄缝与凹口，随后的腐蚀无法复原，缝隙被永久弥合
- 两步之间产生的碎片会在第二步被主体自然吸收

### 4.8.3 形态学梯度

形态学梯度定义为**膨胀结果减去腐蚀结果**，得到的是沿原始边界的一圈边界带，带宽约为结构元素直径（2 倍半径）。常用于边界带提取与轮廓强调。

```csharp
class MorphologyGradient
{
    private const double SCALE = 1e6;

    /// <summary>
    /// 形态学梯度：膨胀 ctDifference 腐蚀（边界带提取）
    /// </summary>
    public static Paths Gradient(Paths input, double radius)
    {
        ClipperOffset co = new ClipperOffset();
        co.AddPaths(input, JoinType.jtRound, EndType.etClosedPolygon);
        ...
        Paths band = new Paths();
        clipper.Execute(ClipType.ctDifference, band);
        return band;
    }
}
```
...
- 腐蚀结果为空（图斑宽度不足 2 倍半径）时，`AddPaths` 空 clip 不影响差集结果，梯度退化为膨胀结果本身，代码无需特殊处理
- 边界带既可用于可视化强调，也可作为后续距离分析的输入（如道路红线带、保护区缓冲带）

### 4.8.4 应用场景

#### 小图斑剔除（开运算 + 面积过滤）

GIS 数据中常见面积微小的碎图斑与噪点。先用开运算断开它们与主体的细窄连接，再按面积阈值过滤：
...
class SmallPatchRemoval
{
    ...
    /// 小图斑剔除：开运算断开窄连接 + 面积过滤
    /// </summary>
    public static Paths RemoveSmallPatches(Paths patches, double radius, double minArea)
    {
        // 1. 开运算：剔除细小突起，断开与主体的窄桥
        Paths opened = MorphologyOpenClose.Opening(patches, radius);
        ...
        {
            if (Math.Abs(Clipper.Area(path)) >= threshold)
            {
                result.Add(path);
            }
        }
...
相邻地块之间的细缝、轮廓线上的小凹口常干扰后续分析。闭运算可以一次性弥合所有宽度小于结构元素直径的缝隙：
...
class GapFilling
{
    ...
    /// 窄缝弥合与凹口填充：闭运算
    /// </summary>
    public static Paths FillNarrowGaps(Paths input, double radius)
    {
        // 宽度小于 2 * radius 的缝隙与凹口会被填平
        return MorphologyOpenClose.Closing(input, radius);
    }
}
```
...
- 结构元素半径应小于待保留特征的最小宽度，否则会误伤有效细节
- 小图斑剔除时，radius 略大于碎斑尺寸的一半即可；minArea 按业务口径设定
- 圆盘/方形结构元素已覆盖多数场景；任意形状结构元素需借助闵可夫斯基和（Clipper1 提供 `Clipper.MinkowskiSum`，见源码解读系列第19章）
...
6. **性能优化**：参数选择和问题处理
7. **形态学操作**：膨胀、腐蚀、开运算、闭运算与形态学梯度

重点掌握：
- 根据应用场景选择合适的连接类型和端点类型
- 理解正负偏移的几何意义
- 处理偏移操作中的各种边缘情况
- 组合膨胀与腐蚀实现开运算、闭运算等形态学清理
- 在实际项目中灵活应用偏移功能
...
7. **形态学清理**：构造一批带碎斑、窄缝的图斑，分别执行开运算与闭运算，比较不同结构元素半径下的清理效果
...
<div style="display: flex; justify-content: space-between; margin: 20px 0;">
  <a href="https://znlgis.github.io/cad/Clipper1/第04章-多边形偏移操作/第03章-布尔运算操作/" style="text-decoration: none;">← 上一章</a>
  <a href="https://znlgis.github.io/cad/Clipper1/第04章-多边形偏移操作/" style="text-decoration: none;">目录</a>
  <a href="https://znlgis.github.io/cad/Clipper1/第04章-多边形偏移操作/第05章-填充规则与高级特性/" style="text-decoration: none;">下一章 →</a>
</div>
