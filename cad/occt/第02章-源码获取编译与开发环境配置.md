...
cd OCCT
git fetch --tags
git checkout V8_0_0   # 或者最新稳定 tag
```

仓库默认分支是 `master`，长期稳定版本以 `V7_x_y`、`V8_x_y` 形式打 tag，发布说明在 `dox/dev_guides/upgrade` 与每个版本的 release notes 中。如需贡献代码，应阅读 `CONTRIBUTING.md` 并使用 Open Cascade 的 mantis bugtracker 流程。
...
```cmake
find_package(OpenCASCADE 8.0 REQUIRED COMPONENTS
