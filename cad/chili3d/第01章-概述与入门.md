...
Chili3D是一款基于Web浏览器的开源3D CAD（计算机辅助设计）应用程序，专为在线模型设计和编辑而构建。该项目由开发者"仙阁"创建，采用TypeScript编写，通过将OpenCascade（OCCT）几何建模内核编译为WebAssembly，并与Three.js 3D渲染引擎集成，实现了接近原生应用程序的性能表现。最新版本为0.7.1（2026年9月）。相比早期的0.6.1（新增暗色主题、可调整侧边栏、改进的相机导航以及STEP/IGES/BREP格式导出），0.7.0 是一次大版本更新：引入了完整的草图工具集（两点弧、三点弧、TTR 相切弧、点、螺旋线、正多边形）、新的建模与修改命令（Pipe 扫掠、Shell 抽壳、Sew 缝合、Check/Repair Shape 检查与修复、2D 圆角/倒角、Extend 延伸修剪），内置插件系统（Macro 脚本与 Grasshopper 风格的 Visual Programming 可视化编程），数组支持线性/矩形/圆形/沿曲线四种模式，布尔运算实时预览，新增表面/曲线/相切/顶点捕捉，重设计首页、键盘快捷键系统与俄语本地化，并将底层 OCCT 几何内核升级到 v8.0.1。Chili3D的目标是让用户无需安装任何本地软件，即可在浏览器中完成强大的3D建模、编辑和渲染操作。
...
<div style="display: flex; justify-content: space-between; margin: 20px 0;">
  <span></span>
  <a href="https://znlgis.github.io/cad/chili3d/" style="text-decoration: none;">目录</a>
  <a href="https://znlgis.github.io/cad/chili3d/第02章-核心架构解析/" style="text-decoration: none;">下一章 →</a>
</div>
