...
Clipper2 是由澳大利亚程序�?Angus Johnson 开发的 Clipper1 库的全面升级版本。它不仅继承�?Clipper1 的所有功能，还在算法效率、代码结构、API 设计等方面进行了大量改进。本教程将深入解读其 C# 源代码实现，帮助读者理解核心算法原理和代码设计�?
...
| 特�?| Clipper1 | Clipper2 |
|------|----------|----------|
| 坐标类型 | 仅整数（Int64/Int32�?| 整数 + 浮点双精�?|
| 命名空间 | ClipperLib | Clipper2Lib / Clipper2ZLib |
| 类型别名 | Path = List<IntPoint> | Path64 类、PathD �?|
| Z坐标支持 | 条件编译 | 条件编译（USINGZ�?|
| 矩形裁剪 | 无专门优�?| RectClip64 高速算�?|
...
**Clipper1**：所有代码在单个 `clipper.cs` 文件中，�?4800 行�?
...
├── Clipper.Minkowski.cs # Minkowski �?�?
...
1. **开源许�?*：采�?Boost Software License，允许商业和非商业使�?
2. **持续维护**：项目从 2010 年开始，持续更新至今
3. **社区贡献**：感谢多位开发者对 C# 版本的贡�?
...
- **默认（未定义 USINGZ�?*：使�?`Clipper2Lib` 命名空间，坐标只�?X、Y
- **定义 USINGZ**：使�?`Clipper2ZLib` 命名空间，坐标增�?Z 分量
...
源代码大量使�?`MethodImplOptions.AggressiveInlining`�?
...
这告�?JIT 编译器尽可能将方法内联，减少方法调用开销，对于频繁调用的小方法特别有效�?
...
Clipper2 仍然基于 Bala Vatti �?1992 年发表的多边形裁剪算法。该算法的核心思想是使用扫描线方法处理多边形：
...
**局部极小值（Local Minima�?*�?
- 多边形轮廓中 Y 坐标最低的点（�?Y 轴向下的坐标系中�?
- 算法从这些点开始向上扫�?

**活动边表（AEL�?*�?
- 存储当前扫描线穿过的所有边
- �?X 坐标排序
- �?Clipper2 中由 `Active` 结构表示
...
Clipper2 需�?.NET Standard 2.0 或更高版本，支持�?
- .NET Framework 4.6.1+
- .NET Core 2.0+
- .NET 5/6/7/8+
...
本章介绍�?Clipper2 C# 源代码的整体结构和设计理念：

1. **模块化设�?*：多个源文件分工明确
2. **双精度支�?*：整数核�?+ 浮点封装
3. **现代 C# 特�?*：可空引用、内联优�?
4. **算法基础**：仍基于经典�?Vatti 扫描线算�?
5. **简�?API**：静态方�?+ 类实例两种风�?
