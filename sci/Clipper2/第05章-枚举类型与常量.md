...
```csharp
clipper.AddSubject(subjectPath);  // 等同�?AddPath(..., PathType.Subject, false)
clipper.AddClip(clipPath);        // 等同�?AddPath(..., PathType.Clip, false)
```
...
**注意**：当角度很小时，斜接点会非常远，因此�?`MiterLimit` 参数限制�?
...
## 5.7 PointInPolygonResult 点在多边形中的结�?
...
switch (result)
{
    case PointInPolygonResult.IsInside:
        Console.WriteLine("点在多边形内�?);
        break;
    case PointInPolygonResult.IsOutside:
        Console.WriteLine("点在多边形外�?);
        break;
...
| 常量 | �?| 说明 |
|------|-----|------|
| `MaxInt64` | 2⁶�?1 | 64位有符号整数最大�?|
| `MaxCoord` | MaxInt64/4 | 最大坐标值，预留溢出空间 |
| `Invalid64` | MaxInt64 | 用于标记无效�?|
| `floatingPointTolerance` | 10⁻¹�?| 浮点数近似相等容�?|
| `defaultMinimumEdgeLength` | 0.1 | 默认最小边�?|

### 5.9.3 为什�?MaxCoord = MaxInt64 / 4�?
...
如果坐标接近 `MaxInt64`，差值可能接�?`2 * MaxInt64`，乘积可能接�?`4 * MaxInt64²`，超�?`long` 范围�?

�?`MaxCoord` 限制�?`MaxInt64 / 4`，确保中间计算不会溢出�?

## 5.10 本章小结

本章详细介绍�?Clipper2 的枚举类型和常量�?
...
理解这些枚举类型是正确使�?Clipper2 的基础�?

---

[上一章：矩形边界](�?4�?矩形边界) | [返回目录](../index) | [下一章：InternalClipper内部工具类](�?6�?InternalClipper内部工具�?
