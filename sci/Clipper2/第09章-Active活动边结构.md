...
�?Vatti 扫描线算法中，活动边表（Active Edge List, AEL）是核心数据结构之一。`Active` 类表示当前扫描线穿过的边，这些边�?X 坐标排序，用于确定多边形区域的边界�?
...
internal class Active
{
    ...
    public long curX;            // 当前扫描线上�?X 坐标
    
    public double dx;            // 斜率的倒数 (dx/dy)
    
    public int windDx;           // 缠绕方向 (+1 �?-1)
    ...
    public LocalMinima localMin; // 所属的局部极小�?
...
| `curX` | `long` | 当前扫描�?Y 处的 X 坐标 |
| `dx` | `double` | 斜率倒数，用于X坐标插�?|
| `windDx` | `int` | 边的方向�?1向上�?1向下 |
| `windCnt` | `int` | 当前边的缠绕计数 |
| `windCnt2` | `int` | 另一类型路径的缠绕计�?|
...
```
AEL �?←→ Active1 ←→ Active2 ←→ Active3 ←→ ... ←→ null
          (X=10)     (X=30)      (X=50)
```
...
水平边的 `dx` �?`MaxValue` �?`MinValue`，需要特殊处理�?
...
当多条边在同一点汇合时，使�?`mergeJump` 快速找到需要处理的边�?
...
理解 Active 的结构和操作是理�?Clipper2 扫描线算法的关键�?

---

[上一章：ClipperBase基类详解](�?8�?ClipperBase基类详解) | [返回目录](../index) | [下一章：Vertex与LocalMinima](�?0�?Vertex与LocalMinima)
