...
填充规则（Fill Rule）决定了多边形的哪些区域被视�?内部"。这在处理自相交多边形、嵌套多边形和复杂形状时尤为重要。Clipper2 支持四种填充规则：EvenOdd、NonZero、Positive �?Negative�?
...
{
    return (ae.windCnt & 1) != 0;  // windCnt 为奇�?
}
```
...
[上一章：布尔运算执行流程](�?4�?布尔运算执行流程) | [返回目录](../index) | [下一章：ClipperOffset偏移类详解](�?6�?ClipperOffset偏移类详�?
