...
`ClipperD` �?Clipper2 提供的浮点坐标裁剪类。它内部将浮点坐标缩放为整数进行计算，然后将结果转换回浮点数。这种设计既保留了整数运算的精确性，又提供了浮点接口的便利性�?
...
```csharp
// roundingDecimalPrecision 范围�?8 �?8
// 
// precision = 2:  _scale = 100,    精确到小数点�?�?
// precision = 3:  _scale = 1000,   精确到小数点�?�?
// precision = 0:  _scale = 1,      整数
// precision = -2: _scale = 0.01,   精确到百�?
```
...
整数坐标 �?浮点坐标（乘�?invScale�?
(1234, 6789) �?(12.34, 67.89) [precision=2]
```

### 13.3.2 Scale �?InvScale
...
```csharp
ClipperD clipper = new ClipperD(3);  // 3 位小数精�?
...
```csharp
// 问题：输出坐标与输入略有不同
ClipperD clipper = new ClipperD(2);  // 2 位小�?

PointD input = new PointD(12.345, 67.891);
// 内部�?1234, 6789)
// 输出�?12.34, 67.89) - 第三位小数丢�?

// 解决：使用更高精�?
ClipperD clipper = new ClipperD(4);  // 4 位小�?
```
...
{
    x = pt.X * scale;  // scale 通常�?invScale
    y = pt.Y * scale;
#if USINGZ
    z = pt.Z;
#endif
...
[上一章：Clipper64裁剪类详解](�?2�?Clipper64裁剪类详�? | [返回目录](../index) | [下一章：布尔运算执行流程](�?4�?布尔运算执行流程)
