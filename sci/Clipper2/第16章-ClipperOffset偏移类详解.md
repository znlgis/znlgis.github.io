---
layout: default
title: �?6章：ClipperOffset 偏移类详�?
---

# �?6章：ClipperOffset 偏移类详�?

## 16.1 概述

`ClipperOffset` 类用于对多边形和路径进行偏移（膨胀或收缩）操作。这�?CAD、CNC 加工、地图缓冲区分析等领域有广泛应用�?
...
1. **JoinType**：四种拐角连接方�?
2. **EndType**：五种端点处理方�?
3. **参数控制**：MiterLimit �?ArcTolerance
4. **自动合并**：使用裁剪器合并重叠区域
...
[上一章：填充规则详解](�?5�?填充规则详解) | [返回目录](../index) | [下一章：RectClip矩形裁剪优化](�?7�?RectClip矩形裁剪优化)
