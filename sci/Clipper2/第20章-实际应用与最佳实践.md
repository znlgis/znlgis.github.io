...
上面这种"外扩内缩取差"的模式（先 `Execute(+w/2)` 得到外边界，再 `Execute(-w/2)` 得到内边界，最后 `Difference`）其实是形态学组合运算的一个特例：膨胀（正 delta）与腐蚀（负 delta）之差正是形态学梯度 `(A ⊕ B) \ (A ⊖ B)`，它提取出形状的边界带。同理，"先收缩再膨胀"对应开运算、"先膨胀再收缩"对应闭运算。关于膨胀、腐蚀、开运算、闭运算与梯度的完整推导和实现，参见[第18章：Minkowski 和与差](https://znlgis.github.io/sci/Clipper2/第18章-Minkowski和与差/)的形态学小节。
...
[上一章：PolyTree多边形树结构](https://znlgis.github.io/sci/Clipper2/第19章-PolyTree多边形树结构/) | [返回目录](https://znlgis.github.io/sci/Clipper2/)
