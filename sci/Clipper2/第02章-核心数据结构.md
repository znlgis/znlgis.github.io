---
layout: default
title: �?章：核心数据结构 - Point64、PointD
---

# �?章：核心数据结构 - Point64、PointD

## 2.1 概述

�?Clipper2 中，所有几何运算的基础是点（Point）数据结构。与 Clipper1 不同，Clipper2 提供了两种点类型�?
...
2. **公共字段**：直接暴�?X、Y 字段，避免属性访问开销
3. **条件 Z 坐标**：通过预处理器指令可选启�?
...
注意所有涉及浮点到整数转换的地方都使用�?`MidpointRounding.AwayFromZero`�?
...
**注意**：相等判断不比较 Z 坐标！这是设计决策——在 2D 裁剪操作中，只关�?X、Y 坐标的相等性�?
...
输出格式�?`"X,Y "` �?`"X,Y,Z "`，注意末尾有空格，便于路径的字符串连接�?

### 2.2.6 Equals �?GetHashCode
...
`GetHashCode` 使用 `HashCode.Combine` 方法，这�?.NET Core 2.1+ 引入的高效哈希计算方法。注释中�?`#599` �?GitHub issue 编号，表明这是一个经过社区讨论和优化的实现�?
...
#if USINGZ
    public long z;  // 注意：Z 仍然是整�?
#endif
}
```

**注意**：即使在浮点点结构中，Z 坐标仍然�?`long` 类型。这是因�?Z 通常用于存储额外信息（如索引、标记），而非连续坐标�?
...
点类型被设计�?`struct` 而非 `class`，原因：

1. **避免堆分�?*：结构体存储在栈上或内联在容器中
2. **�?GC 压力**：不需要垃圾回�?
...
虽然属性提供封装性，但在性能关键代码中，字段访问更快（尽管现�?JIT 通常会内联简单属性）�?
