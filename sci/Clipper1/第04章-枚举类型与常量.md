...
Clipper1 定义了多个枚举类型和常量，用于控制裁剪行为、指定填充规则、设置偏移选项等。理解这些定义对于正确使�?Clipper 至关重要�?
...
`PolyFillType` 定义多边形填充规则，决定哪些区域被视�?内部"�?
...
| 常量 | �?| 说明 |
|------|-----|------|
| `horizontal` | -3.4E+38 | 标识水平边的特殊斜率�?|
| `Skip` | -2 | 标记需要跳过的�?|
| `Unassigned` | -1 | 表示未分配的输出索引 |
| `tolerance` | 1.0E-20 | 浮点数零判断的容�?|
...
| `ioStrictlySimple` | 2 | 确保结果是严格简单的多边�?|
...
这个特殊值用于标识水平边。在 Clipper 的坐标系中（Y 轴向上），水平边的斜率定义为无穷大或极大值。使�?`-3.4E+38` 而非正无穷，是为了便于比较和特殊处理�?
...
�?32 位模式下，坐标范围被严格限制，因为中间计算可能产生溢出�?

### 4.10.2 64位模�?

```csharp
#else
    public const cInt loRange = 0x3FFFFFFF;           // �?10�?
    public const cInt hiRange = 0x3FFFFFFFFFFFFFFFL; // �?4.6×10¹�?
#endif
```

**两个范围的意�?*�?

- **loRange**：低精度范围，中间计算使用普�?64 位整�?
- **hiRange**：高精度范围，中间计算使�?128 位整数（Int128�?

当坐标超�?`loRange` 时，Clipper 自动切换�?128 位计算模式，以避免溢出�?

## 4.11 本章小结

本章详细介绍�?Clipper1 中的枚举类型和常量：
...
[上一章：路径与多边形表示](�?3�?路径与多边形表示) | [返回目录](../index) | [下一章：Int128高精度运算](�?5�?Int128高精度运�?
