...
本章将详细分�?Clipper 执行布尔运算的完整流程，从输入数据的准备到最终结果的构建，深入理�?Vatti 算法�?Clipper 中的实现�?
...
- `LocalMinimaPending()`：检查是否还有未处理的局部极小�?

### 10.4.2 每次迭代的操�?

1. **ProcessHorizontals**：处理当前扫描线上的水平�?
2. **ProcessIntersections**：计算并处理边交�?
3. **ProcessEdgesAtTopOfScanbeam**：处理到达顶部的�?
4. **InsertLocalMinimaIntoAEL**：插入新到达的局部极小�?
...
Intersection（交集）�?
  Subject �?Clip
  边有贡献 �?边在自己类型的多边形�?AND 在对方类型的多边形内

Union（并集）�?
  Subject �?Clip
  边有贡献 �?边在自己类型的多边形边界�?AND 不在对方类型的多边形�?

Difference（差集）�?
  Subject - Clip
  Subject边有贡献 �?边在Subject�?AND 不在Clip�?
  Clip边有贡献 �?边在Clip边界�?AND 在Subject�?
...
foreach (OutRec outRec in m_PolyOuts)
{
    if (outRec.Pts == null) continue;
        ...
        FixupOutPolyline(outRec);  // 清理开放路�?
    else
        FixupOutPolygon(outRec);   // 清理闭合多边�?
}
```
...
5. **结果构建**�?
   - BuildResult：构�?Paths
   - BuildResult2：构�?PolyTree

---

[上一章：Clipper类结构](�?9�?Clipper类结�? | [返回目录](../index) | [下一章：活动边表管理](�?1�?活动边表管理)
