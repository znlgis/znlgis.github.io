...
在多边形裁剪算法中，计算两条边的交点需要进行乘法运算。当使用 64 位整数坐标时，两�?64 位整数相乘可能产�?128 位的中间结果。为了避免溢出并保持精度，Clipper1 实现了一个自定义�?128 位整数结�?`Int128`�?
...
{
    private Int64 hi;   // �?64 位（有符号）
    private UInt64 lo;  // �?64 位（无符号）
...
**原理**�?28 位有符号整数的最高位（hi 的符号位）决定整体符号�?
...
{
    if (val1.hi != val2.hi)
        return val1.hi > val2.hi;  // 先比较高�?
    else
        return val1.lo > val2.lo;  // 高位相同则比较低�?
}
...
```
    lhs.lo = 0xFFFFFFFFFFFFFFFF (最大无符号64�?
  + rhs.lo = 0x0000000000000001
  ─────────────────────────────
  结果.lo = 0x0000000000000000 (溢出，产生进�?
...
// 简化：|val| �?~lo + ~hi * 2^64（忽�?1的精度影响）
double result = -((double)(~lo) + (double)(~hi) * (2^64));
```

### 5.10.2 精度说明

`double` 只有 53 位有效数字，无法精确表示所�?128 位整数值。这个转换主要用于：
- 调试输出
- 近似计算
- 最终结果转�?
...
�?64 位整数分解为高低 32 位后，乘法可以表示为�?

```
         int1Hi    int1Lo
       ×  int2Hi    int2Lo
       ─────────────────────
                   b = int1Lo × int2Lo    (0-64�?
         c1 = int1Hi × int2Lo             (32-96�?
         c2 = int1Lo × int2Hi             (32-96�?
       a = int1Hi × int2Hi                (64-128�?
       ─────────────────────
                   最�?128 位结�?
...
**步骤2：分解为 32 �?*
```csharp
UInt64 int1Hi = (UInt64)lhs >> 32;    // �?2�?
UInt64 int1Lo = (UInt64)lhs & 0xFFFFFFFF;  // �?2�?
```
...
即：每次乘法都创建两�?Int128 对象会很慢，直接使用静态方法更高效�?
...
|------|----------|------|
| 坐标 �?loRange | Int64 | �?|
| 坐标 > loRange �?�?hiRange | Int128 | 较慢（约2-3倍） |
| 坐标 > hiRange | 异常 | N/A |

### 5.13.2 优化建议

1. **尽量使用小坐�?*：如果应用场景允许，将坐标缩放到 loRange 范围�?
2. **避免不必要的精度**：对于不需要极高精度的应用，使用较小的比例因子
3. **预处理坐�?*：在添加�?Clipper 之前检查并调整坐标范围
...
本章详细分析�?Clipper1 �?Int128 高精度整数实现：

1. **设计目的**：避�?64 位乘法的溢出问题

2. **结构设计**�?
   - 使用 hi（有符号�? lo（无符号）表�?128 �?
   - 正确处理符号扩展

3. **核心运算**�?
   - 加法：处理低位进�?
   - 取负：补码取反加1
   - 乘法：将 64 位分解为 32 位部分相�?
...
[上一章：枚举类型与常量](�?4�?枚举类型与常�? | [返回目录](../index) | [下一章：ClipperBase基类详解](�?6�?ClipperBase基类详解)
