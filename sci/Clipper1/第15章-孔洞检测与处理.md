...
- **外轮�?*：逆时针方向的闭合路径，包�?内部"区域
- **孔洞**：顺时针方向的闭合路径，从外轮廓�?挖出"区域
...
void ProcessNode(PolyNode node, int depth)
{
    ...
    if (node.Contour.Count > 0)
    {
        ...
        Console.WriteLine($"{indent}{type}: {node.Contour.Count}�?);
    }
...
[上一章：输出多边形构建](�?4�?输出多边形构�? | [返回目录](../index) | [下一章：填充规则详解](�?6�?填充规则详解)
