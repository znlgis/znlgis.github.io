...
该模式即形态学闭运算——先膨胀后收缩，用于弥合窄缝、填充凹陷；反向组合（先收缩后膨胀）为开运算，用于剔除细小突起与碎图斑（详见第17章 17.12 节与第19章 19.4.3 节）。
...
形态学的开/闭运算与梯度可由 `ClipperOffset` 与布尔运算组合实现。以下案例均约定：整数坐标、缩放因子 `1e6`、外轮廓为 CCW（正面积）方向、结构元素用 `jtRound` 圆盘、偏移经 `Execute(ref solution, delta)` 完成。

### 20.7.1 案例 1：小图斑剔除（开运算）

先负偏移（腐蚀）再正偏移（膨胀），剔除小于结构元素尺度的碎图斑；再按面积阈值过滤残留碎块。

```csharp
using ClipperLib;

const double scale = 1e6;
const double r = 2.0 * scale;                  // 结构元素半径 2 米
const double minArea = 100.0 * scale * scale;  // 最小图斑面积阈值

public static Paths RemoveSmallPolygons(Paths input)
{
    // 开运算：先腐蚀（负偏移）后膨胀（正偏移）
    ClipperOffset co = new ClipperOffset();
    ...
    co.Execute(ref eroded, -r);      // ref + (solution, delta)

    co.Clear();
    co.AddPaths(eroded, JoinType.jtRound, EndType.etClosedPolygon);
    Paths opened = new Paths();
    co.Execute(ref opened, r);

    // 面积阈值过滤残留碎图斑
    Paths result = new Paths();
    foreach (Path p in opened)
        if (Math.Abs(Clipper.Area(p)) >= minArea)
            result.Add(p);
    return result;
}
```

腐蚀会直接消灭小于结构元素尺度的图斑；开运算后的残留再经 `Clipper.Area` 按阈值过滤。外轮廓需保持 CCW，否则正负偏移语义颠倒。

### 20.7.2 案例 2：窄缝弥合与凹口填充（闭运算）

先正偏移（膨胀）后负偏移（收缩），弥合相邻图斑间的窄缝并填充轮廓凹口。

```csharp
using ClipperLib;

const double scale = 1e6;
const double r = 1.0 * scale;       // 弥合间隙的半径

public static Paths CloseGaps(Paths input)
{
    ClipperOffset co = new ClipperOffset();

    co.AddPaths(input, JoinType.jtRound, EndType.etClosedPolygon);
    Paths dilated = new Paths();
    co.Execute(ref dilated, r);      // 先膨胀：弥合窄缝、填充凹口

    co.Clear();
    co.AddPaths(dilated, JoinType.jtRound, EndType.etClosedPolygon);
    Paths closed = new Paths();
    co.Execute(ref closed, -r);      // 后收缩：恢复近似原始尺寸
    return closed;
}
```

间隙小于 `2r` 的两个图斑在膨胀后会粘连成一个连通区域，收缩后仍保持连通；凹口深度小于 `2r` 时被填平。

### 20.7.3 案例 3：边界带提取（形态学梯度）

膨胀结果与腐蚀结果的差集即形态学梯度，得到沿轮廓宽约 `2r` 的边界带。
...
public static Paths BoundaryBand(Paths input)
{
    ClipperOffset co = new ClipperOffset();
...
`gradient` 为图斑的边界带（膨胀减腐蚀），可直接用于边界可视化或后续沿线缓冲区计算。腐蚀结果为空时梯度退化为纯膨胀结果，调用方需自行判断。
...
[上一章：辅助函数与工具](https://znlgis.github.io/sci/Clipper1/第19章-辅助函数与工具/) | [返回目录](https://znlgis.github.io/sci/Clipper1/)
