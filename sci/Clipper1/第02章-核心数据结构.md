---
layout: default
title: �?章：核心数据结构 - IntPoint、DoublePoint、IntRect
---

# �?章：核心数据结构 - IntPoint、DoublePoint、IntRect
...
`IntPoint` �?Clipper 中最核心的数据结构，用于表示一个二维（或三维）整数坐标点�?
...
1. **整数构造函�?*：直接赋值，最高效
2. **浮点构造函�?*：将 `double` 转换�?`cInt`，注意这�?*截断**而非四舍五入
3. **复制构造函�?*：从另一�?IntPoint 复制坐标
...
1. **只比�?X �?Y**：即使在 3D 模式下，相等性比较也只考虑 X、Y 坐标
2. **使用整数比较**：整数比较是精确的，这是选择整数坐标的主要优�?
...
1. **空值检�?*：防�?NullReferenceException
2. **类型检�?*：使�?`is` 关键字检查类�?
3. **值比�?*：与 `==` 运算符行为一�?
...
**注意**：这里的实现只是为了消除编译器警告（重写 Equals 时必须重�?GetHashCode）。实际使用中，如果需要将 IntPoint 用作字典键或放入 HashSet，应该提供更好的哈希实现�?
...
IntPoint p2 = new IntPoint(100.7, 200.9);  // 截断�?(100, 200)
...
1. **法向量计�?*：在偏移操作中计算边的单位法向量
2. **中间计算**：某些几何计算需要浮点精�?
3. **坐标转换**：作为整数和浮点坐标之间的桥�?

### 2.3.4 �?ClipperOffset 中的应用

```csharp
// ClipperOffset 内部使用 DoublePoint 存储法向�?
...
1. **空路径处�?*：跳过没有点的路�?
2. **初始�?*：使用第一个有效点初始化边�?
3. **遍历更新**：检查每个点，更新最�?最大坐�?
4. **时间复杂�?*：O(n)，n 为所有路径的总点�?
...
虽然不是结构体，�?`Path` �?`Paths` �?Clipper 中最常用的类型：
...
bool isOuter = area > 0;  // 正面�?= 逆时�?= 外轮�?
bool isHole = area < 0;   // 负面�?= 顺时�?= 孔洞
```
...
// 避免在循环中频繁创建新实�?
Path optimized = new Path(1000);  // 预分配容�?
...
[上一章：概述与入门](�?1�?概述与入�? | [返回目录](../index) | [下一章：路径与多边形表示](�?3�?路径与多边形表示)
