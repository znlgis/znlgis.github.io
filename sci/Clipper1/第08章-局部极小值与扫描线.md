...
**局部极小值（Local Minimum�?*：多边形轮廓上的一个顶点，�?Y 坐标小于相邻两个顶点�?Y 坐标（在 Y 轴向上的坐标系中，这是局部最低点）�?
...
局部极小�?LM
    ├── LeftBound: e1 �?e2 �?e3 (NextInLML �?
    └── RightBound: e4 �?e5 �?e6 (NextInLML �?
...
这样扫描�?Y=0 开始向上时�?
可以按顺序遇�?LM(Y=0) �?LM(Y=50) �?LM(Y=100)
```
...
2. **边界处理**�?
   - ProcessBound 构建 NextInLML �?
   - 正确处理水平�?
   - 确定左右边界

3. **扫描线管�?*�?
   - InsertScanbeam 维护有序扫描线列�?
   - PopScanbeam 获取下一个扫描位�?
   - 扫描线来自多种事�?
...
理解这些机制是理�?Clipper 裁剪算法的关键基础�?

---

[上一章：TEdge边缘结构](�?7�?TEdge边缘结构) | [返回目录](../index) | [下一章：Clipper类结构](�?9�?Clipper类结�?
