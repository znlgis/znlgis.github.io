...
- `MinkowskiSum` 存在重载 `MinkowskiSum(Path pattern, Paths paths, bool pathIsClosed)`，将同一个 pattern 与 `paths` 中的每一条 path 逐一求和（内部取并集）；上面的单 path 版本只是它的特例。
- `MinkowskiDiff` 只有两参 `MinkowskiDiff(Path poly1, Path poly2)`，**没有** `isClosed` 参数（与 `MinkowskiSum` 不同），内部固定按闭合路径处理（`Minkowski(poly1, poly2, false, true)`）。使用时务必区分这两个签名。
...
二值形态学的腐蚀运算满足恒等式 `A ⊖ B = (Aᶜ ⊕ Bʳ)ᶜ`：先取 `A` 的补集，用结构元素 `B` 关于原点的反射 `Bʳ` 做闵可夫斯基膨胀，再对结果取补集。借助这一恒等式，可以用 `MinkowskiSum` 加布尔差实现**任意形状结构元素**的腐蚀，而不必受限于 `ClipperOffset` 的圆盘/方形偏移。
...
1. **补集依赖包围盒**：补集只能在有限的包围盒内求，否则结果无界。`bounds` 在 `shape` 包围盒的基础上向外扩一个结构元素的 `w × h` 尺寸，保证膨胀后仍落在盒子内，取补集才是有限多边形。
2. **结构元素以原点为中心**：结构元素 `se` 应描述为以原点 `(0,0)` 为中心的形状。若 `B` 关于原点对称（圆盘、正方形、线段等），则 `Bʳ = B`，可省略反射步骤；反射实现即对每个顶点取 `(-X, -Y)`。
3. **腐蚀结果可能为空**：当结构元素大于被腐蚀形状时，`MorphErode` 返回空集合。调用方需先判断 `result.Count == 0` 再使用。
4. **GetBounds 只有 Paths 重载**：`Clipper.GetBounds` 只接受 `Paths`（多路径）参数，单个 `Path` 需包一层 `new Paths { path }`；返回 `IntRect { left, top, right, bottom }`（`cInt` 即 `long`）。
5. **与偏移的对比**：圆盘结构元素的腐蚀等价于 `ClipperOffset` 的负偏移（`jtRound` + 负 delta），但补集法可处理任意形状的结构元素。
