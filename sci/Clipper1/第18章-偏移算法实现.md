...
本章将深入分�?ClipperOffset 的核心算法实现，包括法向量计算、各种连接类型的处理以及圆弧近似等技术细节�?
...
这是两个法向量的叉积，等�?sin(θ)，其�?θ 是两条边的夹角�?
...
```csharp
// 简单地在末端添加两个点
pt1 = srcPoly[m] + normal[m] * delta;  // 一�?
pt2 = srcPoly[m] - normal[m] * delta;  // 另一�?
```
...
本章详细分析�?ClipperOffset 的核心算法：

1. **法向量计�?*�?
   - GetUnitNormal 计算边的单位法向�?
   - 法向量用于确定偏移方�?
...
[上一章：ClipperOffset详解](�?7�?ClipperOffset详解) | [返回目录](../index) | [下一章：辅助函数与工具](�?9�?辅助函数与工�?
