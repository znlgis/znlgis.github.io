...
- **`inputs`**：定义了两个运行时变量 `username` 和 `password`。运行前 CLI 会提示你输入它们的值。注意 `mask: true` 只在 GUI 模式下才做输入遮罩；在 CLI 模式下输入**不会**被隐藏，只会在提示中附带一句「请使用 GUI 模式输入密码」的警告。
...
由于 YAML 定义了 `inputs`，CLI 会先在命令行逐个提示你输入变量的值：

```
用户名 (your_name): 
密码 [注意: CLI 模式下输入不会隐藏，请使用 GUI 模式输入密码]: 
```

输入完成后，框架开始执行：自动定位已打开的浏览器窗口（激活并最大化）、导航到登录页、等待用户名框出现、输入用户名与密码、点击登录、等待欢迎页。整个过程你只需要「旁观」。
...
如果执行过程中发现问题，想中途停下来，可以直接在命令行按 `Ctrl+C` 终止进程，或在 WPF 界面中点击停止。robotgo-flow 基于 Go 的 `context` 取消机制实现了协作式安全停止：执行器在每个步骤与动作边界检查取消信号，收到后干净退出，不会留下「僵尸」操作。需要注意这套机制目前接在 `serve`/GUI 的 `stop` 命令上；CLI 下按 `Ctrl+C` 属于操作系统直接终止进程，并非动作边界的优雅停止。
