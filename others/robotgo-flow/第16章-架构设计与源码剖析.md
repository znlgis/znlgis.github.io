...
| `internal/protocol` | serve 与 DLL（ffi）共用的 JSON-Line 协议消息类型（单一来源） |
| `internal/logger` | 分级日志（Debug/Info/Warn/Error），serve 模式下自动改道 stderr |
| `internal/notify` | 交互输入抽象（`Interactor`：stdin / 非交互宿主）、错误块输出、结构化错误日志 |
...
main.go (cmdRun)
  ...
  │     ├─ （ctx 默认 Background；serve/DLL 场景才 WithContext 注入取消）
  │     └─ RunFromStep(from)
  │           └─ runSteps()
  │                 for step in steps:
...
`Engine` 接口声明了动作可以使用的全部 RPA 原语（共 20 个方法），分组包括：
...
- **`parseTargetedAction[T]`**（泛型）：解析 click/double_click/right_click，兼容「字符串（模板）」「`{template: 路径}` 映射」「`{x, y}` 坐标映射」三种写法；
...
- **模板缓存**：用带 `sync.RWMutex` 的 map 缓存位图，避免重复解码；另有模板尺寸缓存（`templateSize`）免去每次读文件取宽高；并发未命中时以双重检查防止位图重复，关闭时统一释放；
