...
`build.ps1` 的核心流程大致分为四步：

1. **环境检查**：显式设置 `$env:CGO_ENABLED = "1"`（robotgo/bitmap 依赖 CGo，若宿主环境恰好为 0 会报「build constraints exclude all Go files」类错误）；解析 `go version` 输出并验证 ≥ 1.26；确认 GCC 存在且 `gcc -dumpmachine` 为 x86_64 目标；执行 `go mod verify`；
...
4. **构建 C# 解决方案**：Go 构建完成后，脚本还会自动执行 `dotnet build RobotgoFlow.Wpf.sln -c Release`——因此完整运行 `build.ps1` 需要安装 .NET 10 SDK（2.6 节的 `-Dll` 构建同样如此）。
