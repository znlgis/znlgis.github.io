...
�?  ├── RuoYiAuthApplication.java    # 启动�?
�?  ├── controller/
�?  �?  └── TokenController.java     # 认证控制�?
�?  ├── form/
�?  �?  ├── LoginBody.java           # 登录请求�?
...
public class SysLoginService {
    ...
    public LoginUser login(String username, String password) {
        ...
        if (username.length() < UserConstants.USERNAME_MIN_LENGTH
            ...
            recordLogService.recordLogininfor(username, Constants.LOGIN_FAIL, "用户名不在指定范�?);
            throw new ServiceException("用户名不在指定范�?);
        }
        
        // IP黑名单校�?
        String blackStr = Convert.toStr(redisService.getCacheObject(CacheConstants.SYS_LOGIN_BLACKIPLIST));
        if (IpUtils.isMatchedIp(blackStr, IpUtils.getIpAddr())) {
            recordLogService.recordLogininfor(username, Constants.LOGIN_FAIL, "很抱歉，访问IP已被列入系统黑名�?);
            throw new ServiceException("很抱歉，访问IP已被列入系统黑名�?);
        }
        ...
        if (userResult.getData() == null) {
            recordLogService.recordLogininfor(username, Constants.LOGIN_FAIL, "登录用户不存�?);
            throw new ServiceException("登录用户�? + username + " 不存�?);
        }
        ...
        if (UserStatus.DISABLE.getCode().equals(user.getStatus())) {
            ...
            throw new ServiceException("对不起，您的账号�? + username + " 已停�?);
        }
    ...
    public void logout(String loginName) {
        recordLogService.recordLogininfor(loginName, Constants.LOGOUT, "退出成�?);
    }
    ...
    public void register(String username, String password) {
        ...
        if (username.length() < UserConstants.USERNAME_MIN_LENGTH
                || username.length() > UserConstants.USERNAME_MAX_LENGTH) {
            throw new ServiceException("账户长度必须�?�?0个字符之�?);
        }
        
        if (password.length() < UserConstants.PASSWORD_MIN_LENGTH
                || password.length() > UserConstants.PASSWORD_MAX_LENGTH) {
            throw new ServiceException("密码长度必须�?�?0个字符之�?);
        }
    ...
    public void validate(SysUser user, String password) {
        ...
        if (!SecurityUtils.matchesPassword(password, user.getPassword())) {
            ...
            recordLogService.recordLogininfor(username, Constants.LOGIN_FAIL, 
                StringUtils.format("密码输入错误{}�?, retryCount));
            redisService.setCacheObject(getCacheKey(username), retryCount, lockTime, TimeUnit.MINUTES);
            throw new ServiceException("用户不存�?密码错误");
        } else {
...
public class ValidateCodeServiceImpl implements ValidateCodeService {
    ...
    public void checkCaptcha(String code, String uuid) throws CaptchaException {
        if (StringUtils.isEmpty(code)) {
            throw new CaptchaException("验证码不能为�?);
        }
...
1. **认证中心职责**：Token管理、用户认证、安全机�?
2. **登录流程**：完整的登录认证流程
3. **密码安全**：BCrypt加密、密码验证、登录限�?
4. **Token管理**：创建、存储、验证、刷�?
5. **验证�?*：生成、校验机�?
6. **日志记录**：登录日志记�?
7. **权限模型**：RBAC权限模型
...
[上一章：微服务模块详解](./�?4�?微服务模块详�?md) | [返回目录](./index) | [下一章：网关服务详解](./�?6�?网关服务详解)
