...
RuoYi-Cloud需要在Nacos中导入配置文件，配置文件位于项目�?`sql/ry_config_*.sql` 中�?
...
2. **启动各服�?*
   ...
   com.ruoyi.gen.RuoYiGenApplication (可�?
   com.ruoyi.job.RuoYiJobApplication (可�?
   com.ruoyi.file.RuoYiFileApplication (可�?
   com.ruoyi.modules.monitor.RuoYiMonitorApplication (可�?
   ```
...
3. **数据库安�?*：MySQL和Redis的安装配�?
4. **Nacos安装**：注册中心和配置中心的部�?
5. **项目导入**：源码获取和IDEA导入
6. **数据库初始化**：SQL脚本执行
7. **服务启动**：后端和前端的启动方�?
8. **常见问题**：常见错误的解决方案
...
[上一章：系统架构详解](./�?2�?系统架构详解) | [返回目录](./index) | [下一章：微服务模块详解](./�?4�?微服务模块详�?md)
