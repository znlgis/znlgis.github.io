...
CREATE TABLE `sys_test` (
    ...
    `test_type` char(1) DEFAULT '0' COMMENT '测试类型�?类型1 1类型2�?,
...
gen:
  # 作�?
  author: ruoyi
  # 默认生成包路�?system 需改成自己的模块名�?�?system monitor tool
...
|------|------|------|
| input | 文本�?| el-input |
| textarea | 文本�?| el-input type="textarea" |
| select | 下拉�?| el-select |
...
CREATE TABLE `biz_order` (
    ...
    `order_status` char(1) DEFAULT '0' COMMENT '订单状�?,
...
1. **生成器功�?*：支持单表、主子表、树表三种模�?
2. **使用步骤**：创建表、导入、配置、生�?
3. **配置详解**：字段类型映射、显示类型、查询方�?
...
[上一章：系统管理模块](./�?7�?系统管理模块) | [返回目录](./index) | [下一章：定时任务模块](./�?9�?定时任务模块)
