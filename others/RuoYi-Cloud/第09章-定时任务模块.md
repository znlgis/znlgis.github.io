...
�?  ├── RuoYiJobApplication.java      # 启动�?
�?  ├── config/
�?  �?  └── ScheduleConfig.java       # Quartz配置
�?  ├── controller/
�?  �?  ├── SysJobController.java     # 任务管理控制�?
�?  �?  └── SysJobLogController.java  # 任务日志控制�?
�?  ├── domain/
�?  �?  ├── SysJob.java               # 定时任务实体
...
CREATE TABLE QRTZ_TRIGGERS (
    ...
    job_group varchar(200) NOT NULL COMMENT '任务组名',
    description varchar(250) DEFAULT NULL COMMENT '相关介绍',
    next_fire_time bigint(13) DEFAULT NULL COMMENT '上一次触发时�?,
    prev_fire_time bigint(13) DEFAULT NULL COMMENT '下一次触发时�?,
    priority int(11) DEFAULT NULL COMMENT '优先�?,
    trigger_state varchar(16) NOT NULL COMMENT '触发器状�?,
    trigger_type varchar(8) NOT NULL COMMENT '触发器的类型',
    start_time bigint(13) NOT NULL COMMENT '开始时�?,
    end_time bigint(13) DEFAULT NULL COMMENT '结束时间',
    calendar_name varchar(200) DEFAULT NULL COMMENT '日程表名�?,
    misfire_instr smallint(2) DEFAULT NULL COMMENT '补偿执行的策�?,
    job_data blob DEFAULT NULL COMMENT '存放持久化job对象',
...
public class SysJob extends BaseEntity {
    ...
    @Size(min = 0, max = 64, message = "任务名称不能超过64个字�?)
    ...
    /** 调用目标字符�?*/
    @Excel(name = "调用目标字符�?)
    @NotBlank(message = "调用目标字符串不能为�?)
    @Size(min = 0, max = 500, message = "调用目标字符串长度不能超�?00个字�?)
    private String invokeTarget;
    
    /** cron执行表达�?*/
    @Excel(name = "执行表达�?)
    @NotBlank(message = "Cron执行表达式不能为�?)
    @Size(min = 0, max = 255, message = "Cron执行表达式不能超�?55个字�?)
    private String cronExpression;
    
    /** cron计划策略 0=默认 1=立即触发执行 2=触发一次执�?3=不触发立即执�?*/
    @Excel(name = "计划策略", readConverterExp = "0=默认,1=立即触发执行,2=触发一次执�?3=不触发立即执�?)
    ...
    /** 任务状态（0正常 1暂停�?*/
    @Excel(name = "任务状�?, readConverterExp = "0=正常,1=暂停")
...
public class SysJobLog extends BaseEntity {
    ...
    /** 执行状态（0正常 1失败�?*/
    @Excel(name = "执行状�?, readConverterExp = "0=正常,1=失败")
    private String status;
...
public class ScheduleUtils {
    ...
    public static void run(Scheduler scheduler, SysJob job) throws Exception {
        ...
        if (scheduler.checkExists(jobKey)) {
            ...
            throw new TaskException("任务不存在或已删�?, Code.NOT_FOUND);
        }
    }
...
[上一章：代码生成器使用](./�?8�?代码生成器使�?md) | [返回目录](./index) | [下一章：文件服务与存储](./�?0�?文件服务与存�?md)
