...
| 模块名称 | 端口 | 职责说明 |
|---------|------|---------|
| ruoyi-gateway | 8080 | 统一入口，路由转发，鉴权过滤，限流控�?|
| ruoyi-auth | 9200 | 用户认证，Token生成与验�?|
...
public class PreAuthorizeAspect {
    ...
    public void doBeforeLogin(RequiresLogin requiresLogin) {
        ...
        if (loginUser == null) {
            throw new NotLoginException("用户未登�?);
        }
    }
...
2. **通用模块**：掌握core、security、log、redis等核心模�?
3. **API模块**：理解远程服务接口定义和降级处理
4. **服务通信**：掌握OpenFeign调用和请求头传递机�?

这些模块构成了RuoYi-Cloud的基础架构，理解它们对于后续的二次开发至关重要�?

---

[上一章：环境准备与安装部署](./�?3�?环境准备与安装部�?md) | [返回目录](./index) | [下一章：认证与授权中心](./�?5�?认证与授权中�?md)
