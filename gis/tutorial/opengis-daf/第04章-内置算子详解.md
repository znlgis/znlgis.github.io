...
OpenGIS DAF 内置了 9 个算子，按注册分类（`Category`）分为五类：**空间运算**、**空间关系**、**属性操作**、**格式转换**、**质检规则**。本章逐一详解每个算子的用途、参数、输入输出约定与典型用法。
...
| 格式转换 | `coordinate_transform` | 坐标系转换 | `source_epsg`/`target_epsg` |
...
- OpenGIS DAF 内置 9 个算子，按注册分类分属空间运算、空间关系、属性操作、格式转换、质检规则五类；
- 空间运算算子（`buffer`/`clip`）用于几何变换，注意 `clip` 的逐面求交语义与缓冲距离单位；
- 空间关系算子（`intersect_check`/`containment_check`）常用于质检，支持集合间与集合内两种模式；
- 属性操作算子（`field_calculator`/`null_value_filler`）处理字段，格式转换算子（`coordinate_transform`）处理坐标系；
