...
public class SpatialQueryService {
    ...
    private void initCache() {
        ...
        for (OguFeature feature : layer.getFeatures()) {
            if (feature.getGeometry() != null) {
                ...
                geometryCache.put(feature.getId(), geom);
            }
        }
    }
    ...
    public List<OguFeature> queryByPoint(double x, double y) {
        ...
        return layer.getFeatures().stream()
            .filter(f -> {
                Geometry geom = geometryCache.get(f.getId());
    ...
    public List<OguFeature> queryByBuffer(String wkt, double distance) {
        ...
        return layer.getFeatures().stream()
            .filter(f -> {
                Geometry fGeom = geometryCache.get(f.getId());
    ...
    public List<OguFeature> queryNearest(double x, double y, int count) {
        ...
        // 计算距离并排序
        return layer.getFeatures().stream()
            .filter(f -> geometryCache.containsKey(f.getId()))
            .sorted((f1, f2) -> {
                Geometry g1 = geometryCache.get(f1.getId());
                Geometry g2 = geometryCache.get(f2.getId());
...
public class LandDataProcessor {
    ...
    public ProcessResult processTxtFile(String txtPath, String outputShpPath) {
        ...
        try {
            ...
            // 4. 移除无效要素
            layer.getFeatures().removeIf(f -> f.getGeometry() == null);
    ...
    private void processFeature(OguFeature feature, Integer wkid, ProcessResult result) {
        if (feature.getGeometry() == null || feature.getGeometry().isEmpty()) {
            ...
            if (feature.getCoordinates() != null && !feature.getCoordinates().isEmpty()) {
                feature.setGeometry(buildPolygonFromCoordinates(feature.getCoordinates()));
            } else {
                result.setFailedCount(result.getFailedCount() + 1);
                return;
            }
        ...
        if (!validResult.isValid()) {
            ...
            try {
                ...
                feature.setGeometry(GeometryUtil.geometry2Wkt(geom));
    ...
    public OguLayer merge(List<OguLayer> layers, MergeConfig config) {
        ...
        for (OguLayer layer : layers) {
            ...
            for (OguFeature sourceFeature : layer.getFeatures()) {
                // 去重检查
                if (wktSet != null && sourceFeature.getGeometry() != null) {
                    if (wktSet.contains(sourceFeature.getGeometry())) {
                        continue;  // 跳过重复
                    }
                    wktSet.add(sourceFeature.getGeometry());
                }
                
                // 几何验证
                if (config.isValidateGeometry() && sourceFeature.getGeometry() != null) {
                    Geometry geom = GeometryUtil.wkt2Geometry(sourceFeature.getGeometry());
                    TopologyValidationResult valid = GeometryUtil.isValid(geom);
                    if (!valid.isValid()) {
                        try {
                            geom = GeometryUtil.validate(geom);
                            sourceFeature.setGeometry(GeometryUtil.geometry2Wkt(geom));
                        } catch (Exception e) {
                            continue;  // 跳过无效几何
                        }
                    }
                ...
                newFeature.setId(String.valueOf(fid++));
                newFeature.setGeometry(sourceFeature.getGeometry());
...
public class DataQualityChecker {
    ...
    private void checkGeometry(OguFeature feature, Integer wkid, QualityReport report) {
        ...
        if (wkt == null || wkt.isEmpty()) {
            ...
            report.getIssues().add(new QualityIssue(
                feature.getId(), "EMPTY_GEOMETRY", "几何为空"));
            return;
        }
        ...
        try {
            ...
            // 坐标范围检查
            if (wkid != null) {
                checkCoordinateRange(feature.getId(), geom, wkid, report);
            }
            
        } catch (Exception e) {
            report.setInvalidGeometries(report.getInvalidGeometries() + 1);
            report.getIssues().add(new QualityIssue(
                feature.getId(), "PARSE_ERROR", "WKT解析失败: " + e.getMessage()));
        }
    }
    ...
    private void checkCoordinateRange(String fid, Geometry geom, int wkid, 
        ...
        boolean isProjected = CrsUtil.isProjectedCRS(CrsUtil.getSupportedCRS(wkid).getValue());
