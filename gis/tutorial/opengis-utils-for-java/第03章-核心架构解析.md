...
public class GtTxtUtil {
     ...
     * @param fields 字段定义（可为null，为null时按TXT内容推断）
     * @return 图层对象
     */
     ...
     * @param fieldNames 需要导出的字段名（可为null）
     * @param zoneNumber 带号（为null时按首要素推导）
     */
...
public final class GisEngineFactory {
    ...
    /**
     * 根据引擎类型获取GIS引擎
     * engineType为null或AUTO时自动选择（GDAL优先，否则GeoTools）
     * @param engineType 引擎类型
     * @return 引擎实例
     */
    public static GisEngine getEngine(GisEngineType engineType) throws EngineNotSupportedException {
        // GDAL不可用而显式请求GDAL时抛EngineNotSupportedException
        ...
    }
    
    /**
     * 根据数据格式获取合适的引擎（FILEGDB必须GDAL，其余GDAL可用则优先GDAL）
     */
    ...
    /**
     * 直接获取GDAL引擎（不可用时抛EngineNotSupportedException）
     */
...
public class GeoToolsEngine implements GisEngine {
    ...
    private static final Set<DataFormatType> SUPPORTED_FORMATS = EnumSet.of(
            ...
            DataFormatType.POSTGIS, DataFormatType.TXT);
    ...
    public LayerReader getReader(DataFormatType formatType) throws OguException {
        // 不支持的格式抛EngineNotSupportedException
...
public class GdalEngine implements GisEngine {
    ...
    private static final Set<DataFormatType> SUPPORTED_FORMATS = EnumSet.of(
            DataFormatType.SHP, DataFormatType.GEOJSON,
            DataFormatType.FILEGDB, DataFormatType.POSTGIS);
    
    /**
     * 可用性取决于GDAL本地库（gdalalljni）是否加载成功
     * @see OgrUtil#getOgrInitSuccess()
     */
    @Override
    public boolean isAvailable() {
    ...
    @Override
    public boolean supports(DataFormatType formatType) {
        return isAvailable() && SUPPORTED_FORMATS.contains(formatType);
    }
    ...
    // getName()/getWriter()/readLayer()/writeLayer() 其余实现省略
}
```
...
│  - id: String                                                   │
│  - geometry: String (WKT)                                       │
│  - attributes: List<OguFieldValue>                              │
│  - coordinates: List<OguCoordinate>                             │
...
```java
/**
 * OGU4J基础异常（受检异常，读写API均声明throws OguException）
 */
...
/**
 * 图层验证异常 - 数据完整性错误
 * 注意：直接继承RuntimeException，不是OguException的子类
 */
...
- **GeometryUtil**：静态方法基本无状态；注意`validate(Geometry)`对合法几何会执行`normalize()`并返回同一实例（存在副作用），多线程共享同一几何对象时需自行规避
- **CrsUtil**：静态CRS缓存使用`ConcurrentHashMap`（并配合`computeIfAbsent`），线程安全
- **GisEngineFactory**：GeoTools/GDAL引擎各持有一个`static final`单例实例，无缓存Map，线程安全
