...
- 将球面坐标投影到平面
- 单位：米（meter）
- 例如：CGCS2000 / 3度分带高斯-克吕格投影（WKID: 4513-4554，OGU4J支持范围为EPSG:4490-4554）
...
| ... | ... | ... |
| CGCS2000 3度带 45N | 4533 | 投影，中央经线135° |

**3度分带对照表（常用）**

> OGU4J中带号与投影WKID的换算为 `WKID = 4488 + 带号`（见`CrsUtil.getProjectedWkid`/`getDh`）。
...
public class CommonWkids {
    ...
    public static final int CGCS2000_39N = 4527;    // CGCS2000 39带
    public static final int CGCS2000_40N = 4528;    // CGCS2000 40带
}
```
 ...
 * 默认支持范围：EPSG:4490-4554（CGCS2000地理坐标系及其3度分带投影）
 */
public class CrsUtil {
    
    // 坐标转换
    public static String transform(String wkt, Integer sourceWkid, Integer targetWkid);
    public static Geometry transform(Geometry geom, Integer sourceWkid, Integer targetWkid);
    public static Geometry transform(Geometry geom, CoordinateReferenceSystem sourceCRS, CoordinateReferenceSystem targetCRS);
    ...
    public static FeatureCollection<SimpleFeatureType, SimpleFeature> reproject(
            FeatureCollection<SimpleFeatureType, SimpleFeature> featureCollection, Integer targetWkid);
    ...
    // CRS相关（getSupportedCRS返回Map.Entry，取其getValue()即为CRS对象）
...
int targetWkid = 4527;  // CGCS2000 39带
...
// 转换
Geometry geomProj = CrsUtil.transform(geomGeo, 4490, 4527);
...
// 投影转换
OguLayer reprojected = CrsUtil.reproject(layer, 4527);
...
System.out.println("39带WKID: " + wkid);  // 4527

// 从几何获取对应的投影WKID
Geometry geom = GeometryUtil.wkt2Geometry("POINT(116.4 39.9)");
Integer projWkid = CrsUtil.getProjectedWkid(geom);
System.out.println("投影WKID: " + projWkid);  // 4527
```
...
// 获取CRS对象（返回Map.Entry，KEY为EPSG编码，VALUE为坐标系）
Map.Entry<Integer, CoordinateReferenceSystem> entryGeo = CrsUtil.getSupportedCRS(4490);
Map.Entry<Integer, CoordinateReferenceSystem> entryProj = CrsUtil.getSupportedCRS(4527);
CoordinateReferenceSystem crsGeo = entryGeo.getValue();
CoordinateReferenceSystem crsProj = entryProj.getValue();

// 获取CRS信息
System.out.println("名称: " + crsGeo.getName().getCode());
...
```java
// 判断是否为投影坐标系
CoordinateReferenceSystem crsGeo = CrsUtil.getSupportedCRS(4490).getValue();
CoordinateReferenceSystem crsProj = CrsUtil.getSupportedCRS(4527).getValue();
...
double tolGeo = CrsUtil.getTolerance(4490);   // 1.0E-9（地理坐标系）
double tolProj = CrsUtil.getTolerance(4527);  // 1.0E-4（投影坐标系）
```
...
for (Map.Entry<Integer, CoordinateReferenceSystem> entry : crsList.entrySet()) {
    System.out.printf("WKID: %d, 名称: %s%n", 
        entry.getKey(), 
        entry.getValue().getName().getCode());
}
```
...
public class SmartReproject {
    ...
    public static OguLayer smartReproject(OguLayer layer) {
        ...
        // 获取CRS
        CoordinateReferenceSystem crs = CrsUtil.getSupportedCRS(layer.getWkid()).getValue();
...
public class CrsValidator {
    ...
    public static void validateCrs(OguLayer layer) {
        ...
        try {
            CoordinateReferenceSystem crs = CrsUtil.getSupportedCRS(wkid).getValue();
            System.out.println("坐标系有效");
            System.out.println("  WKID: " + wkid);
            System.out.println("  名称: " + crs.getName().getCode());
...
> 提示：`CrsUtil`的静态坐标系缓存使用`ConcurrentHashMap`（配合`computeIfAbsent`），`transform`/`getSupportedCRS`可在并行流中安全调用；但缓存内容会随首次解析的WKID增长，因此`standardizeCRS`的“支持范围”判定与进程内调用历史有关，跨坐标系混合处理时建议先统一声明图层WKID。
...
public class AreaCalculator {
    ...
    public static void calculateLayerAreas(OguLayer layer) {
        ...
        for (OguFeature feature : layer.getFeatures()) {
            double area = calculateArea(feature.getGeometry(), wkid);
...
public class Wgs84ToCgcs2000 {
    ...
    public static OguLayer cgcs2000ToProjected(OguLayer layer) {
        ...
        int dh = CrsUtil.getDh(firstFeature.getGeometry());
        Integer targetWkid = CrsUtil.getProjectedWkid(dh);
...
```java
// 始终转换到投影坐标系后计算面积
if (!CrsUtil.isProjectedCRS(CrsUtil.getSupportedCRS(layer.getWkid()).getValue())) {
    layer = smartReproject(layer);
}

// 然后计算面积
double area = GeometryUtil.area(GeometryUtil.wkt2Geometry(feature.getGeometry()));
```
...
public class TransformCache {
    ...
    public static MathTransform getTransform(int sourceWkid, int targetWkid) {
        ...
        return transformCache.computeIfAbsent(key, k -> {
            try {
                CoordinateReferenceSystem sourceCRS = CrsUtil.getSupportedCRS(sourceWkid).getValue();
                CoordinateReferenceSystem targetCRS = CrsUtil.getSupportedCRS(targetWkid).getValue();
...
<div style="display: flex; justify-content: space-between; margin: 20px 0;">
  <a href="https://znlgis.github.io/gis/tutorial/opengis-utils-for-java/第07章-几何处理与空间分析/" style="text-decoration: none;">← 上一章</a>
  <a href="https://znlgis.github.io/gis/tutorial/opengis-utils-for-java/" style="text-decoration: none;">目录</a>
  <a href="https://znlgis.github.io/gis/tutorial/opengis-utils-for-java/第09章-异常处理体系/" style="text-decoration: none;">下一章 →</a>
</div>
