...
从 `io.read_raster` 读取后，这个字典通过 `$step.output`（或简写 `$step`）在步骤间传递；`io.read_raster` 的 `stats` 中另有 `width`、`height`、`crs`、`band_count`、`dtype`、`bounds` 等摘要信息（`nodata` 等完整元信息保存在 `profile` 内）。
...
| `resampling` | string | ❌ | `nearest` | 重采样方法：`nearest`/`bilinear`/`cubic`/`lanczos` |
...
表达式通过 `utils/safe_eval.py` 中的 AST 白名单求值：仅允许波段引用、数值运算（`+`、`-`、`*`、`/`、`//`、`%`、`**`）、比较与布尔运算，以及 `np.` 前缀的白名单数学函数（`abs`、`sqrt`、`log`、`exp`、`where`、`clip`、`minimum`、`maximum`、`nan_to_num` 等三角/对数/极值函数），其余函数调用一律拒绝，防止代码注入。
...
# 归一化（np.* 白名单函数可用）
expression: "np.clip(B1 / 255.0, 0, 1)"
```

> **注意**：表达式中只能使用 `B1`、`B2`... 形式引用波段，不支持 `band1`、`Band1` 等写法。`B1.min()` 之类的方法调用不在白名单内；只允许基本数学运算符、括号与 `np.*` 白名单函数。
...
|------|------|------|--------|------|
| `input` | raster_info | ✅ | — | 输入栅格数据 |
| `band` | number | ❌ | `1` | 统计的波段编号（1-indexed） |

### 输出

| 属性 | 类型 | 说明 |
|------|------|------|
| `output` | dict | 统计结果字典（与 stats 内容一致） |
| `stats.min` | float | 最小值（忽略 NaN） |
| `stats.max` | float | 最大值 |
| `stats.mean` | float | 平均值 |
| `stats.std` | float | 标准差 |
| `stats.band` | int | 实际统计的波段编号 |
| `stats.pixel_count` | int | 参与统计的像素数量 |
...
| `band` | number | ❌ | `1` | 使用的波段编号（1-indexed） |
...
| 属性 | 类型 | 说明 |
|------|------|------|
| `output` | GeoDataFrame | 等值线矢量数据（LineString），含 `elevation` 字段表示等值 |
| `stats.contour_count` | int | 生成的等值线数量 |
| `stats.interval` | number | 实际使用的等值线间隔 |
| `stats.min_elevation` | float | 最小等值 |
| `stats.max_elevation` | float | 最大等值 |
...
5. **`raster.contour`**：从栅格提取等值线为矢量数据，含 `elevation` 属性字段

---

**导航**：[← 第八章：矢量分析步骤](https://znlgis.github.io/gis/tutorial/GeoPipeAgent/第08章-矢量分析步骤/) ｜ [第十章：空间分析步骤 →](https://znlgis.github.io/gis/tutorial/GeoPipeAgent/第10章-空间分析步骤/)
