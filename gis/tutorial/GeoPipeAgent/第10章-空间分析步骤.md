...
空间分析步骤（`analysis.*`）提供高级地理空间分析功能，共 4 个步骤。其中 `analysis.voronoi` 仅依赖核心依赖 shapely；其余 3 个步骤需要安装 `[analysis]` 可选依赖（scipy、scikit-learn、matplotlib）。
...
| 步骤 ID | 名称 | 功能 | 依赖 |
|---------|------|------|------|
| `analysis.voronoi` | 泰森多边形 | 点→泰森多边形 | shapely（核心依赖，STRtree + voronoi） |
...
根据输入点数据生成泰森多边形（Voronoi 图）。实现基于 shapely 的 `voronoi_diagram` + `STRtree`（核心依赖，无需 `[analysis]` 附加包）：先将无界胞元裁剪到包络范围，再把每个输入点匹配到覆盖它的胞元，输出与输入点一一对应（重合点共享同一胞元）。每个多边形包含距离该点比其他点更近的所有位置。
...
| `envelope` | geodataframe | ❌ | — | 裁剪边界；不指定则使用点数据外接矩形向四周各扩展 10% |
