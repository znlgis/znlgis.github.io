...
从指定中心点出发，计算在给定代价限制内能到达的所有节点，并构建服务区多边形（等时圈或等距圈）。
...
| `center` | list | ✅ | — | 服务中心点坐标 `[x, y]` |
| `cost_limit` | number | ✅ | — | 最大代价（距离/时间上限） |
...
| 属性 | 类型 | 说明 |
|------|------|------|
| `output` | GeoDataFrame | 服务区面数据（两端点均可达的路段合并后按 `cost_limit × 0.05` 缓冲成面；无路段可达时以中心点为圆心按 `cost_limit × 0.01` 缓冲），含 `reachable_nodes`、`reachable_edges`、`cost_limit` 属性字段 |
| `stats.reachable_nodes` | int | 可达节点数量 |
| `stats.reachable_edges` | int | 服务区内的路段数量 |
| `stats.cost_limit` | float | 使用的最大代价 |
...
pipeline:
  ...
  steps:
    ...
    - id: service-area
      ...
      params:
        input: "$reproject"
        center: [12953000, 4836000]      # 医院坐标（EPSG:3857）
        cost_limit: 1200                 # 1200 米
...
steps:
  ...
  - id: filter-passable
    ...
    params:
      input: "$load-roads"
      expression: "road_class not in ['footway', 'cycleway']"
...
2. **`network.service_area`**：从中心点出发的可达服务区，基于代价限制（距离/时间），结果为可达路段缓冲合并而成的面
3. **`network.geocode`**：批量地址→坐标转换，使用 Nominatim，注意使用频率限制，建议配 `on_error: retry`

---

**导航**：[← 第十章：空间分析步骤](https://znlgis.github.io/gis/tutorial/GeoPipeAgent/第10章-空间分析步骤/) ｜ [第十二章：数据质检步骤 →](https://znlgis.github.io/gis/tutorial/GeoPipeAgent/第12章-数据质检步骤/)
