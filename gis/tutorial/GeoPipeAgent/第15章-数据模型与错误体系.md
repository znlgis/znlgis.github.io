...
class StepResult:
    ...
    metadata: dict = {}             # 元数据（QC 步骤在此存放 issues_gdf）
...
def summary(self) -> dict:
    ...
    if self.issues:
        ...
        summary["issues_by_severity"] = dict(Counter(i.severity for i in self.issues))
    return summary
```
...
class StepExecutionError(GeopipeAgentError):
    def __init__(self, step_id: str, message: str, cause: Exception | None = None,
                 suggestion: str | None = None, report: dict | None = None): ...

    step_id: str        # 失败的步骤 ID
    suggestion: str     # AI 友好的修复建议（可能为 None）
    cause: Exception    # 原始异常
    report: dict        # 失败时已生成的步骤报告片段（可能为 None）
...
> `StepExecutionError` 是普通异常子类（不是 `dataclass`），`message` 由 `Exception` 承载；`suggestion` / `cause` / `report` 为空时不会出现在 `to_dict()` 结果中。
...
4. **错误层次**：`GeopipeAgentError` 基类 → 6 个子类，各有特定触发场景
5. **`StepExecutionError`**：含 `step_id` 和 `suggestion` 字段，AI 友好

---

**导航**：[← 第十四章：多后端系统](https://znlgis.github.io/gis/tutorial/GeoPipeAgent/第14章-多后端系统/) ｜ [第十六章：CLI 命令行工具 →](https://znlgis.github.io/gis/tutorial/GeoPipeAgent/第16章-CLI命令行工具/)
