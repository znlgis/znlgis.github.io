...
|------|------|------|--------|------|
| `input` | geodataframe | ✅ | — | 输入矢量数据 |
| `rules` | list | ✅ | — | 拓扑规则列表（`no_overlaps`/`no_gaps`/`no_dangles`） |
...
| `allow_empty` | boolean | ❌ | `false` | 是否允许空字符串值（false 时空字符串也算缺失） |
| `severity` | string | ❌ | `warning` | 问题严重级别 |

检测：字段不存在、字段值为 NULL/空字符串。
...
| `tolerance` | number | ❌ | `0.0` | 几何重复判定的简化容差（>0 时先按该容差简化再比较） |
| `severity` | string | ❌ | `warning` | 问题严重级别 |
...
| `min` | number | ❌ | — | 最小像素值 |
| `max` | number | ❌ | — | 最大像素值 |
| `band` | integer | ❌ | `0` | 检查的波段（0 起始索引，默认第 1 波段） |
| `severity` | string | ❌ | `warning` | 问题严重级别 |
...
**导航**：[← 第十一章：网络分析步骤](https://znlgis.github.io/gis/tutorial/GeoPipeAgent/第11章-网络分析步骤/) ｜ [第十三章：流水线引擎 →](https://znlgis.github.io/gis/tutorial/GeoPipeAgent/第13章-流水线引擎/)
