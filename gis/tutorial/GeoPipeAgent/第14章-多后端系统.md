...
**工作原理**：直接调用 `gdf.geometry.buffer(distance, cap_style=...)`、`gpd.clip(input_gdf, clip_gdf)` 等 GeoPandas/Shapely 方法，无需文件 I/O，速度快。
...
最灵活的后端，允许执行命令行命令，通过标准输出或文件参数传递数据。
...
| 安全策略 | 默认 `allow_shell=False`，`execute(..., shell=True)` 抛 `PermissionError`；优先传入参数列表。超时上限由 `max_timeout`（默认 300 秒）约束，`allowed_cwd` 可限定工作目录 |
...
| 安全策略 | 仅允许 `http`/`https`；未配置 `allowed_hosts` 时默认拒绝回环、内网、链路本地、保留与组播地址；响应体上限 `max_response_bytes`（默认 10 MiB）、请求超时上限 `max_timeout`（默认 30 秒）；`insecure=True` 需显式构造 `CurlApiBackend(allow_insecure=True)` |
...
若步骤声明了 `backends`，`StepRunner` 会调用 `BackendManager.get(step_def.backend)`：不指定 `backend` 时返回第一个可用后端，指定了名字但本机不可用时抛 `BackendNotAvailableError`。注意该列表目前**只作为元数据**（供 `list-steps`/`describe`/Skill 文档展示），执行时并不会把请求的后端与它做比对，因此声明外的可用后端同样会被取用。若步骤没有 `backends`（如 IO 步骤），则不使用后端（`ctx.backend` 为 `None`，直接调用 GeoPandas/rasterio）。
...
**导航**：[← 第十三章：流水线引擎](https://znlgis.github.io/gis/tutorial/GeoPipeAgent/第13章-流水线引擎/) ｜ [第十五章：数据模型与错误体系 →](https://znlgis.github.io/gis/tutorial/GeoPipeAgent/第15章-数据模型与错误体系/)
