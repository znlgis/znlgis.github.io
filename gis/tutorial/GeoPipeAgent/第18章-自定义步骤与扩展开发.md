...
```python
# src/geopipe_agent/steps/__init__.py
...
def _iter_step_modules():
    ...
    for _importer, modname, ispkg in pkgutil.walk_packages([package_dir], prefix=prefix):
        if ispkg:                                  # 跳过子包的 __init__
            continue
        short = modname[len(prefix):]
        if short in _SKIP_MODULES:                 # 跳过 registry
            continue
        if short.rsplit(".", 1)[-1].startswith("_"):  # 跳过 _helpers/_graph/_grid/_delegate
            continue
        yield modname
...
`import geopipe_agent` 时，所有 `steps/` 子目录下非下划线开头的模块都被自动导入，`@step` 装饰器在模块导入时即注册步骤。另有 `reload_builtin_steps()` 供测试在 `registry.reset()` 后强制重载。
...
**导航**：[← 第十七章：Skill 文件与 AI 集成](https://znlgis.github.io/gis/tutorial/GeoPipeAgent/第17章-Skill文件与AI集成/) ｜ [第十九章：Cookbook 示例精讲 →](https://znlgis.github.io/gis/tutorial/GeoPipeAgent/第19章-Cookbook示例精讲/)
