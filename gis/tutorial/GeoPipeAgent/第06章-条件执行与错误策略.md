...
步骤失败时自动重试，最多 3 次，第 1、2 次重试前分别等待 0.5s、1s（`RetryPolicy.delay_before(attempt) = initial_delay × attempt`）。若 3 次都失败，抛出 `StepExecutionError`。

```yaml
- id: geocode-address
  use: network.geocode
  params:
    addresses: ["北京市海淀区清华大学"]
...
若 `when` 表达式因解析错误、AST 白名单检查不通过或引用无法求值而无法得到布尔结果，框架记录警告日志并将结果视为 `False`（步骤跳过）。引用会被绑定为受限命名空间中的占位变量（如 `_when_ref_0`），解析/求值失败时在日志中回显：

```
WARNING: Cannot evaluate when='$check.nonexistent > 0' (resolved='_when_ref_0 > 0'), treating as False
```
