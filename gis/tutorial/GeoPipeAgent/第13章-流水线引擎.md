...
GeoPipeAgent 的流水线引擎由 `engine/` 目录下 9 个模块组成（经重构按职责拆分：`parser`、`validator`、`executor`、`step_runner`、`conditions`、`policies`、`context`、`references`、`reporter`），本章深入解析每个模块的内部机制，帮助开发者理解框架工作原理、调试问题和进行二次开发。
...
校验通过（或抛出 PipelineValidationError）
    ↓ executor.py（协调 step_runner / conditions / policies / context+references）
执行所有步骤，收集结果
    ↓ reporter.py
JSON 执行报告
```
...
## 13.4 `context.py` 与 `references.py`：上下文与引用解析

**职责**：`context.py` 维护流水线执行状态；引用解析逻辑已拆分到 `references.py` 的 `ReferenceResolver`（`PipelineContext` 委托调用）。
...
class PipelineContext:
    ...
    _resolver: ReferenceResolver # 引用解析器（references.py）

    def set_output(step_id, result)  # 步骤完成后存储结果
    def get_output(step_id)          # 获取步骤结果（未执行则抛 VariableResolutionError）
    def resolve(value)               # 解析值（变量替换/步骤引用）→ _resolver.resolve
    def resolve_params(params)       # 递归解析整个 params 字典（含嵌套 dict/list）
```

### 引用解析算法（`ReferenceResolver.resolve`）
...
def resolve(self, value, context):
    ...
    reference = parse_reference(value)  # 整串恰好是一个引用才解析
    if reference is not None:
        return self._resolve_reference(reference, context)

    if "${" in value:                   # 嵌入式变量 → 字符串插值
        return _FULL_VARIABLE_PATTERN.sub(..., value)

    return value                        # 普通字符串原样返回
```

`parse_reference` 使用模式 `\$\{(\w+)\}` 与 `\$(\w[\w-]*(?:\.\w+)?)` 的 **fullmatch**：步骤引用只支持一层属性（`$step-id` 或 `$step-id.attr`），`attr` 缺省为 `output`：

```python
def _resolve_reference(self, reference, context):
    if reference.kind == "variable":
        return context.variables[reference.name]   # 未定义则抛 VariableResolutionError
    result = context.get_output(reference.name)
    return getattr(result, reference.attribute or "output")  # 访问 StepResult 属性
```
...
**导航**：[← 第十二章：数据质检步骤](https://znlgis.github.io/gis/tutorial/GeoPipeAgent/第12章-数据质检步骤/) ｜ [第十四章：多后端系统 →](https://znlgis.github.io/gis/tutorial/GeoPipeAgent/第14章-多后端系统/)
