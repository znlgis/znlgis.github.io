...
引用解析由 `engine/references.py` 中的 `ReferenceResolver` 完成（`context.py` 的 `PipelineContext.resolve()` 委托给它）。正则模式 `\$\{(\w+)\}|\$(\w[\w-]*(?:\.\w+)?)` 要求**整个字符串恰好是一个引用**才会解析：

```
参数值处理顺序：
1. 非字符串值 → 直接返回（保持原类型）
2. 整串恰好匹配 "$step-id" 或 "$step-id.attr" → 步骤引用（attr 缺省为 output）
3. 包含 "${" → 变量替换
   3a. 整个值是 "${var}" → 返回原始类型
   3b. "${var}" 嵌入字符串 → 字符串插值
4. 其他 → 原样返回
...
- 步骤引用只支持**一层**属性（`$step-id.attr`，attr 为单个单词）。多级写法如 `$step-id.stats.feature_count` 不匹配引用模式，会被当作普通字符串原样传入步骤参数；正确写法是直接访问 stats 键：`$step-id.feature_count`
...
**导航**：[← 第四章：YAML 流水线格式](https://znlgis.github.io/gis/tutorial/GeoPipeAgent/第04章-YAML流水线格式/) ｜ [第六章：条件执行与错误策略 →](https://znlgis.github.io/gis/tutorial/GeoPipeAgent/第06章-条件执行与错误策略/)
