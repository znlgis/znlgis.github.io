...
| 可选依赖 | scipy、scikit-learn、matplotlib（空间分析）；networkx、geopy（网络分析） |
...
├──────────────────────────────────────────────────────────┤
│                     执行引擎层                             │
│   executor.py │ step_runner.py │ conditions/policies      │
│   context.py  │ references.py（上下文与引用解析）          │
├──────────────────────────────────────────────────────────┤
│                     步骤注册层                             │
...
│   │   ├── validator.py       # 流水线校验
│   │   ├── executor.py        # 流水线执行调度
│   │   ├── step_runner.py     # 单步骤解析、校验与调用
│   │   ├── conditions.py      # when 条件安全求值
│   │   ├── policies.py        # 重试与错误策略
│   │   ├── context.py         # 执行上下文
│   │   ├── references.py      # 变量与步骤引用解析
...
**导航**：[← 教程目录](https://znlgis.github.io/gis/tutorial/GeoPipeAgent/) ｜ [第二章：安装与环境配置 →](https://znlgis.github.io/gis/tutorial/GeoPipeAgent/第02章-安装与环境配置/)
