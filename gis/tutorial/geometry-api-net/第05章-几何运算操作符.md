...
{
    // 整圆离散段数默认值（Esri Java 默认每 6° 一段 = 60；此处取 100 以提高精度）
    ...
    // 精确控制圆的离散段数（最小值会被钳制为 8）
    public Polygon Execute(Geometry geometry, double distance, int circleSegments,
                           SpatialReference? spatialRef = null);
}
```

> `GeometryEngine.Buffer(geometry, distance)` 使用默认 `DefaultCircleSegments`；需要更高/更低精度时，直接调用 `BufferOperator.Instance.Execute(geometry, distance, circleSegments)`。负距离实现内缩（erosion）。
...
Buffer 现在实现为**真正的圆形缓冲区**——即以半径 `distance` 的圆盘对几何做 Minkowski 和，圆按 `circleSegments` 段离散化为正多边形，顶点处用圆弧（round join）连接。核心思路如下：

| 输入类型 | 缓冲结果 |
|----------|----------|
| Point / MultiPoint | 以该点为圆心的正多边形圆（`CircleRing`） |
| Line / Polyline | 每条线段生成两端半圆 + 两条平行边构成的胶囊（`StadiumRing`），线段端点补 `CircleRing`，再对所有片元做并 |
| Polygon / Envelope | 对每个环向外偏移得到 `OffsetRing`，再经稳健裁剪清理（保留绕数非零区域、按最小包含关系归并洞） |
| 负距离 | 向内收缩（erosion，`BufferNegative`） |
                       ...
                       SpatialReference? spatialRef = null)
{
    ...
    if (distance == 0)    return geometry.Copy();
    ...
    switch (geometry)
    {
        case Point p:
            // 点 → 正多边形圆
            return ToPolygon(new List<Ring> { CircleRing(p.X, p.Y, Math.Abs(distance), circleSegments) });

        case Polyline line:
            // 每段 → 胶囊（Stadium）+ 端点圆，随后并集
            var rings = new List<Ring>();
            foreach (var (a, b) in line.Segments())
            {
        ...
        case Polygon polygon when distance < 0:
            return BufferNegative(polygon.Rings(), -distance, circleSegments);   // 内缩

        case Polygon polygon:
            return BufferPolygon(polygon.Rings(), distance, circleSegments);     // 外扩 + 稳健清理
        // Envelope 走 Polygon 分支
    }
    ...
}
```

对于**复杂多边形**（自交、近相切接缝、大量凹角），单靠通用叠加不够稳健：实现采用双策略并以“输出面积 ⊇ 原面积、不越 Steiner 上界、且真正覆盖原形所有顶点”作为不变式门控——先尝试 **snap-rounding 平面叠加**（`SnapRoundOverlay.RobustUnion`，把交点吸附到网格以消除尺度抖动造成的假接缝），失败时回退到**片元顺序折叠**（把向内缩的条带 quad 与凸角圆盘切成简单片元，逐批布尔合并）。这样既保证简单场景不降速，又让重度自交的偏移环也能得到稳健结果，并与 PostGIS/GEOS 对拍验证。

> 提示：早期版本对点/包络采用“正方形缓冲区”的简化实现，现已全部替换为上述真圆缓冲。
