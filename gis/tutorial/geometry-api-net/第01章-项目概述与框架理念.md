...
> 上述 9 个谓词（外加 `IsSimpleOGC`）均由内部的 **DE-9IM 九交矩阵引擎**（`RelateEngine` / `RelateOps`）统一驱动，已对 OGC-SF 语料与 PostGIS 逐条验证，对任意几何类型组合给出精确结果，而非仅包络判断。

### 1.4.3 几何运算操作符

| 操作符 | 类名 | 说明 |
|--------|------|------|
| 缓冲区 | BufferOperator | 真圆缓冲区（Minkowski 圆盘近似，可配 `circleSegments`；负距离内缩；复杂多边形稳健叠加） |
...
| 简化(OGC) | SimplifyOGCOperator | 按 OGC 规范简化 |
| 邻近分析 | Proximity2DOperator | 最近坐标/顶点查询（GetNearestCoordinate、GetNearestVertex、GetNearestVertices） |
...
> 集合操作由内部 **`SetOpsCore` + 稳健绕数裁剪器 `PolygonClipper`** 统一驱动：面×面为真布尔结果，支持带洞与多部件，已与 PostGIS 对拍；不再退化为包络近似。
...
| 格式 | 导入 | 导出 | 说明 |
|------|------|------|------|
| WKT / EWKT | ✅ | ✅ | Well-Known Text；EWKT 带 `SRID=…;` 前缀 |
| WKB / EWKB | ✅ | ✅ | Well-Known Binary；EWKB 覆盖 PostGIS Z/M/SRID 高位标志 |
| GeoJSON | ✅ | ✅ | GeoJSON 格式 |
...
<div style="display: flex; justify-content: space-between; margin: 20px 0;">
  ...
  <a href="https://znlgis.github.io/gis/tutorial/geometry-api-net/第02章-快速开始指南/" style="text-decoration: none;">下一章 →</a>
</div>
