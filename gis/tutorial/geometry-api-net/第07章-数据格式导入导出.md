...
| 格式 | 导入 | 导出 | 用途 |
|------|------|------|------|
| WKT / EWKT | ✅ | ✅ | 人类可读的文本格式；EWKT 带 `SRID=…;` 前缀 |
| WKB / EWKB | ✅ | ✅ | 紧凑的二进制格式；EWKB 覆盖 PostGIS Z/M/SRID 高位标志 |
| GeoJSON | ✅ | ✅ | Web 应用和 API |
...
public static class WktExportOperator
{
    ...
    // EWKT：带 "SRID=<srid>;" 前缀
    public static string ExportToWkt(Geometry geometry, int srid);
}
```
...
{
    // 同时接受 WKT 与 EWKT；若输入含 "SRID=…;" 前缀会自动剥离并解析
...
### 7.2.7 EWKT（Extended WKT）

EWKT 是 PostGIS 对 WKT 的扩展，在几何体前加上 `SRID=<id>;` 前缀以携带空间参考。本库的导入导出对 EWKT 提供对称支持：
...
string ewkt = WktExportOperator.ExportToWkt(point, 4326);
// 结果: "SRID=4326;POINT (116.4074 39.9042)"

// 导入：EWKT 的 "SRID=…;" 前缀会被自动剥离并解析（普通 WKT 亦可）
var geom = WktImportOperator.ImportFromWkt("SRID=4326;POINT (116.4074 39.9042)");
```
...
### 7.3.7 EWKB（Extended WKB）

PostGIS 的 `ST_AsEWKB` 会在标准 WKB 的类型字上复用高位标志来携带维度与 SRID。本库的 WKB 导入器完整覆盖 EWKB 变体：

- **Z / M 标志**：支持 PostGIS 的 `0x80000000`（Z）、`0x40000000`（M）以及 ISO 风格的 `1000/2000/3000` 维度编码；
- **SRID 标志**：`0x20000000`，其后紧跟 4 字节 SRID；
- **Multi 子几何**：每个子几何带独立的维度/SRID 头，逐一解析。

```csharp
using OpenGIS.Esri.Geometry.Core.IO;

// 直接解析 PostGIS 输出的 EWKB（含 Z 值与 SRID）
byte[] ewkb = /* reader["geom"] as byte[]  ->  ST_AsEWKB(geom) */;
var geometry = WkbImportOperator.ImportFromWkb(ewkb);   // Z 坐标被保留，M/SRID 标志被正确识别
```

> 说明：`ExportToWkb` 目前导出的是标准 WKB（可选大端），导入侧则同时兼容标准 WKB 与 EWKB。
