                       ...
                       SpatialReference? spatialReference = null)
{
    ...
    // 其余（含复杂多边形）：交由稳健布尔引擎做真正的多边形裁剪，而非包络近似
    return SetOpsCore.Intersection(geometry1, geometry2);
}
```

> 上面的 `Envelope + Envelope`、`MultiPoint + Envelope` 只是几个可手算的快捷分支；实际实现把复杂几何统一交给内部的 `SetOpsCore`（稳健绕数叠加裁剪器 `PolygonClipper`），支持带洞与多部件，并已与 PostGIS 逐条对拍。
...
// 复杂多边形的差集由稳健布尔引擎（SetOpsCore + PolygonClipper）精确计算，
// 支持带洞与多部件，已与 PostGIS 对拍
...
// SymmetricDifference 等价于 Union(A-B, B-A)，二者结果一致（由同一稳健引擎计算）
```
...
集合操作对全部几何类型均提供完整支持：点/多点/包络走可手算的快捷分支，复杂多边形则统一交由内部**稳健布尔裁剪引擎**（`SetOpsCore` + `PolygonClipper` 绕数叠加）计算，支持带洞与多部件，并已与 PostGIS 逐条对拍验证，不再使用包络近似。
...
<div style="display: flex; justify-content: space-between; margin: 20px 0;">
  <a href="https://znlgis.github.io/gis/tutorial/geometry-api-net/第05章-几何运算操作符/" style="text-decoration: none;">← 上一章</a>
  <a href="https://znlgis.github.io/gis/tutorial/geometry-api-net/" style="text-decoration: none;">目录</a>
  <a href="https://znlgis.github.io/gis/tutorial/geometry-api-net/第07章-数据格式导入导出/" style="text-decoration: none;">下一章 →</a>
</div>
