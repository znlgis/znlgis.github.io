...
### 9.6.1 库内部日志（OguLogging）

OGU4Net 内置了基于 `Microsoft.Extensions.Logging.Abstractions` 的库级日志门面 `OguLogging`。默认不产生任何输出（`NullLoggerFactory`），用于接收库内部此前被静默丢弃的诊断信息（例如写入失败的要素、解析失败的坐标行等）。
...
配置后，`GdalReader`、`GdalWriter`、`PostgisUtil`、`GtTxtUtil` 等组件内部被捕获的异常会以 Debug/Warning 级别输出，便于排查那些过去被静默忽略的边界情况。库默认不输出任何日志，因此未配置时对现有应用零影响。
...
<div style="display: flex; justify-content: space-between; margin: 20px 0;">
  <a href="https://znlgis.github.io/gis/tutorial/opengis-utils-for-net/第08章-坐标系管理与转换/" style="text-decoration: none;">← 上一章</a>
  <a href="https://znlgis.github.io/gis/tutorial/opengis-utils-for-net/" style="text-decoration: none;">目录</a>
  <a href="https://znlgis.github.io/gis/tutorial/opengis-utils-for-net/第10章-实用工具类详解/" style="text-decoration: none;">下一章 →</a>
</div>
