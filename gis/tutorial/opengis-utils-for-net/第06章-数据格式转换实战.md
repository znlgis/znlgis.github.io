...
写入 GeoJSON 时 GdalWriter 自动附加图层创建选项 `COORDINATE_PRECISION=15` 与 `WRITE_NULL_FIELDS=YES`，全为 null 的属性字段也能在 SHP→GeoJSON→SHP 往返中保留。
...
// GeoPackage可以包含多个图层
// 向已存在的数据源追加要素使用 GdalWriter.Append（见下方说明）
```

`GdalWriter.Append` 可向已存在的数据源（含 GPKG）追加要素而不删除原数据源；输入字段必须能映射到目标图层，非零 FID 在驱动支持时尽量保留：

```csharp
var writer = new OpenGIS.Utils.Engine.GdalWriter();
writer.Append(layer, "output/cities.gpkg", layerName: "my_cities");
```
...
```csharp
// 写入 .gdb 时 GdalWriter 默认选择 OpenFileGDB 驱动
// （GDAL 3.6+ 支持创建，不再依赖 ESRI FileGDB SDK）
...
// 若运行环境缺少 OpenFileGDB 写入能力，可退回其他格式
// OguLayerUtil.WriteLayer(
//     DataFormatType.GEOPACKAGE,
//     layer,
//     "output/sample.gpkg"
...
> **覆盖行为**：默认写入**不覆盖**已存在的同名表。需要覆盖时，在 options 中传 `overwrite = true`（等价于图层创建选项 `OVERWRITE=YES`），文件驱动同样接受该选项：
>
> ```csharp
> var options = new Dictionary<string, object> { { "overwrite", true } };
> OguLayerUtil.WriteLayer(DataFormatType.POSTGIS, layer, connStr,
>     layerName: "public.cities", options: options);
> ```
>
> **连接串注意**：PostGIS 连接串整体不能加引号。写成 `PG:"host=..."` 会让 GDAL 把引号当作参数名的一部分而连接失败。`OguLayerUtil.WriteLayer(DataFormatType.POSTGIS, ...)` 与 `PostgisUtil.WritePostGIS` 均通过 `PG:` 前缀选择 PostgreSQL 驱动。
...
// 例如：显式读写表、表存在性检查、空间索引创建
```

PostgisUtil 还提供 `ReadPostGIS`、`WritePostGIS`、`TableExists`、`CreateSpatialIndex` 等专用方法，详见第10章 10.7 节。
...
> **日期字段注意**：KML 规范没有日期类型，原生 KML 写驱动遇到 `DATE`/`DATETIME` 字段会导致全部要素写入失败。`GdalWriter` 会自动把日期列降级为文本列，内容以 ISO 格式（`yyyy-MM-dd` / `yyyy-MM-ddTHH:mm:ss`）保留。
...
> **DXF 写入说明**：
> - DXF 驱动使用固定实体属性 schema，不支持任意自定义字段。字段创建失败时 `GdalWriter` 会跳过该字段并记录告警（需配置 `OguLogging.LoggerFactory` 可见），而不是让整次写入失败。
> - DXF 写入依赖 GDAL_DATA 中的模板文件 `header.dxf`。v1.0.8 修复了 GDAL_DATA 查找路径按线程缓存陈旧导致的间歇性写入失败（"Failed to find template header file header.dxf"），`GdalConfiguration` 现在会显式定位 MaxRev 部署的 `runtimes/any/native/gdal-data` 目录并在配置后重置查找缓存（`FinderClean`）。
...
### 6.9.5 单行坐标解析（TryParseTxtLine）

`GtTxtUtil.LoadTxt` 遇到无法解析的坐标行时会抛 `FormatParseException`，不再静默丢弃输入。当需要逐行解析且不希望抛异常时，可使用非抛异常的 `TryParseTxtLine`：

```csharp
using OpenGIS.Utils.DataSource;

// 逐行解析坐标（不抛异常）
string line = "J1\t1\t456789.123\t4321098.456\t\t界址点";
...
{
    Console.WriteLine("该行无法解析");
}
```

旧的 `ParseTxtLine` 方法仍然保留，空行或无效输入继续返回 `null`，以维持兼容性。`SaveTxt` 会为缺失的"点号"自动填充要素 Fid、为缺失的"圈号"填充 "1"，保证保存后的文件能被 `LoadTxt` 重新读取。
...
<div style="display: flex; justify-content: space-between; margin: 20px 0;">
  <a href="https://znlgis.github.io/gis/tutorial/opengis-utils-for-net/第05章-GDAL引擎架构设计/" style="text-decoration: none;">← 上一章</a>
  <a href="https://znlgis.github.io/gis/tutorial/opengis-utils-for-net/" style="text-decoration: none;">目录</a>
  <a href="https://znlgis.github.io/gis/tutorial/opengis-utils-for-net/第07章-几何处理与空间分析/" style="text-decoration: none;">下一章 →</a>
</div>
