...
public static class GdalConfiguration
{
    ...
    public static void ConfigureGdal()
    {
        lock (_lock)
        {
            ...
            // 清除可能冲突的系统级 GDAL 环境变量（GDAL_DATA/PROJ_LIB/OSGEO4W_ROOT 等）
            // 必须在任何 GDAL/OGR 调用之前定位并设置 GDAL_DATA：
            // GDAL 的 CPLFindFile 按线程缓存查找路径快照，首次调用时指向错误目录
            // 会导致 DXF 等驱动报 "Failed to find template header file header.dxf"
            var gdalDataDir = ConfigureGdalData();
            
            // 将正确目录显式传给 MaxRev，并完成驱动注册
            GdalBase.ConfigureGdalDrivers(gdalDataDir);
            
            // 完成 PROJ 等剩余配置
            GdalBase.ConfigureAll();
            
            // MaxRev 可能用错误目录覆盖 config option，此处重新断言 GDAL_DATA
            // 并调用 Gdal.FinderClean() 清除按线程缓存的查找路径快照
...
{
    /// <summary>
    /// 默认坐标系容差（投影坐标系使用；地理坐标系由 CrsUtil 返回更小的角度容差）
    /// </summary>
...
public class GdalReader : ILayerReader
{
    ...
    private OguLayer ReadOgrLayer(Layer ogrLayer, ...)
    {
        ...
        while ((ogrFeature = ogrLayer.GetNextFeature()) != null)
        {
            using (ogrFeature)
            {
                var feature = ReadOgrFeature(ogrFeature, layer.Fields);  // Fid 取自 ogrFeature.GetFID()
...
3. **过滤支持**：属性过滤和空间过滤（无效过滤器抛出 `FormatParseException`，不静默忽略）
4. **FID 保留**：读取结果保留源数据的 FID（超出 Int32 范围时报错）
5. **编码保护**：`options["encoding"]` 读取 Shapefile 时串行化进程级 `SHAPE_ENCODING` 配置，读取后恢复原值
...
<div style="display: flex; justify-content: space-between; margin: 20px 0;">
  <a href="https://znlgis.github.io/gis/tutorial/opengis-utils-for-net/第02章-快速入门与环境配置/" style="text-decoration: none;">← 上一章</a>
  <a href="https://znlgis.github.io/gis/tutorial/opengis-utils-for-net/" style="text-decoration: none;">目录</a>
  <a href="https://znlgis.github.io/gis/tutorial/opengis-utils-for-net/第04章-统一图层模型详解/" style="text-decoration: none;">下一章 →</a>
</div>
