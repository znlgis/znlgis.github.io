...
public static class ProjectionUtil
{
    ...
    public static int GetBestProjectedCrs(OguLayer layer)
    {
        if (layer.Features.Count == 0)
            return 4527; // 默认返回CGCS2000 3度带39带（EPSG:4527）
...
**真实数据集成测试（RealDataHarness）**

仓库的 `tests/OpenGIS.Utils.RealDataHarness` 项目提供了一个可复用的真实数据集成测试工具，用于在真实 Shapefile/PostGIS 数据上验证库的七类能力：

- **读取**：递归发现目录下的 `.shp`/`.dbf` 配对，独立于 GDAL 从 DBF/SHP 头解析期望要素数与几何类型
- **3D 几何**、**CRS**、**格式转换**、**几何运算**、**写回往返**、**PostGIS 往返**

`RealDataChecks` 不内置任何数据集名称、路径或坐标范围假设。`RealDataIntegrationTests` 通过 `OGU_REAL_DATA_DIR` 环境变量复用这些检查（未设置时自动跳过），`RealDataPostgisTests` 通过 `OGU_POSTGIS_CONN` 增加 PostGIS 维度。PostGIS 维度创建的所有表都带 `ogu_test_` 前缀，并在 `finally` 块中删除。

```csharp
// 在 xUnit 测试中复用真实数据检查
// 设置环境变量后运行：
//   OGU_REAL_DATA_DIR=D:\gisdata dotnet test
//   OGU_POSTGIS_CONN="PG:host=... dbname=... user=... password=..." dotnet test
```
...
<div style="display: flex; justify-content: space-between; margin: 20px 0;">
  <a href="https://znlgis.github.io/gis/tutorial/opengis-utils-for-net/第11章-开发实战案例/" style="text-decoration: none;">← 上一章</a>
