...
```csharp
// PostgisUtil 未提供连接串构建辅助方法，按 GDAL PostgreSQL 驱动格式直接拼接：
// PG:host=<host> port=<port> dbname=<database> user=<user> password=<password>

string connStr = $"PG:host=localhost port=5432 dbname=mydb user=postgres password=secret";

// 注意：整个连接串不能加引号（PG:"host=..." 会让 GDAL 把引号当作参数名的一部分而连接失败）
```

### 10.7.2 连接测试

```csharp
// PostgisUtil 未提供独立的 TestConnection 方法；
// 可用 TableExists 探活：数据源打不开时抛 DataSourceException，
// 只有连接成功且库中不含该表才返回 false
try
{
    bool exists = PostgisUtil.TableExists(connStr, "__ogu_conn_probe__");
    Console.WriteLine("连接成功");
}
catch (OpenGIS.Utils.Exception.DataSourceException)
{
...
```csharp
// 读取 PostGIS 表为图层
OguLayer layer = PostgisUtil.ReadPostGIS(connStr, "cities");

// 写入图层到 PostGIS 表
PostgisUtil.WritePostGIS(layer, connStr, "cities_copy");
```

> **连接串注意**：PostGIS 连接串必须能被 GDAL PostgreSQL 驱动识别，且**整体不能加引号**。写成 `PG:"host=..."` 会让 GDAL 把引号当作参数名的一部分而连接失败。正确写法：`PG:host=127.0.0.1 port=5432 dbname=postgres user=postgres password=postgres`。

### 10.7.4 判断表是否存在

```csharp
// 判断表是否存在
bool exists = PostgisUtil.TableExists(connStr, "cities");
if (exists)
{
...
{
    Console.WriteLine("表不存在，可以安全建表");
}
```

> **注意**：`TableExists` 在数据源**打不开时抛 `DataSourceException`**，只有连接成功且库中不含该表时才返回 `false`。这样"连接失败"与"表不存在"两种结果可区分，避免上层误判为可以安全建表。

### 10.7.5 创建空间索引

```csharp
// 创建 GIST 空间索引（默认几何列 wkb_geometry）
PostgisUtil.CreateSpatialIndex(connStr, "cities");

// 几何列名非默认时，可显式指定
PostgisUtil.CreateSpatialIndex(connStr, "cities", "geom");

// 传 null 时自动从 geometry_columns 探测实际列名
PostgisUtil.CreateSpatialIndex(connStr, "cities", null);
```

`CreateSpatialIndex` 通过 GDAL/OGR 的 PostgreSQL 驱动执行 `CREATE INDEX IF NOT EXISTS` 的 GIST 索引 DDL，因此对同一张表重复调用是**幂等**的。表名和几何列名仅允许字母、数字和下划线；数据库用户需具备创建索引的权限。

## 10.8 GdalCmdUtil GDAL命令工具

### 10.8.1 查看数据源结构（模拟 ogrinfo）

`GdalCmdUtil` 不启动外部 ogr2ogr 进程，而是通过 OGR 绑定在进程内模拟 ogrinfo/gdalinfo 输出：

```csharp
using OpenGIS.Utils.Engine.Util;

// 获取数据源的图层、字段、要素数等信息（等效 ogrinfo -so）
string info = GdalCmdUtil.ExecuteOgrInfo("data/cities.shp");
Console.WriteLine(info);

// 对矢量数据 ExecuteGdalInfo 委托给 ExecuteOgrInfo
string info2 = GdalCmdUtil.ExecuteGdalInfo("data/cities.shp");

// 获取 FileGDB 的图层结构（返回 GdbGroupModel，含所有图层名）
GdbGroupModel gdb = GdalCmdUtil.GetGdbDataStructure("data/sample.gdb");
foreach (var name in gdb.LayerNames)
{
    Console.WriteLine($"GDB图层: {name}");
}
```
...
```csharp
// GdalCmdUtil 不提供 ogr2ogr 重投影封装（无 ReprojectLayer/GetOgr2OgrPath 方法）。
// 库内坐标转换使用 CrsUtil；也可自行调用系统 ogr2ogr：
...
<div style="display: flex; justify-content: space-between; margin: 20px 0;">
  <a href="https://znlgis.github.io/gis/tutorial/opengis-utils-for-net/第09章-异常处理体系/" style="text-decoration: none;">← 上一章</a>
  <a href="https://znlgis.github.io/gis/tutorial/opengis-utils-for-net/" style="text-decoration: none;">目录</a>
  <a href="https://znlgis.github.io/gis/tutorial/opengis-utils-for-net/第11章-开发实战案例/" style="text-decoration: none;">下一章 →</a>
</div>
